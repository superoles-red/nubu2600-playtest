(() => {
  ...
  function canPlace(candidate, ignoreIds = [], options={}){const ignored=new Set(ignoreIds);if(candidate.x<0||candidate.y<0||candidate.x+candidate.w>state.level.size.width||candidate.y+candidate.h>state.level.size.height)return{ok:false,message:'Предмет выходит за границы уровня.'};const overlap=state.level.objects.find(object=>!ignored.has(object.id)&&rectsOverlap(candidate,object)&&!overlapAllowed(candidate,object));if(overlap)return{ok:false,message:`Здесь уже находится «${TYPE_DEFS[overlap.type]?.label||overlap.type}».`};return options.checkOwnPair===false?{ok:true}:ownPairPlacement(candidate);}
  ...
  function makeLibraryCard(slot,{campaign=false}={}){
    ...
    title.textContent=campaign?displayTitle:`№${slot.sequence} · ${displayTitle}`;

(() => {
  ...
  function defaultProps(type) {
    switch (type) {
      ...
      case 'movingPlatform': return { path: [], speedCellsPerSecond: 2.4, enabled: true, loop: false, repeatSpacing: 20 };
  ...
  function renderMobilePalette() {
    ...
    for (const category of MOBILE_PALETTE_CATEGORIES) {
      if (category.id === 'other' && isMobilePlayerMode()) continue;
  ...
  function handlePointerMove(event){if(state.pointers.has(event.pointerId))state.pointers.set(event.pointerId,{x:event.clientX,y:event.clientY});if(state.pinch){updatePinch();return;}const point=pointerGridPoint(event);state.hoverPoint=point;$('cursorReadout').style.display='none';$('cursorStatus').textContent='';
    ...
    if(state.drag){state.drag.current=point;if(state.drag.kind==='move'){state.drag.deleteCandidate=!pointInsideCanvas(event.clientX,event.clientY);canvas.classList.toggle('will-delete',state.drag.deleteCandidate);}if(state.drag.kind==='pathEndpoint')canvas.style.cursor='grabbing';renderCanvas();}
  ...
  function renderContextToolbar(){const root=$('contextToolbar'),handles=$('resizeHandles'),object=selectedObject();if(!root||!handles||!object||state.tool==='link'){if(root)root.hidden=true;if(handles)handles.hidden=true;return;}const def=TYPE_DEFS[object.type],add=(icon,label,action,className='',active=false,id='')=>{const button=contextIconButton(icon,label,action,className,active,id);root.append(button);return button;};root.innerHTML='';root.hidden=false;
    ...
    if(object.type==='smartPlatform'){add('＋',`Добавить узел маршрута (сейчас ${(object.props?.path||[]).length})`,()=>contextMutation('Узел маршрута добавлен',()=>appendTramNode(object),'Добавлен новый узел трамвая'),'context-variant',false,'route-add');if((object.props?.path||[]).length>2)add('−','Убрать последний узел маршрута',()=>contextMutation('Узел маршрута удалён',()=>removeTramNode(object),'Последний узел удалён'),'context-variant',false,'route-remove');}
    ...
    if(object.type==='enemyFlyer'){const value=Number(object.props?.distance)||5,label=value<=2?'Близко':value>=10?'Далеко':'Средне';add(directionArrow(object.props?.direction||'right'),`Направление и расстояние: ${label}`,()=>cycleContextNumber(object,'distance',[2,5,10],'Расстояние',''),'context-variant',false,'distance');}
  ...
  function showInteractionHint(objectOrItem){const root=$('interactionHint');if(!root)return;const type=objectOrItem?.type,def=TYPE_DEFS[type],label=objectOrItem?.label||def?.label||'Поле уровня',help=TYPE_HELP[type]||'Тяните пустое место, чтобы двигать карту.';root.querySelector('strong').textContent=label;root.querySelector('span').textContent=help;}
  ...
  async function renderLibrary(){await refreshUserSlots();const root=$('libraryList'),campaignRoot=$('campaignLibraryList');root.innerHTML='';campaignRoot.innerHTML='';$('userSlotCount').textContent=`${state.userSlots.length} / ${MAX_USER_LEVELS}`;$('libraryLevelTitleInput').value=state.level?.title||'';$('libraryDifficultySelect').value=state.difficulty;const campaignSlots=(await dbGetAll()).filter(slot=>slot.kind==='campaign'&&Number(slot.episode||1)===Number($('libraryEpisodeSelect').value||1)).sort((a,b)=>a.sequence-b.sequence);for(const slot of campaignSlots){const open=document.createElement('button');open.type='button';open.className='campaign-level-button';open.textContent=`${slot.episode}-${slot.sequence} · ${slot.title}`;open.addEventListener('click',async()=>{closeLibrary();await loadSlot(slot.key,$('libraryDifficultySelect').value);});campaignRoot.append(open);}if(!state.userSlots.length){const empty=document.createElement('div');empty.className='empty-state';empty.textContent='Пока нет пользовательских уровней. Включите случайную заготовку по желанию и создайте новый уровень.';root.append(empty);}for(const slot of state.userSlots){const status=playerSlotStatus(slot);const card=document.createElement('article');card.className='level-card-wrap';card.dataset.status=status.key;const open=document.createElement('button');open.type='button';open.className='level-card';const thumb=document.createElement('canvas');thumb.width=64;thumb.height=64;const info=document.createElement('div');const title=document.createElement('strong');title.textContent=`№${slot.sequence} · ${slot.title}`;const meta=document.createElement('small');meta.textContent=`${slot.difficulties.easy.size.width}×${slot.difficulties.easy.size.height} · ${new Date(slot.updatedAt).toLocaleDateString('ru-RU')}`;const statusChip=document.createElement('small');statusChip.className='level-status';statusChip.dataset.status=status.key;statusChip.textContent=status.label;info.append(title,meta,statusChip);open.append(thumb,info);open.addEventListener('click',async()=>{closeLibrary();await loadSlot(slot.key,'easy');});const remove=document.createElement('button');remove.type='button';remove.className='level-card-delete';remove.textContent='Удалить';remove.addEventListener('click',()=>deleteUserSlot(slot.key));card.append(open,remove);root.append(card);drawLevelThumbnail(thumb,slot.difficulties.easy);}}
  ...
  function bindUi(){renderPalette();renderFavorites();renderMobilePalette();
    ...
    $('libraryButton').addEventListener('click',openLibrary);$('mobileLevelButton').addEventListener('click',openLibrary);$('closeLibraryButton').addEventListener('click',closeLibrary);$('newUserLevelButton').addEventListener('click',()=>createUserSlot($('randomStarterCheckbox').checked));$('randomStarterButton').addEventListener('click',()=>createUserSlot(true));$('manualSaveButton').addEventListener('click',()=>saveNow({revision:true}).then(()=>toast('Уровень сохранён.','ok')));$('editLevelButton').addEventListener('click',()=>{$('libraryLevelTitleInput').focus();$('libraryLevelTitleInput').select();});$('libraryLevelTitleInput').addEventListener('change',event=>{const title=String(event.target.value).trim().slice(0,48)||'Без названия';mutate('Название изменено',()=>{state.level.title=title;state.slot.title=title;});renderLibrary();});$('libraryCopyMapButton').addEventListener('click',copyMap);$('libraryPasteMapButton').addEventListener('click',pasteMap);$('shareLevelButton').addEventListener('click',()=>shareCurrentLevel().catch(error=>toast(error.message,'error')));$('receiveLevelInput').addEventListener('change',receiveSharedLevel);$('libraryEpisodeSelect').addEventListener('change',renderLibrary);$('libraryDifficultySelect').addEventListener('change',async event=>{if(state.slot?.kind==='user')await switchDifficulty(event.target.value);await renderLibrary();});$('examLevelButton').addEventListener('click',async()=>{if(state.slot?.kind!=='user'){toast('Экзамен доступен только для уровней игрока.','error');return;}const ok=await confirmAction('Экзамен уровня','Уровень запустится с обычного входа. Чтобы получить статус «Опубликован», нужно дойти до выхода без смерти.');if(!ok)return;try{localStorage.setItem(EXAM_PENDING_KEY,JSON.stringify({slotKey:state.slotKey,difficulty:state.difficulty}));}catch(error){}closeLibrary();playLevel();});document.querySelectorAll('[data-size]').forEach(button=>button.addEventListener('click',()=>{document.querySelectorAll('[data-size]').forEach(item=>item.classList.toggle('active',item===button));state.chosenSize=button.dataset.size.split('x').map(Number);}));const updateChosenSize=()=>{state.chosenSize=[Number($('newLevelWidth').value),Number($('newLevelHeight').value)];};$('newLevelWidth').addEventListener('change',updateChosenSize);$('newLevelHeight').addEventListener('change',updateChosenSize);

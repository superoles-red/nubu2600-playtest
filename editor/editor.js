(() => {
  ...
  function drawConnections(cell) {
    ...
    for(const object of state.level.objects){
      ...
      if(object.type==='smartPlatform'){
        ctx.setLineDash([]);
        for(let index=0;index<points.length;index++){const point=points[index],invalid=color==='#ff6974';ctx.beginPath();ctx.arc((point.x+object.w/2)*cell,(point.y+object.h/2)*cell,6,0,Math.PI*2);ctx.fillStyle=invalid?'#ff6974':index===0?color:TRAM_BEND_COLOR;ctx.fill();ctx.lineWidth=2;ctx.strokeStyle=invalid?'#ffe1e4':index===0?'#08100d':color;ctx.stroke();}
  ...
  function drawWireDrag(){const drag=state.wireDrag;if(!drag)return;const rect=canvas.getBoundingClientRect(),source=state.level.objects.find(object=>object.id===drag.sourceId);if(!source)return;const cell=cellPixels(),plug=linkSocketGeometry(source,source.x*cell,source.y*cell,source.w*cell,source.h*cell);drawLinkEndpointStem(plug,'#65ff9a',4);ctx.save();ctx.strokeStyle='#65ff9a';ctx.lineWidth=4;ctx.setLineDash([7,5]);ctx.beginPath();ctx.moveTo(plug.cx,plug.cy);ctx.lineTo(drag.clientX-rect.left,drag.clientY-rect.top);ctx.stroke();ctx.restore();}
  ...
  function handlePointerUp(event){state.pointers.delete(event.pointerId);if(state.pinch){if(state.pointers.size<2)state.pinch=null;return;}if(state.pan){if(state.pan.pointerId!==event.pointerId)return;state.pan=null;viewport.classList.remove('dragging');return;}const drag=state.drag;if(drag&&drag.pointerId!==event.pointerId)return;state.drag=null;if(!drag)return;
    ...
    else if(drag.kind==='pathNode'){const current=state.level.objects.find(object=>object.id===drag.object.id),next=pathNodeFromDrag(drag),previous=drag.object.props.path[drag.nodeIndex];if(current&&previous&&(next.x!==previous.x||next.y!==previous.y)){const candidatePath=current.props.path.map((point,index)=>index===drag.nodeIndex?next:point);mutate('Узел маршрута перемещён',()=>{current.props.path=candidatePath;});}}

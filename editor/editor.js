(() => {
  ...
  const DB_VERSION = 2;
  const ARCHIVE_STORE_NAME = 'campaignArchives';
  ...
  const HINT_VISIBILITY_KEY = 'nubu2600.editor.hints-hidden.v1';
  const GRID_MARKING_KEY = 'nubu2600.editor.grid-marking.v1';
  ...
  const LIBRARY_SECTIONS = Object.freeze({
    ...
    'campaign-3': { title:'Эпизод 3 · эскизы', selector:'3', prefix:'campaign-ep3', folder:'ep3', episode:3 },
    'campaign-4': { title:'Эпизод 4 · эскизы', selector:'4', prefix:'campaign-ep4', folder:'ep4', episode:4 },
    'campaign-5': { title:'Эпизод 5 · эскизы', selector:'5', prefix:'campaign-ep5', folder:'ep5', episode:5 },
    'campaign-6': { title:'Эпизод 6 · эскизы', selector:'6', prefix:'campaign-ep6', folder:'ep6', episode:6 },
    'campaign-7': { title:'Эпизод 7 · эскизы', selector:'7', prefix:'campaign-ep7', folder:'ep7', episode:7 },
    'campaign-8': { title:'Эпизод 8 · эскизы', selector:'8', prefix:'campaign-ep8', folder:'ep8', episode:8 },
  ...
  const UNIQUE_TYPES = new Set(['spawn','exit','secretExit']);
  ...
  const AUTHORING_WIDTH_CAPS = Object.freeze({ oneWay:8, fragilePlatform:8, blinkPlatform:8, movingPlatform:8, fallingPlatform:8, conveyor:16, bouncePad:5, smartPlatform:4 });
  ...
  const MOBILE_PALETTE_LONG_PRESS_MS = 240;
  const MOBILE_PALETTE_GESTURE_SLOP = 6;
  const MOBILE_PALETTE_OUTWARD_DRAG_SLOP = 11;
  const MOBILE_PALETTE_OUTWARD_DOMINANCE = 1.15;
  ...
  const BOSS_KINDS = { ratSpider: { label: 'Бункерная крыса — паук', size: [5, 5] } };
  function bossKindDefinition(kind){return typeof kind==='string'&&Object.prototype.hasOwnProperty.call(BOSS_KINDS,kind)?BOSS_KINDS[kind]:null;}

  const TYPE_DEFS = {
    boss: { label: 'Босс', color: '#eda65f', layer: 'entity', group: 'Предметы', fixedSize: BOSS_KINDS.ratSpider.size },
    ...
    movingPlatform: { label: 'Подъёмник', color: '#f3b562', layer: 'gameplay', group: 'Платформы', resize: 'x', defaultSize: [4, 1] },
    ...
    crusherWall: { label: 'Пресс', color: '#f04c61', layer: 'hazard', group: 'Опасности', resize: 'xy', defaultSize: [2, 2] },
    ...
    spawn: { label: 'Вход', color: '#55e39e', layer: 'meta', group: 'Мета', fixedSize: [3, 3], protected: true },
    exit: { label: 'Выход', color: '#ff84cd', layer: 'meta', group: 'Мета', fixedSize: [3, 3], protected: true },
    secretExit: { label:'Секретный выход', color:'#55e39e', layer:'meta', group:'Мета', fixedSize:[3,3] },
  ...
  const PALETTE_ITEMS = [
    ...
    ...['door', 'solid', 'oneWay', 'fragilePlatform', 'blinkPlatform', 'smartPlatform', 'fallingPlatform', 'conveyor', 'bouncePad', 'driftField', 'spike', 'crusherWall'].map(type => ({ id: type, type })),
    { id:'movingPlatform', type:'movingPlatform', label:'Подъёмник', color:'#f3b562', preset:{loop:false} },
    ...
    { id:'secretExit',type:'secretExit',campaignOnly:true },
  ];
  ...
  const MOBILE_FORMER_SPECIAL_IDS = ['collectible','unlockSwitch','pickup-abilities','heartVendor','developerNote','secretExit'];
  const MOBILE_COMPACT_LABELS = { collectible:'Значок', 'pickup-abilities':'Навыки', developerNote:'Заметка',secretExit:'Секретный выход' };
  const MOBILE_PALETTE_CATEGORIES = [
    { id:'base', label:'Основа', icon:'▦', items:['solid','coin','boss','label','pickup-gravity',...MOBILE_FORMER_SPECIAL_IDS] },
    { id:'platforms', label:'Платформы', icon:'═', items:['oneWay','fragilePlatform','blinkPlatform','movingPlatform','hoist','smartPlatform','fallingPlatform','bouncePad'] },
    { id:'hazards', label:'Опасности', icon:'▲', items:['spike','pushBlock','enemyGoomba','enemyFlyer','enemyLeech'] },
    { id:'mechanisms', label:'Механизмы', icon:'⚙', items:['button-toggle','door','conveyor','driftField','crusherWall','portal','playerCannon','generator'] },
  ];

  const PROPERTY_DEFS = {
    boss: [{ key:'bossKind', label:'Босс эпизода', type:'select', options:[['ratSpider','1 · Бункерная крыса — паук']] }],
    ...
    button: [
      { key:'bossOnly', label:'Только для босса', type:'checkbox' },
  ...
  const state = {
    ...
    touchSelectionSnapshot: null,
    fieldTapCandidate: null,
  ...
  function selectedObjectIds() { if(state.selectedId!==null&&state.selectedIds instanceof Set&&state.selectedIds.size&&state.selectedIds.has(state.selectedId))return state.selectedIds;return new Set(state.selectedId?[state.selectedId]:[]); }
  ...
  function constrainCrusherSize(object){if(object.type!=='crusherWall')return object;const sizes=[[2,2],[3,3],[4,4],[5,5],[2,5],[5,2],[2,10],[10,2]];const best=sizes.reduce((a,b)=>Math.hypot(b[0]-object.w,b[1]-object.h)<Math.hypot(a[0]-object.w,a[1]-object.h)?b:a);[object.w,object.h]=best;return object;}
  function constrainNewObjectWidth(object){constrainCrusherSize(object);const maximum=authoringWidthCap(object?.type);if(maximum&&object.w>maximum)object.w=maximum;return object;}
  ...
  function activeWireSource(){return state.level?.objects.find(object=>object.id===(state.wireDrag?.sourceId||state.linkSourceId))||selectedObject();}
  function isLinkTarget(target,source=activeWireSource()){return !!target&&(LINKABLE_TYPES.has(target.type)||source?.type==='boss'&&target.type==='button'&&target.props?.buttonType!=='H');}
  ...
  function normalizeDisconnectedDoors(level){for(const object of level.objects||[]){const motionLinked=(level.objects||[]).some(source=>isControlSource(source)&&(source.props?.targets||[]).some(target=>target===object.id||['@toggle','@on','@off','@invert'].some(suffix=>target===object.id+suffix)));if(!motionLinked){if(object.type==='crusherWall')object.props.enabled=true;if(object.type==='movingPlatform')object.props.moving=true;}if(incomingLinks(object.id,level).length)continue;if(object.type==='door')object.props.open=false;else if(['spike','blinkPlatform'].includes(object.type)&&object.props?.mode==='always')object.props.startActive=true;}return level;}
  function initialObjectEnabled(object,level=state.level){
    ...
    return object.props?.enabled!==false;
  }
  ...
  function defaultProps(type) {
    switch (type) {
      case 'boss': return { bossKind:'ratSpider', direction:'left', targets:[] };
      ...
      case 'fallingPlatform': return { speed: 'slow', triggerDelay: .5, respawnDelay: 2 };
      ...
      case 'secretExit': return {route:'secret'};
  ...
  function normalizeObject(raw, index = 0) {
    ...
    if (type === 'portal') {
      const side=normalizePortalSide(object.props.side,object.props.orientation==='horizontal'?'down':'right');
      object.props.side=side;
      const horizontal = ['up', 'down'].includes(side);
  ...
  function normalizePortalSide(value, fallback='right') {
    const aliases={up:'up',top:'up',u:'up',down:'down',bottom:'down',d:'down',b:'down',left:'left',l:'left',right:'right',r:'right'};
    return aliases[String(value||'').toLowerCase()]||fallback;
  }

  function portalWithSide(object, requestedSide, level = state.level) {
    const side=normalizePortalSide(requestedSide),horizontal=['up','down'].includes(side),next=normalizeObject({...deepClone(object),props:{...deepClone(object.props||{}),side,orientation:horizontal?'horizontal':'vertical',length:6}},0);
    if(level?.size){
      next.x=clamp(next.x,0,Math.max(0,level.size.width-next.w));next.y=clamp(next.y,0,Math.max(0,level.size.height-next.h));
      const footprints=[next,...portalShellFootprints(next)],minX=Math.min(...footprints.map(item=>item.x)),minY=Math.min(...footprints.map(item=>item.y)),maxX=Math.max(...footprints.map(item=>item.x+item.w)),maxY=Math.max(...footprints.map(item=>item.y+item.h));
      if(minX<0)next.x-=minX;else if(maxX>level.size.width)next.x-=maxX-level.size.width;
      if(minY<0)next.y-=minY;else if(maxY>level.size.height)next.y-=maxY-level.size.height;
    }
    return next;
  }

  function portalHalfTurn(object, level = state.level) {
    const opposite={up:'down',down:'up',left:'right',right:'left'},side=normalizePortalSide(object?.props?.side);
    return portalWithSide(object,opposite[side],level);
  }
  ...
  function normalizeLevel(raw, fallback = {}) {
    ...
    const level={
      ...
      title: String(raw.title || fallback.title || 'Без названия').slice(0, raw.metadata?.campaign?.layoutVersion===CAMPAIGN_LAYOUT_VERSION?96:48),
    ...
    for(const key of ['supersededDraft','supersededLibraryLevel','geometryMigrations'])delete level.metadata[key];
    ...
    return normalizeDisconnectedDoors(level);
  }
  ...
  function makeBlankLevel(width=20, height=20, title, difficulty = 'easy', withFloor = true, options={}) {
    ...
    const spawnPosition=firstShapePosition(shapeSeed,3,3,{x:1,y:Math.max(0,height-5)},floors)||{x:0,y:0};
    const spawn={id:'spawn-01',type:'spawn',...spawnPosition,w:3,h:3,layer:'meta',props:{semantics:'playerBody',anchor:'centerTop'}};
    const exitPosition=firstShapePosition(shapeSeed,3,3,{x:Math.max(0,width-4),y:Math.max(0,height-5)},[...floors,spawn])||{x:Math.max(0,width-3),y:0};
    const level = normalizeLevel({
      ...
      objects: [
        spawn,
        { id: 'exit-main', type:'exit', ...exitPosition, w:3, h:3, layer:'meta', props:{route:'main'} },
        ...floors,
      ],
  ...
  function campaignPendingDifficulties(slot){return slot?.metadata?.campaignComplete===false&&Array.isArray(slot.metadata.campaignPendingDifficulties)?slot.metadata.campaignPendingDifficulties.filter(difficulty=>DIFFICULTIES.includes(difficulty)):[];}
  function setCampaignPendingDifficulties(slot,pending){slot.metadata={...(slot.metadata||{}),campaignComplete:pending.length===0};if(pending.length)slot.metadata.campaignPendingDifficulties=pending;else delete slot.metadata.campaignPendingDifficulties;}
  function markCampaignSlotModified(slot,difficulty){if(slot?.kind!=='campaign'||!slot.metadata)return;delete slot.metadata.campaignPristineSeed;if(difficulty&&slot.metadata.campaignComplete===false)setCampaignPendingDifficulties(slot,campaignPendingDifficulties(slot).filter(candidate=>candidate!==difficulty));}
  function invalidateSlotVerification(slot,difficulty){if(slot?.clearProofs)delete slot.clearProofs[difficulty];if(slot?.kind==='user')slot.publicationStatus='draft';else markCampaignSlotModified(slot,difficulty);}
  function mergeCompletedCampaignDifficulties(target,source){
    if(target?.kind!=='campaign'||target.key!==source?.key||target.metadata?.campaignSourceVersion!==source.metadata?.campaignSourceVersion)return[];
    const pending=campaignPendingDifficulties(target),sourcePending=campaignPendingDifficulties(source);
    if(source.metadata?.campaignComplete===false&&!Array.isArray(source.metadata.campaignPendingDifficulties))return[];
    const completed=pending.filter(difficulty=>!sourcePending.includes(difficulty)&&source.difficulties?.[difficulty]);
    for(const difficulty of completed)target.difficulties[difficulty]=deepClone(source.difficulties[difficulty]);
    if(completed.length)setCampaignPendingDifficulties(target,pending.filter(difficulty=>!completed.includes(difficulty)));
    return completed;
  }
  function syncActiveCampaignCompletion(slot){
    const completed=mergeCompletedCampaignDifficulties(state.slot,slot);
    if(completed.includes(state.difficulty)&&!state.dirty){state.level=normalizeLevel(state.slot.difficulties[state.difficulty],{difficulty:state.difficulty,episode:state.slot.episode,sequence:state.slot.sequence});state.slot.difficulties[state.difficulty]=state.level;resetHistory();if(state.ready)refreshAll();}
  }
  ...
  function openDatabase() {
    return new Promise((resolve,reject)=>{
      ...
      const timer=setTimeout(()=>fail(new Error('Библиотека не ответила. Закройте другие вкладки NuBu2600 и откройте редактор снова.')),STORAGE_TIMEOUT_MS);
      request.onupgradeneeded=()=>{
        if(settled){try{request.transaction?.abort();}catch(error){}return;}
        const db=request.result;if(!db.objectStoreNames.contains(STORE_NAME))db.createObjectStore(STORE_NAME,{keyPath:'key'});if(!db.objectStoreNames.contains(ARCHIVE_STORE_NAME))db.createObjectStore(ARCHIVE_STORE_NAME,{keyPath:'key'});
      };
      ...
      request.onerror=()=>fail(request.error||new Error('Не удалось открыть библиотеку уровней.'));
      request.onblocked=()=>fail(new Error('Обновление библиотеки заблокировано другой вкладкой NuBu2600. Закройте старые вкладки и откройте редактор снова.'));
    });
  }
  ...
  async function dbGetCampaignArchives(){return requestToPromise(state.db.transaction(ARCHIVE_STORE_NAME,'readonly').objectStore(ARCHIVE_STORE_NAME).getAll());}
  async function restoreCampaignArchives(archives){
    const valid=(Array.isArray(archives)?archives:[]).filter(record=>typeof record?.key==='string'&&Array.isArray(record.slots));if(!valid.length)return;
  ...
  async function migrateCampaignLayout(){
    ...
    await completion;
    if(archivedEmergency)try{if(localStorage.getItem(EMERGENCY_DRAFT_KEY)===emergencyRaw)localStorage.removeItem(EMERGENCY_DRAFT_KEY);}catch(error){}
    return archivedCount;
  }
  async function dbPut(value) {
    const tx=state.db.transaction(STORE_NAME,'readwrite'),store=tx.objectStore(STORE_NAME),completion=transactionDone(tx);
    if(value?.kind==='campaign'&&campaignPendingDifficulties(value).length){
      const request=store.get(value.key);request.onsuccess=()=>{mergeCompletedCampaignDifficulties(value,request.result);store.put(value);};request.onerror=()=>{try{tx.abort();}catch(error){}};
    }else store.put(value);
    await completion;syncActiveCampaignCompletion(value);return value;
  }
  ...
  async function dbInstallCampaignSeed(value,{expectedSourceVersion=null,allowPartialRefresh=false}={}) {
    ...
    request.onsuccess=()=>{
      ...
      const canCompletePartial=allowPartialRefresh
        && current?.kind==='campaign'
        && current.metadata?.campaignComplete===false
        && campaignPendingDifficulties(current).length>0
        && String(current.metadata?.campaignSourceVersion||'')===String(value.metadata?.campaignSourceVersion||'');
      if(!canRefresh&&!canCompletePartial)return;
      if(canCompletePartial&&!canRefresh){mergeCompletedCampaignDifficulties(current,value);current.updatedAt=Date.now();store.put(current);written=true;return;}
  ...
  function writeLibraryMirrorNow({waitMs=0}={}){const operation=state.mirrorQueue.then(async()=>{if(!state.db||!navigator.storage?.getDirectory)return false;try{const [slots,campaignArchives]=await Promise.all([dbGetAll(),dbGetCampaignArchives()]),deletedSlots=readDeletionTombstones(),payload={kind:'nubu.library-backup',version:1,exportedAt:Date.now(),deletedSlots,slots,campaignArchives};const root=await navigator.storage.getDirectory();const handle=await root.getFileHandle(LIBRARY_MIRROR_FILE,{create:true});const writer=await handle.createWritable();await writer.write(`${JSON.stringify(payload)}\n`);await writer.close();return true;}catch(error){return false;}});state.mirrorQueue=operation.catch(()=>false);return waitMs>0?withTimeout(operation,waitMs,'Резервное зеркало ещё закрывает предыдущую запись').catch(()=>false):operation;}
  ...
  async function recoverLibraryMirror(){
    const payload=await readLibraryMirror();if(!payload)return 0;
    await restoreCampaignArchives(payload.campaignArchives);if(!payload.slots?.length)return 0;
    const deletedSlots={...(payload.deletedSlots||{}),...readDeletionTombstones()},legacy=payload.slots.filter(slot=>slot?.key&&!deletedSlots[slot.key]&&oldCampaignLayout(slot));
    if(legacy.length){await restoreCampaignArchives([{key:`ep1-mirror-${stableHash(legacy)}`,episode:1,layoutVersion:1,archivedAt:Number(payload.exportedAt)||Date.now(),reason:'Зеркальная копия прежней нумерации эпизода 1',slots:legacy}]);state.mirrorLegacyArchivedCount=legacy.length;}
    let restored=0;
    for(const candidate of payload.slots){
      if(!candidate?.key||!candidate.difficulties||deletedSlots[candidate.key]||oldCampaignLayout(candidate))continue;
      const current=await dbGet(candidate.key);if(current&&Number(current.updatedAt)>=Number(candidate.updatedAt))continue;
      const slot=deepClone(candidate);for(const difficulty of DIFFICULTIES)if(slot.difficulties[difficulty]?.kind==='nubu.level')slot.difficulties[difficulty]=normalizeLevel(slot.difficulties[difficulty],{difficulty,episode:slot.episode||1,sequence:slot.sequence||1});
      await dbPut(slot);restored++;
    }
    return restored;
  }
  function downloadLibraryPayload(payload,filename){const blob=new Blob([`${JSON.stringify(payload)}\n`],{type:'application/json'}),url=URL.createObjectURL(blob),anchor=document.createElement('a');anchor.href=url;anchor.download=filename;document.body.append(anchor);anchor.click();anchor.remove();setTimeout(()=>URL.revokeObjectURL(url),1000);}
  async function exportLibrary(){await saveNow({revision:true});const [slots,campaignArchives]=await Promise.all([dbGetAll(),dbGetCampaignArchives()]);downloadLibraryPayload({kind:'nubu.library-backup',version:1,exportedAt:Date.now(),slots,campaignArchives},`nubu2600-library-${new Date().toISOString().slice(0,10)}.json`);toast('Скачана внешняя резервная копия библиотеки и архивов.','ok');}
  async function exportCampaignArchives(){
    const campaignArchives=await dbGetCampaignArchives();if(!campaignArchives.length){toast('Архивов прежней нумерации нет.');return;}
    const slots=campaignArchives.flatMap((archive,batch)=>{
      const originals=deepClone(archive.slots),emergency=archive.emergencyDraft;
      if(emergency?.level?.kind==='nubu.level'){
        let original=originals.find(slot=>slot.key===emergency.slotKey);
        if(!original){original={key:emergency.slotKey,title:emergency.level.title,episode:1,sequence:campaignSequenceFromKey(emergency.slotKey)||1,difficulties:Object.fromEntries(DIFFICULTIES.map(difficulty=>[difficulty,deepClone(emergency.level)])),revisions:[]};originals.push(original);}
        original.difficulties[emergency.difficulty||'easy']=deepClone(emergency.level);
      }
      return originals.map((original,index)=>{const slot=deepClone(original);slot.key=`archive-ep1-${batch}-${archive.archivedAt}-${index}`;slot.kind='user';slot.title=`Архив · ${original.title||original.key}`.slice(0,48);slot.publicationStatus='draft';slot.clearProofs={};slot.metadata={...(slot.metadata||{}),archivedFrom:original.key,archiveKey:archive.key};for(const difficulty of DIFFICULTIES)if(slot.difficulties?.[difficulty]){slot.difficulties[difficulty].id=`${slot.key}-${difficulty}`;slot.difficulties[difficulty].title=slot.title;}return slot;});});
    downloadLibraryPayload({kind:'nubu.library-backup',version:1,exportedAt:Date.now(),slots,campaignArchives},`nubu2600-ep1-archive-${new Date().toISOString().slice(0,10)}.json`);toast('Архив скачан. Его карты можно восстановить отдельными копиями в «Мои уровни».','ok');
  }
  async function importLibrary(event){const file=event.target.files?.[0];event.target.value='';if(!file)return;try{
    if(file.size>64*1024*1024)throw new Error('файл больше 64 МБ');const payload=JSON.parse(await file.text());
    ...
    const ok=await confirmAction('Восстановить библиотеку?',`${valid.length} наборов карт будут восстановлены. Карты эпизода с прежней нумерацией сохранятся в отдельный архив; совпадающие ключи остальных карт заменятся содержимым копии.`);if(!ok)return;
    await saveNow({revision:true});const legacy=valid.filter(oldCampaignLayout);if(legacy.length)await restoreCampaignArchives([{key:`ep1-import-${Date.now()}-${stableHash(legacy)}`,episode:1,layoutVersion:1,archivedAt:Date.now(),reason:'Импорт библиотеки с прежней нумерацией',slots:legacy}]);const restoredSlots=valid.filter(candidate=>!oldCampaignLayout(candidate)).map(candidate=>{const slot=deepClone(candidate);for(const difficulty of DIFFICULTIES)slot.difficulties[difficulty]=normalizeLevel(slot.difficulties[difficulty],{difficulty,episode:slot.episode||1,sequence:slot.sequence||1});return slot;});
    clearTimeout(state.mirrorTimer);state.mirrorTimer=null;await restoreCampaignArchives(payload.campaignArchives);await dbPutAll(restoredSlots);for(const slot of restoredSlots)removeDeletionTombstone(slot.key);
  ...
  async function fetchCampaignLevel(sequence, difficulty, source = 'campaign-1') {
    ...
    if(source==='lobby')difficulty='easy';
  ...
  async function ensureCampaignSlot(key,{preferredDifficulty='easy',fast=false}={}) {
    ...
    if(existing&&!refreshPristine&&existing.metadata?.campaignComplete!==false)return existing;
    ...
    const operation=(async()=>{
      ...
      const difficulties=source==='lobby'?['easy']:fast?[priority]:DIFFICULTIES;
      ...
      const installed=await dbGet(key);syncActiveCampaignCompletion(installed);return installed;
    })();
  ...
  function seedCampaignInBackground() {
    ...
    const run=(async()=>{
      ...
      const missing=Array.from({length:CAMPAIGN_LEVEL_COUNT},(_,index)=>index+1).filter(sequence=>{const slot=existingSlots.get(campaignSlotKey(sequence));return !slot||slot.metadata?.campaignComplete===false||(!!sourceVersion&&slot.metadata?.campaignPristineSeed===true&&String(slot.metadata?.campaignSourceVersion||'')!==sourceVersion);});
  ...
  function mutate(label, callback, options={}) {
    ...
    try { callback(); normalizeDisconnectedDoors(state.level); }
  ...
  function restoreHistory(index) {
    ...
    else markCampaignSlotModified(state.slot,state.difficulty);
    state.dirty = true;
    scheduleSave();
    refreshAll();
  }
  ...
  function refreshPlayerHeader() {
    ...
    if (title) title.textContent = 'Библиотека';
    if (status) { status.textContent = ''; status.hidden = true; }
    $('mobileLevelButton')?.setAttribute('aria-label', 'Открыть библиотеку уровней');
  }
  ...
  function syncEditorViewportLayout(){
    ...
    updateInteractionHintHeight(mode);
    if(mode!==state.layoutMode){state.layoutMode=mode;renderMobilePalette();requestAnimationFrame(fitLevel);}
    else renderCanvas();
    requestAnimationFrame(()=>{updateMobileCategoryScrollCue();updateLibraryScrollCue();});
  }
  ...
  async function loadSlot(key, difficulty = state.difficulty, { skipSave = false, skipCampaignEnsure = false } = {}) {
    ...
    try {
      ...
      const slot = campaignSequenceFromKey(key)
        ? (skipCampaignEnsure ? await dbGet(key) : await ensureCampaignSlot(key))
        : await dbGet(key);
      if (requestId !== state.loadRequestId) return false;
      if (!slot) throw new Error('Уровень не найден в локальной библиотеке.');
      state.slot = slot;
      state.slotKey = key;
      state.difficulty = librarySectionForKey(key)==='lobby' ? 'easy' : DIFFICULTIES.includes(difficulty) ? difficulty : 'easy';
      ...
      state.tool = 'select';state.selectionMode=false;state.selectedIds=new Set();
      resetHistory();
      ...
      refreshSelectors();renderPalette();renderMobilePalette();
      refreshAll();
  ...
  async function switchDifficulty(difficulty) {
    if (librarySectionForKey(state.slotKey)==='lobby' || !state.ready || state.loadingSlot || !DIFFICULTIES.includes(difficulty) || difficulty === state.difficulty) return;
    const requestId = ++state.loadRequestId;
    state.loadingSlot = true;
    setEditorReady(false);
    try {
  ...
  function drawGrid(cell, width, height) {
    ...
    const marking=state.gridMarking==='none'?null:state.gridMarking==='5'?5:4;
    for (let x = 0; x <= state.level.size.width; x++) { const major=!!marking&&x%marking===0;ctx.beginPath();ctx.strokeStyle=major?'#344a42':'#20322c';ctx.lineWidth=major?1.1:.65;ctx.moveTo(Math.round(x*cell)+.5,0);ctx.lineTo(Math.round(x*cell)+.5,height);ctx.stroke(); }
    for (let y = 0; y <= state.level.size.height; y++) { const major=!!marking&&y%marking===0;ctx.beginPath();ctx.strokeStyle=major?'#344a42':'#20322c';ctx.lineWidth=major?1.1:.65;ctx.moveTo(0,Math.round(y*cell)+.5);ctx.lineTo(width,Math.round(y*cell)+.5);ctx.stroke(); }
    ctx.restore();
  }

  function gridMarkingLabel(value=state.gridMarking){ return value==='none'?'Без разметки':`Сетка ${value}×${value}`; }
  function updateGridMarkingButton(){const button=$('gridMarkingButton');if(!button)return;button.textContent=gridMarkingLabel();button.title='Разметка сетки: 4×4 → 5×5 → без разметки';button.setAttribute('aria-label',button.title);}
  function cycleGridMarking(){const values=['4','5','none'],next=values[(values.indexOf(state.gridMarking)+1)%values.length];state.gridMarking=next;try{localStorage.setItem(GRID_MARKING_KEY,next);}catch(error){}updateGridMarkingButton();renderCanvas();}
  ...
  function portalShellFootprints(object) {
    if(object?.type!=='portal')return[];
    const horizontal=object.props?.orientation==='horizontal'||['up','down'].includes(object.props?.side),side=object.props?.side||(horizontal?'up':'right'),x=Number(object.x)||0,y=Number(object.y)||0,w=Number(object.w)||0,h=Number(object.h)||0;
    if(horizontal){
      const anchorY=side==='up'?y+h:y,backY=side==='up'?anchorY:anchorY-1,capY=side==='up'?anchorY-1:anchorY;
      return[
        {type:'portalShell',ownerId:object.id,x:x-1,y:backY,w:w+2,h:1},
        {type:'portalShell',ownerId:object.id,x:x-1,y:capY,w:1,h:1},
        {type:'portalShell',ownerId:object.id,x:x+w,y:capY,w:1,h:1},
      ];
    }
    const anchorX=side==='left'?x+w:x,backX=side==='left'?anchorX:anchorX-1,capX=side==='left'?anchorX-1:anchorX;
    return[
      {type:'portalShell',ownerId:object.id,x:backX,y:y-1,w:1,h:h+2},
      {type:'portalShell',ownerId:object.id,x:capX,y:y-1,w:1,h:1},
      {type:'portalShell',ownerId:object.id,x:capX,y:y+h,w:1,h:1},
    ];
  }

  function staticPlacementFootprints(object) {
    if(object?.type==='boss')return [object,...bossControls(object).map((control,index)=>({...control,type:'bossControl',id:object.id,ownerId:object.id,controlIndex:index}))];
    return object?.type==='portal'?[object,...portalShellFootprints(object)]:[object];
  }

  function bossControls(object) {
    const defaults=[{x:object.x||0,y:(object.y||0)+(object.h||5)-1,w:1,h:1},{x:(object.x||0)+(object.w||5)-1,y:(object.y||0)+(object.h||5)-1,w:1,h:1}];
    return defaults.map((fallback,index)=>{const raw=object.props?.controls?.[index]||fallback;return{x:Number.isFinite(Number(raw.x))?Math.round(Number(raw.x)):fallback.x,y:Number.isFinite(Number(raw.y))?Math.round(Number(raw.y)):fallback.y,w:clamp(Math.round(Number(raw.w)||1),1,4),h:1};});
  }
  function shiftedBossControls(object,dx,dy){return bossControls(object).map(control=>({...control,x:control.x+dx,y:control.y+dy}));}
  function showBossControlHint(index){showHintText(`Педаль ${index+1}`, 'Перетащите педаль отдельно. Кнопки − и + меняют ширину; корпус переносит обе педали вместе.');}
  function selectedBossControl(object=selectedObject()){const selection=state.selectedBossControl;return object?.type==='boss'&&selection?.id===object.id?bossControls(object)[selection.index]||null:null;}
  function bossControlAt(point,pointerType='mouse') {
    const radius=(pointerType==='touch'?15:7)/cellPixels(),hits=[];
    for(const object of state.level.objects.filter(object=>object.type==='boss'))bossControls(object).forEach((control,index)=>{
      const x=control.x+control.w/2,y=control.y+.75,distance=Math.hypot(point.rawX-x,point.rawY-y);
      const exact=point.rawX>=control.x&&point.rawX<=control.x+control.w&&point.rawY>=control.y&&point.rawY<=control.y+1;
      if(exact||distance<=radius)hits.push({object,index,distance,exact});
    });
    hits.sort((a,b)=>Number(b.exact)-Number(a.exact)||a.distance-b.distance);return hits[0]||null;
  }
  function bossControlCandidate(drag) {
    const candidate=deepClone(drag.object),control=bossControls(candidate)[drag.controlIndex],point=drag.current||drag.start;
    control.x=Math.round(point.rawX-drag.offsetX);control.y=Math.round(point.rawY-drag.offsetY);
    candidate.props.controls=bossControls(candidate).map((item,index)=>index===drag.controlIndex?control:item);return candidate;
  }
  function resizeBossControl(object,index,delta) {
    const candidate=deepClone(object),controls=bossControls(candidate);controls[index].w=clamp(controls[index].w+delta,1,4);candidate.props.controls=controls;
    const verdict=placementPreviewVerdict(candidate,[object.id]);if(!verdict.ok){toast(verdict.message,'error');return;}
    contextMutation('Ширина педали изменена',()=>{object.props.controls=controls;},`Педаль ${index+1}: ${controls[index].w} клеток`);
  }
  function drawBossControlHandles(cell) {
    const object=selectedObject();if(object?.type!=='boss')return;
    const candidate=state.drag?.kind==='bossControl'&&state.drag.object.id===object.id?bossControlCandidate(state.drag):object;
    const invalid=!placementPreviewVerdict(candidate,[object.id]).ok;
    ctx.save();ctx.setLineDash([3,3]);
    bossControls(candidate).forEach((control,index)=>{
      const active=state.selectedBossControl?.id===object.id&&state.selectedBossControl.index===index,cx=(control.x+control.w/2)*cell,cy=(control.y+.75)*cell;
      ctx.strokeStyle=invalid?'#ff6974':active?'#f1ff9a':'#67e2c5';ctx.lineWidth=active?2.5:1.3;ctx.strokeRect(control.x*cell,control.y*cell,control.w*cell,cell);
      ctx.beginPath();ctx.moveTo((candidate.x+candidate.w/2)*cell,(candidate.y+candidate.h/2)*cell);ctx.lineTo(cx,cy);ctx.stroke();
      ctx.setLineDash([]);ctx.fillStyle=invalid?'#ff6974':active?'#f1ff9a':'#67e2c5';ctx.beginPath();ctx.arc(cx,cy,Math.max(5,Math.min(9,cell*.35)),0,Math.PI*2);ctx.fill();ctx.fillStyle='#102018';ctx.font='bold 10px sans-serif';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(String(index+1),cx,cy);ctx.setLineDash([3,3]);
    });ctx.restore();
  }

  function drawPortalShell(context,object,x,y,w,h,options={}) {
    if(object?.type!=='portal'||options.mini)return;
    const cellX=w/Math.max(.001,Number(object.w)||1),cellY=h/Math.max(.001,Number(object.h)||1);
    context.save();
    context.fillStyle=options.invalid?'rgba(104,18,28,.92)':'#050505';
    context.strokeStyle=options.invalid?INVALID_PREVIEW_COLOR:(options.selected?'#f1ff9a':'rgba(116,143,132,.72)');
    context.lineWidth=options.selected?2:1;
    for(const shell of portalShellFootprints(object)){
      const px=x+(shell.x-object.x)*cellX,py=y+(shell.y-object.y)*cellY,pw=shell.w*cellX,ph=shell.h*cellY;
      context.fillRect(px,py,pw,ph);
      context.strokeRect(px+.5,py+.5,Math.max(0,pw-1),Math.max(0,ph-1));
    }
    context.restore();
  }

  function drawObjectShape(context, object, x, y, w, h, options = {}) {
    const def = {...(TYPE_DEFS[object.type] || { color:'#aeb8b3',label:object.type }), color:objectColor(object)||'#aeb8b3'};
    const selected = !!options.selected;
    const preview = !!options.preview;
    const mini = !!options.mini;
    context.save();
    const initiallyDisabled = LINKABLE_TYPES.has(object.type) && object.type!=='door' && !initialObjectEnabled(object,options.level||state.level);
    ...
    drawPortalShell(context,object,x,y,w,h,options);
    ...
    if (platformTypes.has(object.type)) {
      ...
      if (!options.plainPlatform && (object.type === 'movingPlatform' || object.type === 'smartPlatform')) { context.fillStyle = '#160f24'; context.font = `900 ${Math.max(8, thin * .8)}px sans-serif`; context.textAlign = 'center'; context.fillText(object.type==='movingPlatform'&&object.props?.loop===false?'↓':'↔', x + w / 2, y + thin); }
      ...
      if(!options.testPoint){context.fillStyle='#10191f';context.fillRect(x,y,w,h);context.strokeStyle='#718493';context.strokeRect(x+.5,y+.5,w-1,h-1);}const bodyX=options.testPoint?x:x+w/3,bodyY=y,bodyW=options.testPoint?w:w/3,bodyH=options.testPoint?h:h*2/3;context.fillStyle='#ecf4ff';context.fillRect(bodyX,bodyY,bodyW,bodyH);context.fillStyle='#203150';context.fillRect(bodyX+bodyW*.22,bodyY+bodyH*.3,Math.max(1,bodyW*.18),Math.max(1,bodyH*.1));context.fillRect(bodyX+bodyW*.6,bodyY+bodyH*.3,Math.max(1,bodyW*.18),Math.max(1,bodyH*.1));
    } else if (object.type === 'exit'||object.type==='secretExit') {
      context.fillStyle = withAlpha(def.color,.25); context.fillRect(x,y,w,h); context.strokeRect(x+.5,y+.5,w-1,h-1); context.fillStyle=def.color; context.fillRect(x+w*.2,y+h*.12,w*.6,h*.12);if(object.type==='secretExit'){context.font=`900 ${Math.max(7,Math.min(12,w*.2))}px sans-serif`;context.textAlign='center';context.textBaseline='middle';context.fillText(mini?'С':'Секрет',x+w/2,y+h*.6,Math.max(1,w-4));}
    } else if (object.type === 'boss') {
      context.strokeStyle=selected?'#f1ff9a':'#d98d51';context.setLineDash([4,3]);context.strokeRect(x+.5,y+.5,w-1,h-1);context.setLineDash([]);
      const cx=x+w*.5,cy=y+h*.43;context.lineWidth=Math.max(2,Math.min(w,h)*.055);context.strokeStyle='#d7a978';
      for(const side of [-1,1])for(let leg=0;leg<3;leg++){context.beginPath();context.moveTo(cx+side*w*.17,cy+(leg-1)*h*.09);context.lineTo(cx+side*w*(.33+.04*leg),y+h*(.18+.25*leg));context.lineTo(cx+side*w*.45,y+h*(.28+.25*leg));context.stroke();}
      context.fillStyle='#66584f';context.beginPath();context.ellipse(cx,cy,w*.26,h*.27,0,0,Math.PI*2);context.fill();context.strokeStyle='#efbf7d';context.stroke();
      context.fillStyle='#b0a197';for(const side of [-1,1]){context.beginPath();context.ellipse(cx+side*w*.105,cy-h*.13,w*.07,h*.065,0,0,Math.PI*2);context.fill();}
      context.fillStyle='#c4b7a8';context.beginPath();context.ellipse(cx,cy,w*.13,h*.105,0,0,Math.PI*2);context.fill();context.fillStyle='#ff6c55';for(const side of [-1,1])context.fillRect(cx+side*w*.075-w*.025,cy-h*.025,w*.05,Math.max(1,h*.025));
      const padH=Math.max(3,h/Math.max(1,object.h||5)*.25);context.fillStyle='#67e2c5';if(mini){const padW=w/Math.max(2,object.w||5);context.fillRect(x,y+h-padH,padW,padH);context.fillRect(x+w-padW,y+h-padH,padW,padH);}else{const unit=w/object.w;for(const control of bossControls(object))context.fillRect(x+(control.x-object.x)*unit,y+(control.y+1-object.y)*unit-padH,control.w*unit,padH);}
      if(!mini){context.fillStyle='#ffdc94';context.font=`bold ${Math.max(8,Math.min(12,w*.13))}px sans-serif`;context.textAlign='center';context.fillText('WJ',cx,y+h*.83);}
      ...
      const linked = incomingLinks(object.id,options.level||state.level).length > 0;
      if(object.props?.open && linked){context.globalAlpha*=.56;context.setLineDash([5,4]);context.strokeRect(x+.5,y+.5,w-1,h-1);context.setLineDash([]);}else{context.fillRect(x,y,w,h);context.strokeRect(x+.5,y+.5,w-1,h-1);context.strokeStyle='rgba(20,15,8,.5)';for(let offset=4;offset<(object.props?.orientation==='horizontal'?h:w);offset+=5){context.beginPath();if(object.props?.orientation==='horizontal'){context.moveTo(x,y+offset);context.lineTo(x+w,y+offset);}else{context.moveTo(x+offset,y);context.lineTo(x+offset,y+h);}context.stroke();}}
  ...
  function objectColor(object){return object.type==='movingPlatform'?((object.props||object.preset)?.loop===false?'#f3b562':'#69c8ff'):TYPE_DEFS[object.type]?.color;}
  function movingPlatformLabel(object){return object.type==='movingPlatform'&&(object.props||object.preset)?.loop!==false?'Лифт':TYPE_DEFS[object.type]?.label||object.type;}
  function liftRouteLimit(object){return object.type==='movingPlatform'&&object.props?.loop!==false?10:Infinity;}
  function clampLiftRoutePoint(object,anchor,desired){
    const limit=liftRouteLimit(object);
    if(Math.hypot(desired.x-anchor.x,desired.y-anchor.y)<=limit)return desired;
    let best=null;
    for(let x=Math.max(0,Math.ceil(anchor.x-limit));x<=Math.min(state.level.size.width-object.w,Math.floor(anchor.x+limit));x++)for(let y=Math.max(0,Math.ceil(anchor.y-limit));y<=Math.min(state.level.size.height-object.h,Math.floor(anchor.y+limit));y++){
      if(Math.hypot(x-anchor.x,y-anchor.y)>limit+1e-9)continue;
      const distance=Math.hypot(x-desired.x,y-desired.y);if(!best||distance<best.distance)best={x,y,distance};
    }
    return best?{x:best.x,y:best.y}:desired;
  }
  function fallingLiftGhosts(object,end=pathEnd(object)){
    if(object.type!=='movingPlatform'||object.props?.loop!==false)return[];
    const distance=Math.hypot(end.x-object.x,end.y-object.y),count=Math.max(1,Math.ceil((distance-1e-9)/8));
    return Array.from({length:count-1},(_,i)=>{const t=8*(i+1)/distance;return{x:object.x+(end.x-object.x)*t,y:object.y+(end.y-object.y)*t};});
  }
  function constrainPathEndpoint(object, end, { allowOutside = false } = {}) {
    ...
    if (allowOutside && outside && !['movingPlatform','crusherWall'].includes(object.type)) return snapped;
    const constrained = {
      x: clamp(snapped.x, 0, state.level.size.width - object.w),
      y: clamp(snapped.y, 0, state.level.size.height - object.h),
    };
    Object.assign(constrained,clampLiftRoutePoint(object,object,constrained));
    if (object.type === 'movingPlatform') {
      ...
      for(const x of new Set(axisCandidates(constrained.x,object.x,object.w,maxX)))for(const y of new Set(axisCandidates(constrained.y,object.y,object.h,maxY))){
        ...
        if(Math.max(dx,dy)<4||Math.hypot(dx,dy)>liftRouteLimit(object)||rectsOverlap(object,endpoint))continue;
  ...
  function pathEndpointPlacement(object, end) {
    if(!routeLengthAllowed(object,[object,end]))return{ok:false,message:'Максимальная длина маршрута лифта — 10 клеток.'};
    ...
    if (object.type === 'movingPlatform' && Math.max(Math.abs(end.x-object.x), Math.abs(end.y-object.y)) < 4) return { ok:false, message:'Маршрут должен проходить минимум 4 клетки хотя бы по одной оси.' };
  ...
  function drawConnections(cell) {
    ...
    for(const object of state.level.objects){
      ...
      const color=object.type==='smartPlatform'?(pairedPathPlacement(object,points).ok?objectColor(object):'#ff6974'):(pathEndpointPlacement(object,end).ok?objectColor(object):'#ff6974');
  ...
  function renderCanvas() {
    ...
    if (state.testSpawn) drawObjectShape(ctx,{type:'spawn',props:{}},state.testSpawn.x*cell,state.testSpawn.y*cell,cell,cell*2,{preview:true,testPoint:true});
    const preview = dragPreview();
    if (preview) {const previewInvalid=state.drag?.kind==='erase'||!placementPreviewVerdict(preview).ok,previewColor=previewInvalid?'#ff6974':undefined;drawObjectShape(ctx,preview,preview.x*cell,preview.y*cell,preview.w*cell,preview.h*cell,{preview:true,color:previewColor,invalid:previewInvalid});if(PATH_ENDPOINT_TYPES.has(preview.type))drawPairedPlacementPreview(preview,cell,previewColor||objectColor(preview),previewInvalid);}
    ...
    if(state.drag?.kind==='moveGroup'){const result=groupMovePreview(state.drag);for(const candidate of result.candidates){drawObjectShape(ctx,candidate,candidate.x*cell,candidate.y*cell,candidate.w*cell,candidate.h*cell,{preview:true,selected:true,color:result.ok?undefined:'#ff6974',invalid:!result.ok});if(PATH_ENDPOINT_TYPES.has(candidate.type))drawPairedPlacementPreview(candidate,cell,result.ok?objectColor(candidate):'#ff6974',!result.ok);}}
  ...
  function drawPathEndpointHandle(cell) {
    ...
    ctx.fillStyle = placement.ok ? objectColor(object) : '#ff6974';
  ...
  function renderPalette(filter = '') {
    ...
    for(const item of PALETTE_ITEMS){if(item.type==='secretExit'&&state.slot?.kind!=='campaign')continue;const def=TYPE_DEFS[item.type];const label=item.label||def.label;const help=item.help||TYPE_HELP[item.type]||'Предмет игрового уровня.';const itemGroup=item.campaignOnly?'Только сюжет':def.group;if(query&&!label.toLocaleLowerCase('ru').includes(query))continue;if(itemGroup!==group){group=itemGroup;const title=document.createElement('div');title.className='palette-group-title';title.textContent=group;root.append(title);}const button=document.createElement('button');button.type='button';button.className=`palette-tool ${item.campaignOnly?'campaign-only':''}`;button.dataset.paletteId=item.id;button.dataset.toolSize=(def.fixedSize||def.defaultSize||[1,1]).join('x');button.draggable=true;button.style.setProperty('--tool-color',item.color||def.color);button.title=`${label}: ${help}`;const icon=document.createElement('canvas');icon.width=68;icon.height=52;const text=document.createElement('small');text.textContent=label;button.append(icon,text);if(item.type==='coin'){const badge=document.createElement('span');badge.className='coin-cap';badge.textContent=`0/${state.level?coinLimit():5}`;button.append(badge);}const explain=()=>{const panel=$('paletteHelp');if(panel)panel.innerHTML=`<strong>${label}</strong><p>${help}</p>`;showInteractionHint(item);};button.addEventListener('pointerenter',explain);button.addEventListener('focus',explain);button.addEventListener('pointerup',event=>{if(event.pointerType==='touch')toast(`${label}: ${help}`);});button.addEventListener('click',()=>{explain();toast(`${label}: ${help}`);});button.addEventListener('dragstart',event=>{setTool('select');state.desktopPaletteDrag={paletteId:item.id};state.hoverPoint=null;event.dataTransfer.setData('text/nubu-tool',item.id);event.dataTransfer.effectAllowed='copy';});button.addEventListener('dragend',()=>{state.desktopPaletteDrag=null;state.hoverPoint=null;renderCanvas();});root.append(button);requestAnimationFrame(()=>drawToolIcon(icon,item));}
    updateToolButtons();
  }

  function updateScrollCue(scroller,cue,axis='vertical'){
    if(!scroller||!cue)return;
    const vertical=axis==='vertical',client=vertical?scroller.clientHeight:scroller.clientWidth,scrollSize=vertical?scroller.scrollHeight:scroller.scrollWidth,position=vertical?scroller.scrollTop:scroller.scrollLeft,overflow=scrollSize-client;
    cue.classList.toggle('vertical',vertical);
    cue.classList.toggle('horizontal',!vertical);
    cue.hidden=overflow<=2||client<=0;
    if(cue.hidden)return;
    requestAnimationFrame(()=>{
      if(cue.hidden)return;
      const track=vertical?cue.clientHeight:cue.clientWidth,thumb=Math.min(track,Math.max(28,track*client/scrollSize)),offset=overflow>0?(track-thumb)*clamp(position/overflow,0,1):0,handle=cue.querySelector('span');
      if(!handle)return;
      if(vertical){handle.style.height=`${thumb}px`;handle.style.width='';handle.style.transform=`translateY(${offset}px)`;}
      else{handle.style.width=`${thumb}px`;handle.style.height='';handle.style.transform=`translateX(${offset}px)`;}
    });
  }

  function updateMobileCategoryScrollCue(){updateScrollCue($('mobileCategoryItems'),$('mobileCategoryScrollCue'),state.layoutMode==='portrait'?'horizontal':'vertical');}
  function updateLibraryScrollCue(){const scroller=$('libraryScroll'),cue=$('libraryScrollCue');if(scroller&&cue){cue.style.top=`${scroller.offsetTop+8}px`;cue.style.height=`${Math.max(0,scroller.clientHeight-16)}px`;}updateScrollCue(scroller,cue,'vertical');}
  function resetMobileCategoryHint(category){
    const help=$('mobileCategoryHelp');
    $('mobileCategoryInstruction').textContent='Смахните список · предмет тяните на поле';
    $('mobileCategoryTitle').textContent=category?.label||'Предметы';
  ...
  function showMobileCategoryHint(item){
    ...
    if(!def)return;
    const label=item.label||def.label,copy=item.help||TYPE_HELP[item.type]||'Предмет игрового уровня.';
  ...
  function renderMobilePalette() {
    ...
    document.documentElement.classList.toggle('mobile-category-open',state.mobilePaletteExpanded);
    itemsRoot.innerHTML = '';
    if (!state.mobilePaletteExpanded) { $('mobileCategoryScrollCue').hidden=true;resetMobileCategoryHint();updateToolButtons();return; }
    const category = MOBILE_PALETTE_CATEGORIES.find(candidate => candidate.id === state.mobileCategory) || MOBILE_PALETTE_CATEGORIES[0];
    resetMobileCategoryHint(category);
    let visibleItemCount=0;
    for (const id of category.items) {
      const item = PALETTE_BY_ID.get(id);
      if (!item||item.type==='secretExit'&&state.slot?.kind!=='campaign') continue;
      visibleItemCount++;
      const def = TYPE_DEFS[item.type];
      const variant = document.createElement('button');
      variant.type = 'button';
      variant.className = 'mobile-variant-button';
      variant.classList.toggle('mobile-former-special',MOBILE_FORMER_SPECIAL_IDS.includes(id));
    ...
    requestAnimationFrame(updateMobileCategoryScrollCue);
    updateToolButtons();
  }
  ...
  function setTool(tool){const selectionRequested=tool==='boxSelect';state.selectionMode=selectionRequested?!state.selectionMode:false;if(selectionRequested)tool='select';if(tool==='erase'&&state.tool==='erase')tool='select';clearTouchObjectIntent();state.tool=tool;state.activePaletteId=null;state.linkSourceId=null;state.drag=null;updateToolButtons();renderContextToolbar();renderCanvas();}
  function updateToolButtons(){document.querySelectorAll('[data-tool]').forEach(button=>button.classList.toggle('active',button.dataset.tool==='boxSelect'?state.selectionMode:button.dataset.tool===state.tool&&!state.selectionMode));const label=({select:'Выбор',pan:'Камера',erase:'Ластик',testSpawn:'Тест отсюда',link:'Выберите цель'})[state.tool]||state.tool;$('toolStatus').textContent=state.selectionMode?'Выделение группы':label;canvas.style.cursor=state.tool==='pan'?'grab':state.tool==='erase'?'not-allowed':state.tool==='select'?'grab':'crosshair';}
  ...
  function routeLengthAllowed(object,path,level=state.level){
    if(!Array.isArray(path)||path.length<2||Math.hypot(path[1].x-path[0].x,path[1].y-path[0].y)<=liftRouteLimit(object)+.001)return true;
  ...
  function pairedPathPlacement(object,path,level=state.level,{checkStyle=true}={}) {
    ...
    if(object.type==='movingPlatform'&&Math.max(Math.abs(path[1].x-object.x),Math.abs(path[1].y-object.y))<4)return{ok:false,message:'Маршрут должен проходить минимум 4 клетки хотя бы по одной оси.'};
    return{ok:true};
  }
  ...
  function snapSpikeToSupport(object, maxDistance = .76) {
    ...
    const current=object.props?.direction||'up';
    const directions=object.w===1&&object.h===1?[current,...['up','down','left','right'].filter(d=>d!==current)]:[current];
    const candidates=[];
    for(const direction of directions)for(const solid of state.level.objects){
      if(solid.type!=='solid'||solid.id===object.id)continue;
      const horizontal=['up','down'].includes(direction);
      if(horizontal?!(solid.x<object.x+object.w&&solid.x+solid.w>object.x):!(solid.y<object.y+object.h&&solid.y+solid.h>object.y))continue;
      const axis=horizontal?'y':'x',value=direction==='up'?solid.y-object.h:direction==='down'?solid.y+solid.h:direction==='left'?solid.x-object.w:solid.x+solid.w;
      const distance=Math.abs(object[axis]-value);
      if(distance<=maxDistance)candidates.push({axis,value,distance,direction});
    }
    candidates.sort((a,b)=>a.distance-b.distance);
    if(candidates[0]){const best=candidates[0];object[best.axis]=best.value;object.props.direction=best.direction;}
    return object;
  }
  ...
  function constrainMoveTarget(object,requestedX,requestedY,step=1) {
    ...
    if(object.type==='movingPlatform')Object.assign(next,clampLiftRoutePoint(object,pathEnd(object),next));
    if(object.type==='boss'){const pieces=staticPlacementFootprints(object),minDx=Math.max(...pieces.map(piece=>-piece.x)),maxDx=Math.min(...pieces.map(piece=>state.level.size.width-piece.x-piece.w)),minDy=Math.max(...pieces.map(piece=>-piece.y)),maxDy=Math.min(...pieces.map(piece=>state.level.size.height-piece.y-piece.h));next.x=object.x+clamp(next.x-object.x,minDx,maxDx);next.y=object.y+clamp(next.y-object.y,minDy,maxDy);}
  ...
  function ownPairPlacement(candidate){if(!PATH_ENDPOINT_TYPES.has(candidate.type))return{ok:true};const end=pathEnd(candidate);if(!routeLengthAllowed(candidate,[candidate,end]))return{ok:false,message:'Максимальная длина маршрута лифта — 10 клеток.'};if(rectsOverlap(candidate,{...candidate,x:end.x,y:end.y}))return{ok:false,message:'Начальная и конечная позиции парного предмета не должны накладываться.'};if(candidate.type==='movingPlatform'&&Math.max(Math.abs(end.x-candidate.x),Math.abs(end.y-candidate.y))<4)return{ok:false,message:'Маршрут должен проходить минимум 4 клетки хотя бы по одной оси.'};return{ok:true};}
  function canPlaceInLevel(level,candidate,ignoreIds=[],options={}){const ignored=new Set(ignoreIds),candidateFootprints=placementFootprints(candidate);if(candidate.type==='boss'&&rectsOverlap(...bossControls(candidate)))return{ok:false,message:'Две педали босса не должны перекрываться.'};if(candidateFootprints.some(footprint=>!rectInsideLevelShape(level,footprint)))return{ok:false,message:candidate.type==='portal'?'Корпус портала выходит за доступную область уровня.':isPanelLevel(level)?'Предмет выходит за доступную форму уровня.':'Предмет выходит за границы уровня.'};for(const owner of level.objects){if(ignored.has(owner.id))continue;for(const candidateFootprint of candidateFootprints)for(const ownerFootprint of placementFootprints(owner))if(rectsOverlap(candidateFootprint,ownerFootprint)&&!overlapAllowed(candidateFootprint,ownerFootprint))return{ok:false,message:`Здесь уже находится «${movingPlatformLabel(owner)}».`};}return options.checkOwnPair===false?{ok:true}:ownPairPlacement(candidate);}
  ...
  function portalPairPlacement(first,level=state.level,ignoreIds=[]){
    ...
    const used=new Set(level.objects.filter(object=>object.type==='portal').map(object=>object.props?.color).filter(Boolean)),color=PORTAL_COLORS.find(candidate=>!used.has(candidate))||PORTAL_COLORS[new Set(level.objects.filter(object=>object.type==='portal').map(object=>object.props?.pairId)).size%PORTAL_COLORS.length];
    const horizontal=first.props?.orientation==='horizontal'||['up','down'].includes(first.props?.side),gapX=horizontal?2:1,gapY=horizontal?1:2,offsets=[[first.w+gapX,0],[-first.w-gapX,0],[0,first.h+gapY],[0,-first.h-gapY]];
    for(const [dx,dy] of offsets){const second={...deepClone(first),x:first.x+dx,y:first.y+dy},pairClear=!placementFootprints(first).some(firstFootprint=>placementFootprints(second).some(secondFootprint=>rectsOverlap(firstFootprint,secondFootprint)));if(pairClear&&canPlaceInLevel(level,second,ignoreIds).ok)return{ok:true,color,second:{x:second.x,y:second.y}};}
    return{ok:false,reason:'partner',message:'Для пары порталов рядом нет свободного места. Освободите клетки и попробуйте снова.'};
  }
  function placementPreviewVerdict(candidate,ignoreIds=[],level=state.level){if(!candidate)return{ok:false,message:'Нет предмета для проверки.'};if(candidate.authoringPairError)return{ok:false,message:candidate.authoringPairError};const ignored=new Set(ignoreIds);if(UNIQUE_TYPES.has(candidate.type)&&level.objects.some(object=>object.type===candidate.type&&!ignored.has(object.id)))return{ok:false,message:`${movingPlatformLabel(candidate)} уже есть на уровне.`};if(candidate.type==='pickup'&&candidate.props?.abilityGroup==='gravity'&&level.objects.filter(object=>object.type==='pickup'&&object.props?.abilityGroup==='gravity'&&!ignored.has(object.id)).length >= (/^lobby-04-/.test(level.id)?3:1))return{ok:false,message:'Достигнут лимит гравитации: 3 в Лобби 04, 1 в остальных комнатах.'};if(candidate.type==='coin'&&level.objects.filter(object=>object.type==='coin'&&!ignored.has(object.id)).length>=coinLimit(level))return{ok:false,message:'Достигнут лимит монет.'};if(PATH_ENDPOINT_TYPES.has(candidate.type))return pairedPathPlacement(candidate,candidate.props?.path,level);if(candidate.type==='portal'&&!level.objects.some(object=>object.id===candidate.id))return portalPairPlacement(candidate,level,ignoreIds);return canPlaceInLevel(level,candidate,ignoreIds);}
  ...
  function linkTargetAt(point,pointerType='mouse') {
    ...
    const targets=[...state.level.objects].filter(object=>isLinkTarget(object));let best=null;
  ...
  function linkedSocketAt(point,pointerType='mouse'){const source=selectedObject();if(!isControlSource(source))return null;const target=linkTargetAt(point,pointerType),descriptor=target?.linkDescriptor||target?.id;return target&&(source.props?.targets||[]).some(value=>String(value)===String(descriptor))?target:null;}
  ...
  function addPlacedObject(object){constrainNewObjectWidth(object);if(object.authoringPairError){toast(object.authoringPairError,'error');return false;}if(PATH_ENDPOINT_TYPES.has(object.type)){const routePlacement=pairedPathPlacement(object,object.props?.path);if(!routePlacement.ok){toast(`Парный предмет не добавлен: ${routePlacement.message}`,'error');return false;}}if(object.type==='secretExit'&&state.slot?.kind!=='campaign'){toast('Секретный выход доступен только на картах автора.','error');return false;}if(UNIQUE_TYPES.has(object.type)){const existing=state.level.objects.find(candidate=>candidate.type===object.type);if(existing){state.selectedId=existing.id;refreshAll();toast(`${movingPlatformLabel(object)} уже есть — переместите его.`);return false;}}
    if(object.type==='pickup'&&object.props?.abilityGroup==='gravity'&&state.level.objects.filter(candidate=>candidate.type==='pickup'&&candidate.props?.abilityGroup==='gravity').length >= (state.slotKey==='lobby-04'?3:1)){toast('Достигнут лимит гравитации: 3 в Лобби 04, 1 в остальных комнатах.','error');return false;}
    if(object.type==='coin'&&state.level.objects.filter(candidate=>candidate.type==='coin').length>=coinLimit()){toast(`Лимит монет для карты ${state.level.size.width}×${state.level.size.height}: ${coinLimit()}.`,'error');return false;}const placement=canPlace(object);if(!placement.ok){toast(placement.message,'error');return false;}if(object.type==='portal')return addPortalPair(object);mutate(`Добавлен: ${movingPlatformLabel(object)}`,()=>{state.level.objects.push(object);state.selectedId=object.id;});if(object.type==='developerNote')requestAnimationFrame(()=>openDeveloperNote(object));return true;}

  function removeObjectGroup(ids){
    const selected=new Set(ids),removed=new Set(),pairs=new Set();
    for(const object of state.level.objects)if(selected.has(object.id)&&!PROTECTED_TYPES.has(object.type)){removed.add(object.id);if(object.type==='portal')pairs.add(object.props?.pairId);}
    for(const object of state.level.objects)if(object.type==='portal'&&pairs.has(object.props?.pairId))removed.add(object.id);
    if(!removed.size){toast('Вход и выход нельзя удалить — только переместить.','error');return;}
    mutate('Группа предметов удалена',()=>{
      state.level.objects=state.level.objects.filter(object=>!removed.has(object.id));
      for(const object of state.level.objects)if(Array.isArray(object.props?.targets))object.props.targets=object.props.targets.filter(target=>!removed.has(targetDescriptorId(target)));
      state.selectedIds=new Set([...selectedObjectIds()].filter(id=>!removed.has(id)));
      state.selectedId=state.selectedIds.values().next().value||null;
    });
    toast(`Удалено предметов: ${removed.size}. Вход и выход сохранены.`,'ok');
  }

  function removeObject(id){const object=state.level.objects.find(candidate=>candidate.id===id);if(!object)return false;if(PROTECTED_TYPES.has(object.type)){toast(`${movingPlatformLabel(object)} нельзя удалить — только переместить.`,'error');return false;}const removedIds=new Set(object.type==='portal'?state.level.objects.filter(candidate=>candidate.type==='portal'&&candidate.props?.pairId===object.props?.pairId).map(candidate=>candidate.id):[id]);mutate(object.type==='portal'?'Удалена пара порталов':`Удалён: ${movingPlatformLabel(object)}`,()=>{state.level.objects=state.level.objects.filter(candidate=>!removedIds.has(candidate.id));for(const candidate of state.level.objects){if(Array.isArray(candidate.props?.targets))candidate.props.targets=candidate.props.targets.filter(target=>!removedIds.has(targetDescriptorId(target)));}state.selectedIds=new Set([...selectedObjectIds()].filter(candidate=>!removedIds.has(candidate)));if(removedIds.has(state.selectedId))state.selectedId=state.selectedIds.values().next().value||null;});return true;}

  function subtractSolid(solid, cut, reservedIds = []) {
    ...
    const pieces=[];
    if(top>solid.y)pieces.push({...deepClone(solid),id:nextObjectId('solid',[...reservedIds,...pieces.map(item=>item.id)]),x:solid.x,y:solid.y,w:solid.w,h:top-solid.y});
    if(bottom<solid.y+solid.h)pieces.push({...deepClone(solid),id:nextObjectId('solid',[...reservedIds,...pieces.map(item=>item.id)]),x:solid.x,y:bottom,w:solid.w,h:solid.y+solid.h-bottom});
    if(left>solid.x)pieces.push({...deepClone(solid),id:nextObjectId('solid',[...reservedIds,...pieces.map(item=>item.id)]),x:solid.x,y:top,w:left-solid.x,h:bottom-top});
    if(right<solid.x+solid.w)pieces.push({...deepClone(solid),id:nextObjectId('solid',[...reservedIds,...pieces.map(item=>item.id)]),x:right,y:top,w:solid.x+solid.w-right,h:bottom-top});
  ...
  function eraseRegion(rect) {
    const affected=state.level.objects.filter(object=>!PROTECTED_TYPES.has(object.type)&&(object.type==='boss'?[object]:staticPlacementFootprints(object)).some(footprint=>rectsOverlap(footprint,rect)));
  ...
  function rotateSelected(){const object=selectedObject();if(!object||!TYPE_DEFS[object.type]?.rotate){toast('У этого предмета нет поворота.');return;}const next=deepClone(object);const cycle=value=>DIRECTION_CYCLE[(DIRECTION_CYCLE.indexOf(value)+1)%DIRECTION_CYCLE.length];
    ...
    else if(next.type==='portal')Object.assign(next,portalHalfTurn(next));
  ...
  function captureTouchSelectionSnapshot(){if(state.touchSelectionSnapshot)return;state.touchSelectionSnapshot={selectedId:state.selectedId,selectedIds:[...selectedObjectIds()],selectedWire:state.selectedWire?deepClone(state.selectedWire):null,selectedBossControl:state.selectedBossControl?deepClone(state.selectedBossControl):null};}
  function discardTouchSelectionSnapshot(){state.touchSelectionSnapshot=null;}
  function restoreTouchSelectionSnapshot(){const snapshot=state.touchSelectionSnapshot;if(!snapshot)return false;state.touchSelectionSnapshot=null;state.selectedIds=new Set((snapshot.selectedIds||[]).filter(id=>state.level?.objects.some(object=>object.id===id)));state.selectedId=snapshot.selectedId&&state.level?.objects.some(object=>object.id===snapshot.selectedId)?snapshot.selectedId:null;state.selectedBossControl=snapshot.selectedBossControl||null;state.selectedWire=snapshot.selectedWire&&state.level?.objects.some(object=>object.id===snapshot.selectedWire.sourceId)&&state.level?.objects.some(object=>object.id===snapshot.selectedWire.targetId)?snapshot.selectedWire:null;showInteractionHint(selectedObject());refreshInspector();renderContextToolbar();return true;}
  ...
  function finishFieldTapCandidate(event){
    ...
    const tap=candidate.gestureVersion===state.touchGestureVersion&&!state.pinch&&performance.now()-candidate.startedAt<=320&&Math.hypot(event.clientX-candidate.startX,event.clientY-candidate.startY)<=TOUCH_OBJECT_GESTURE_SLOP;
    // Dismissing the sheet must not commit an erasure, route edit, or wire drag.
    if(tap){resetCanvasGestureState({restoreSelection:true});closeMobileCategorySheet();}return tap;
  }
  ...
  function beginTouchObjectIntent(event,point,object){beginTouchCanvasIntent(event,()=>{const current=state.level.objects.find(candidate=>candidate.id===object.id);if(!current)return;const ids=selectedObjectIds();state.drag=(ids.size>1||state.groupSelection)&&ids.has(object.id)?{kind:'moveGroup',pointerId:event.pointerId,pointerType:'touch',objects:state.level.objects.filter(candidate=>ids.has(candidate.id)).map(deepClone),start:point,current:point}:{kind:'move',pointerId:event.pointerId,pointerType:'touch',object:deepClone(current),start:point,current:point,offsetX:point.rawX-current.x,offsetY:point.rawY-current.y};canvas.style.cursor='grabbing';renderCanvas();});}
  function beginPinch(){if(state.pointers.size<2)return;clearTouchObjectIntent();restoreTouchSelectionSnapshot();state.fieldTapCandidate=null;state.touchGestureVersion++;const [a,b]=[...state.pointers.values()].slice(0,2);const distance=Math.hypot(a.x-b.x,a.y-b.y);state.pinch={distance,zoom:state.zoom,midX:(a.x+b.x)/2,midY:(a.y+b.y)/2,scrollLeft:viewport.scrollLeft,scrollTop:viewport.scrollTop};state.drag=null;state.pan=null;state.domResize=null;state.wireDrag=null;state.activeTramInsertion=null;canvas.classList.remove('will-delete');viewport.classList.remove('dragging');renderCanvas();}
  ...
  function beginNativeGesture(event){event.preventDefault();const rect=viewport.getBoundingClientRect(),gesture={zoom:state.zoom,x:(Number(event.clientX)||rect.left+rect.width/2)-rect.left,y:(Number(event.clientY)||rect.top+rect.height/2)-rect.top};resetCanvasGestureState({restoreSelection:true});state.nativeGesture=gesture;}
  ...
  function pointerInDeleteZone(event){
    if(!pointInsideCanvas(event.clientX,event.clientY))return true;
    const visual=window.visualViewport,left=visual?.offsetLeft||0,top=visual?.offsetTop||0,width=visual?.width||innerWidth,height=visual?.height||innerHeight;
    const margin=event.pointerType==='touch'?0:16;
    return event.clientX<left+margin||event.clientX>left+width-margin||event.clientY<top+margin||event.clientY>top+height-margin;
  }
  function pointInsideCanvas(clientX, clientY) {
    const rect=canvas.getBoundingClientRect(),view=viewport.getBoundingClientRect(),visual=window.visualViewport;
    const left=Math.max(rect.left,view.left,visual?.offsetLeft||0),top=Math.max(rect.top,view.top,visual?.offsetTop||0);
    const right=Math.min(rect.right,view.right,(visual?.offsetLeft||0)+(visual?.width||innerWidth)),bottom=Math.min(rect.bottom,view.bottom,(visual?.offsetTop||0)+(visual?.height||innerHeight));
    return clientX>=left&&clientX<=right&&clientY>=top&&clientY<=bottom;
  }

  function activateMobilePaletteDrag(pointerId,paletteId,clientX,clientY,target=null) {
    setTool('select');
  ...
  function beginMobilePaletteDrag(event, paletteId) {
    ...
    if(event.pointerType!=='touch'){
      ...
      activateMobilePaletteDrag(event.pointerId,paletteId,event.clientX,event.clientY,event.currentTarget);
      return;
    }
    event.preventDefault();safelyCapturePointer(event.currentTarget,event.pointerId);const scroller=$('mobileCategoryItems');const gesture={pointerId:event.pointerId,paletteId,target:event.currentTarget,startX:event.clientX,startY:event.clientY,lastX:event.clientX,lastY:event.clientY,scrollLeft:scroller?.scrollLeft||0,scrollTop:scroller?.scrollTop||0,mode:'pending',timer:null};
    gesture.timer=setTimeout(()=>{if(state.mobilePaletteGesture!==gesture||gesture.mode!=='pending')return;gesture.mode='drag';gesture.timer=null;activateMobilePaletteDrag(gesture.pointerId,gesture.paletteId,gesture.lastX,gesture.lastY,gesture.target);},MOBILE_PALETTE_LONG_PRESS_MS);
  ...
  function updateMobilePaletteGesture(event) {
    ...
    const portrait=state.layoutMode==='portrait',primary=Math.abs(portrait?deltaX:deltaY),outward=portrait?-deltaY:deltaX,outwardDrag=outward>MOBILE_PALETTE_OUTWARD_DRAG_SLOP&&outward>primary*MOBILE_PALETTE_OUTWARD_DOMINANCE;
    if(gesture.mode==='pending'&&outwardDrag){if(gesture.timer)clearTimeout(gesture.timer);gesture.timer=null;gesture.mode='drag';event.preventDefault();activateMobilePaletteDrag(gesture.pointerId,gesture.paletteId,event.clientX,event.clientY,gesture.target);}
    else if(gesture.mode==='pending'&&(primary>MOBILE_PALETTE_GESTURE_SLOP||Math.hypot(deltaX,deltaY)>MOBILE_PALETTE_OUTWARD_DRAG_SLOP)){if(gesture.timer)clearTimeout(gesture.timer);gesture.timer=null;gesture.mode='scroll';state.mobileIgnoreClickUntil=Date.now()+450;}
    if(gesture.mode==='scroll'){event.preventDefault();const scroller=$('mobileCategoryItems');if(scroller){scroller.scrollLeft=gesture.scrollLeft-deltaX;scroller.scrollTop=gesture.scrollTop-deltaY;updateMobileCategoryScrollCue();}}
  ...
  function endMobilePaletteGesture(event) {
    ...
    if(mode==='scroll')state.mobileIgnoreClickUntil=Date.now()+450;
  }
  ...
  function handlePointerDown(event,routeHandleOverride=null){
    ...
    const point=pointerGridPoint(event),pedalHit=!routeHandleOverride&&!state.spaceHeld&&state.tool!=='pan'?bossControlAt(point,event.pointerType):null;
    if(pedalHit&&(event.button===2||state.tool==='erase')){toast('Педаль принадлежит боссу: её можно переместить, но нельзя удалить отдельно.');return;}
    if(pedalHit&&state.tool==='select'&&event.button===0){
      const object=pedalHit.object,control=bossControls(object)[pedalHit.index];setSelectedObjectIds([object.id]);state.selectedBossControl={id:object.id,index:pedalHit.index};state.selectedWire=null;
      const activate=()=>{state.drag={kind:'bossControl',pointerId:event.pointerId,pointerType:event.pointerType,object:deepClone(object),controlIndex:pedalHit.index,start:point,current:point,startClientX:event.clientX,startClientY:event.clientY,offsetX:point.rawX-control.x,offsetY:point.rawY-control.y};canvas.style.cursor='grabbing';renderCanvas();};
      // Keep the mobile header height stable until the pointer is released.
      if(touch){refreshInspector();renderContextToolbar();renderCanvas();beginTouchCanvasIntent(event,activate);}else{activate();refreshAll();}return;
    }
    state.selectedBossControl=null;
    const wireHit=!routeHandleOverride&&state.tool==='select'?wireAtPoint(point,event.pointerType):null;
    if(wireHit?.edge==='target'||wireHit?.edge==='source'){
      const startsNewWire=wireHit.edge==='source',title=startsNewWire?'Новый провод':'Провод выбран',message=startsNewWire?'Потяните вилку к новой розетке. Уже подключённые провода останутся на месте.':`Потяните край к другой розетке или отпустите в стороне, чтобы отключить: ${movingPlatformLabel(wireHit.source)} → ${movingPlatformLabel(wireHit.target)}.`;
    ...
    if(wireHit?.edge==='line'){
      state.selectedWire={sourceId:wireHit.source.id,targetId:wireHit.target.id,descriptor:wireHit.descriptor};state.selectedId=wireHit.source.id;showHintText('Провод выбран',`${movingPlatformLabel(wireHit.source)} → ${movingPlatformLabel(wireHit.target)}. Потяните за кружок на конце, чтобы переподключить.`);
    ...
    if(state.fieldTapCandidate&&(state.tool==='testSpawn'||state.tool==='link'))return;
    ...
    if(state.tool==='testSpawn'){const candidate={x:clamp(point.x,0,state.level.size.width-1),y:clamp(point.y,0,state.level.size.height-2),w:1,h:2,type:'spawn'};if(rectInsideLevelShape(state.level,candidate)&&!state.level.objects.some(object=>placementFootprints(object).some(footprint=>rectsOverlap(footprint,candidate)&&!overlapAllowed(footprint,candidate)))){state.testSpawn={x:candidate.x,y:candidate.y};setTool('select');toast('Play начнётся с временной точки; прохождение не будет засчитано.');renderCanvas();}else toast('Точка теста должна быть в свободном месте.','error');return;}
    if(state.tool==='link'){linkSelectedTo(linkTargetAt(point,event.pointerType));return;}
    if(state.tool==='select'){
      const groupHit=pathGhostObjectAt(point)||pathEndpointObjectAt(point,event.pointerType)||objectAt(point,event.pointerType),groupIds=selectedObjectIds();
      if(groupHit&&groupIds.has(groupHit.id)&&(state.groupSelection||groupIds.size>1)){
        const activate=()=>{state.drag={kind:'moveGroup',pointerId:event.pointerId,pointerType:event.pointerType,objects:state.level.objects.filter(o=>groupIds.has(o.id)).map(deepClone),start:point,current:point};renderCanvas();};
      ...
      if(object){
        if(!selectedObjectIds().has(object.id))setSelectedObjectIds([object.id]);else state.selectedId=object.id;state.selectedWire=null;showInteractionHint(object);
        if(event.pointerType==='touch'){beginTouchObjectIntent(event,point,object);refreshInspector();renderContextToolbar();renderCanvas();}
        else{const ids=selectedObjectIds();state.drag=(ids.size>1||state.groupSelection)&&ids.has(object.id)?{kind:'moveGroup',pointerId:event.pointerId,pointerType:event.pointerType,objects:state.level.objects.filter(candidate=>ids.has(candidate.id)).map(deepClone),start:point,current:point}: {kind:'move',pointerId:event.pointerId,pointerType:event.pointerType,object:deepClone(object),start:point,current:point,offsetX:point.rawX-object.x,offsetY:point.rawY-object.y};refreshAll();}
        return;
      }
      state.selectedId=null;state.selectedIds=new Set();state.selectedWire=null;showInteractionHint(null);if(state.selectionMode)state.drag={kind:'selectRect',pointerId:event.pointerId,pointerType:event.pointerType,start:point,current:point};else{state.pan={pointerId:event.pointerId,x:event.clientX,y:event.clientY,scrollLeft:viewport.scrollLeft,scrollTop:viewport.scrollTop};viewport.classList.add('dragging');}refreshAll();return;
    }
  }
  ...
  function handlePointerMove(event){if(!state.ready||!state.level)return;if(state.pointers.has(event.pointerId))state.pointers.set(event.pointerId,{x:event.clientX,y:event.clientY});updateFieldTapCandidate(event);if(state.pinch){updatePinch();return;}const intent=state.touchObjectIntent;if(intent?.pointerId===event.pointerId){intent.lastX=event.clientX;intent.lastY=event.clientY;if(Math.hypot(event.clientX-intent.startX,event.clientY-intent.startY)>TOUCH_OBJECT_GESTURE_SLOP){clearTouchObjectIntent();state.pan={pointerId:event.pointerId,x:intent.startX,y:intent.startY,scrollLeft:intent.scrollLeft,scrollTop:intent.scrollTop};viewport.classList.add('dragging');viewport.scrollLeft=state.pan.scrollLeft-(event.clientX-state.pan.x);viewport.scrollTop=state.pan.scrollTop-(event.clientY-state.pan.y);}return;}if(state.pan&&state.pan.pointerId!==event.pointerId||state.drag&&state.drag.pointerId!==event.pointerId)return;const point=pointerGridPoint(event);state.hoverPoint=point;$('cursorReadout').style.display='none';$('cursorStatus').textContent='';
    ...
    if(state.drag){state.drag.current=point;if(state.drag.kind==='move'||state.drag.kind==='moveGroup'||state.drag.kind==='pathNode'){state.drag.outside=pointerInDeleteZone(event);state.drag.deleteCandidate=state.drag.outside&&(state.drag.kind==='move'||!state.drag.insertedNode);canvas.classList.toggle('will-delete',state.drag.outside);}if(state.drag.kind==='pathEndpoint'||state.drag.kind==='pathNode'||state.drag.kind==='moveGroup'||state.drag.kind==='bossControl')canvas.style.cursor='grabbing';renderCanvas();}
  ...
  function handlePointerUp(event){state.pointers.delete(event.pointerId);if(finishFieldTapCandidate(event))return;if(event.pointerType==='touch')discardTouchSelectionSnapshot();if(state.touchObjectIntent?.pointerId===event.pointerId){clearTouchObjectIntent();if(selectedBossControl())showBossControlHint(state.selectedBossControl.index);return;}if(state.pinch){if(state.pointers.size<2)state.pinch=null;return;}if(state.pan){if(state.pan.pointerId!==event.pointerId)return;state.pan=null;viewport.classList.remove('dragging');return;}const drag=state.drag;if(drag&&drag.pointerId!==event.pointerId)return;state.drag=null;if(!drag)return;
    if(drag.kind==='draw'){const item=PALETTE_BY_ID.get(drag.paletteId);const object=makeObjectFromTool(item,normalizedGridRect(drag.start,drag.current));const placed=addPlacedObject(object);if(placed&&item.type!=='solid')setTool('select');}
      ...
      const current=state.level.objects.find(object=>object.id===drag.object.id);drag.current=pointerGridPoint(event);const candidate=bossControlCandidate(drag);
      const pointerMoved=Math.hypot(event.clientX-drag.startClientX,event.clientY-drag.startClientY)>=3;
      if(!pointerMoved)showBossControlHint(drag.controlIndex);
  ...
  function cancelCanvasPointer(event){const registered=state.pointers.has(event.pointerId);state.pointers.delete(event.pointerId);if(state.fieldTapCandidate?.pointerId===event.pointerId)state.fieldTapCandidate=null;const cancelledIntent=state.touchObjectIntent?.pointerId===event.pointerId,cancelledPinch=registered&&!!state.pinch,cancelledDrag=state.drag?.pointerId===event.pointerId,cancelledPan=state.pan?.pointerId===event.pointerId,cancelledMove=cancelledDrag&&state.drag?.kind==='move'&&state.drag?.deleteCandidate,cancelledGroup=cancelledDrag&&state.drag?.kind==='moveGroup'&&state.drag?.deleteCandidate?state.drag.objects:null,cancelledObject=cancelledMove?state.drag.object:null;if(cancelledIntent)clearTouchObjectIntent();if(cancelledPinch)state.pinch=null;if(cancelledDrag)state.drag=null;if(cancelledPan)state.pan=null;if(cancelledMove&&cancelledObject)removeObject(cancelledObject.id);if(cancelledGroup)removeObjectGroup(cancelledGroup.map(object=>object.id));if(cancelledIntent||cancelledDrag||cancelledPan)restoreTouchSelectionSnapshot();else if(registered&&!state.pointers.size)discardTouchSelectionSnapshot();if(!cancelledIntent&&!cancelledPinch&&!cancelledDrag&&!cancelledPan)return;state.activeTramInsertion=null;state.hoverPoint=null;if(cancelledDrag)canvas.classList.remove('will-delete');if(cancelledPan)viewport.classList.remove('dragging');renderCanvas();}

  function groupMovePreview(drag){
    const sources=drag.objects.filter(object=>state.level.objects.some(current=>current.id===object.id));
    if(!sources.length)return{ok:false,message:'Нет выделенных предметов.',candidates:[],changed:false};
    const footprints=sources.flatMap(placementFootprints),point=drag.current||drag.start;
    let minDx=Math.max(...footprints.map(object=>-object.x)),maxDx=Math.min(...footprints.map(object=>state.level.size.width-object.x-object.w));
    let minDy=Math.max(...footprints.map(object=>-object.y)),maxDy=Math.min(...footprints.map(object=>state.level.size.height-object.y-object.h));
    // A half-cell neighbour must not pull the boss or its pedals off their whole-cell grid.
    if(sources.some(object=>object.type==='boss')){minDx=Math.ceil(minDx);maxDx=Math.floor(maxDx);minDy=Math.ceil(minDy);maxDy=Math.floor(maxDy);}
    if(minDx>maxDx||minDy>maxDy)return{ok:false,message:'Группа не помещается на поле с целочисленным переносом.',candidates:[],changed:false};
    const dx=clamp(snap(point.rawX-drag.start.rawX,1),minDx,maxDx),dy=clamp(snap(point.rawY-drag.start.rawY,1),minDy,maxDy);
    const candidates=sources.map(source=>({...deepClone(source),x:source.x+dx,y:source.y+dy,props:{...deepClone(source.props),...(Array.isArray(source.props?.path)?{path:source.props.path.map(point=>({x:point.x+dx,y:point.y+dy}))}:{}),...(source.type==='boss'?{controls:shiftedBossControls(source,dx,dy)}:{})}}));
    const byId=new Map(candidates.map(object=>[object.id,object])),level={...state.level,objects:state.level.objects.map(object=>byId.get(object.id)||object)};
    for(const candidate of candidates){const verdict=placementPreviewVerdict(candidate,[candidate.id],level);if(!verdict.ok)return{...verdict,candidates,changed:!!(dx||dy)};}
    return{ok:true,candidates,changed:!!(dx||dy)};
  }
  ...
  function renderFriendlyProperties(object){const root=$('friendlyProperties');root.innerHTML='';const defs=PROPERTY_DEFS[object.type]||[];for(const definition of defs){const row=document.createElement('label');row.className='property-row';const label=document.createElement('span');label.textContent=definition.label;let input;if(definition.type==='select'){input=document.createElement('select');for(const [value,text] of definition.options){const option=document.createElement('option');option.value=value;option.textContent=text;input.append(option);}input.value=String(getPropertyValue(object,definition.key)??definition.options[0][0]);}else{input=document.createElement('input');input.type=definition.type==='checkbox'?'checkbox':definition.type==='text'?'text':'number';if(definition.type==='checkbox')input.checked=!!getPropertyValue(object,definition.key);else{input.value=String(getPropertyValue(object,definition.key)??'');if(definition.type==='number'){input.min=definition.min;input.max=definition.max;input.step=definition.step;}if(definition.maxLength)input.maxLength=definition.maxLength;}}
    ...
    if(isControlSource(object)){const links=document.createElement('div');links.className='link-list';for(const descriptor of object.props?.targets||[]){const target=state.level.objects.find(candidate=>candidate.id===targetDescriptorId(descriptor)),action=connectionActionLabel(descriptor);const chip=document.createElement('button');chip.type='button';chip.className='link-chip';chip.textContent=target?`${movingPlatformLabel(target)}${action?` · ${action}`:''}`:'Потерянная связь';chip.title='Нажмите, чтобы удалить связь';chip.addEventListener('click',()=>mutate('Связь удалена',()=>{object.props.targets=object.props.targets.filter(value=>value!==descriptor);}));links.append(chip);}root.append(links);}
  ...
  function refreshInspector(){
    if(!state.level)return;
    const object=selectedObject();$('noSelection').hidden=!!object;$('objectForm').hidden=!object;if(!object)return;
    const def=TYPE_DEFS[object.type],step=['solid','boss'].includes(object.type)?1:GRID_STEP,widthInput=$('objectWInput'),heightInput=$('objectHInput');
    const widthCap=authoringWidthCap(object.type),door=object.type==='door',horizontalDoor=door&&object.props?.orientation==='horizontal',weight=object.type==='pushBlock',label=object.type==='label',resize=def.resize||'none';
    $('objectTypeLabel').textContent=movingPlatformLabel(object);$('objectPositionLabel').textContent=`x ${formatNumber(object.x)} · y ${formatNumber(object.y)}`;
    $('objectXInput').value=formatNumber(object.x);$('objectYInput').value=formatNumber(object.y);widthInput.value=formatNumber(object.w);heightInput.value=formatNumber(object.h);
    for(const input of [$('objectXInput'),$('objectYInput'),widthInput,heightInput])input.step=step;
    widthInput.min=String(weight?2:label||horizontalDoor?3:1);heightInput.min=String(door&&!horizontalDoor?3:1);
    widthInput.max=String(Math.min(state.level.size.width-object.x,weight||label?8:horizontalDoor?6:widthCap||Infinity));
    heightInput.max=String(Math.min(state.level.size.height-object.y,door&&!horizontalDoor?6:Infinity));
    widthInput.title=weight?'Размер: 2, 4 или 8 клеток':door?'Длина двери: 3–6 клеток':widthCap?`Максимальная ширина: ${widthCap} клеток`:'';
    const setLabel=(input,text)=>{const node=[...(input.closest('label')?.childNodes||[])].find(node=>node.nodeType===3);if(node)node.textContent=text;};
    setLabel(widthInput,weight?'Размер (2/4/8)':horizontalDoor?'Длина (3–6)':widthCap?`Ширина (макс. ${widthCap})`:'Ширина');setLabel(heightInput,door&&!horizontalDoor?'Длина (3–6)':'Высота');
    widthInput.disabled=door?!horizontalDoor:!!def.fixedSize||!['x','xy','square','label'].includes(resize)&&!(resize==='axis'&&['up','down'].includes(object.props?.direction||'up'));
    heightInput.disabled=door?horizontalDoor:!!def.fixedSize||weight||label||!['y','xy','square'].includes(resize)&&!(resize==='axis'&&['left','right'].includes(object.props?.direction||''));
    $('rotateObjectButton').disabled=!def.rotate;$('linkObjectButton').hidden=!isControlSource(object);$('deleteObjectButton').disabled=PROTECTED_TYPES.has(object.type)||!!selectedBossControl(object);
    renderFriendlyProperties(object);requestAnimationFrame(()=>drawToolIcon($('selectedObjectIcon'),{type:object.type,preset:object.props,color:objectColor(object)}));
  }
  ...
  function renderContextToolbar(){const root=$('contextToolbar'),handles=$('resizeHandles'),object=selectedObject();if(!root||!handles||!object||state.tool==='link'){if(root)root.hidden=true;if(handles)handles.hidden=true;return;}const def=TYPE_DEFS[object.type],add=(icon,label,action,className='',active=false,id='')=>{const button=contextIconButton(icon,label,action,className,active,id);root.append(button);return button;};root.innerHTML='';root.hidden=false;
    const pedal=selectedBossControl(object);
    if(pedal){const index=state.selectedBossControl.index;
      add('1',`Босс 1 · ${(bossKindDefinition(object.props.bossKind)||BOSS_KINDS.ratSpider).label} · 5×5`,()=>toast('Вид определяет размер корпуса. Паук: 5×5 клеток.'),'context-variant',true,'boss-kind');
      add('−',`Сузить педаль ${index+1}: ${pedal.w} клеток`,()=>resizeBossControl(object,index,-1),'context-variant',false,'boss-control-narrow').disabled=pedal.w<=1;
      add('+',`Расширить педаль ${index+1}: ${pedal.w} клеток`,()=>resizeBossControl(object,index,1),'context-variant',false,'boss-control-widen').disabled=pedal.w>=4;
    ...
    if(object.type==='spike'){const linked=incomingLinks(object.id).length>0,mode=linked?'toggle':object.props?.mode||'always';if(!linked){const modeControl=add(mode==='always'?'∞':'◷',`Режим: ${mode==='always'?'всегда':'по циклу'}`,()=>cycleContextValue(object,'mode',['always','cycle'],'Режим',['всегда','по циклу']),'context-variant context-parent',mode==='cycle','mode');modeControl.style.order='-12';if(mode==='cycle'){const startActive=object.props?.startActive!==false,cycleSeconds=Number(object.props?.cycle)||2,cycleLabel=cycleSeconds>=4?'Медленно':cycleSeconds>=2?'Средне':'Быстро',lamp=add('bulb',startActive?'Шипы показаны в начале цикла':'Шипы скрыты в начале цикла',()=>contextMutation('Начало цикла шипов изменено',()=>{object.props.startActive=!startActive;},startActive?'Шипы будут начинать скрытыми':'Шипы будут начинать показанными'),'context-state context-nested',startActive,'spike-cycle-start'),cycle=add(cycleSeconds>=4?'>':cycleSeconds>=2?'>>':'>>>',`Скорость цикла: ${cycleLabel} · ${formatNumber(cycleSeconds)} сек`,()=>cycleContextNumber(object,'cycle',[4,2,1],'Скорость цикла',' сек'),'context-variant context-nested',false,'cycle');lamp.style.order='-11';cycle.style.order='-10';}}}
  ...
  function positionSelectionUi(object,def){const root=$('contextToolbar'),handles=$('resizeHandles'),stage=$('canvasStage'),cell=cellPixels(),geometry=selectedBossControl(object)||object,left=canvas.offsetLeft+geometry.x*cell,top=canvas.offsetTop+geometry.y*cell,width=geometry.w*cell,height=geometry.h*cell,resizable=!selectedBossControl(object)&&!def.fixedSize&&!!def.resize&&!PROTECTED_TYPES.has(object.type);handles.hidden=!resizable;if(resizable){handles.style.left=`${left}px`;handles.style.top=`${top}px`;handles.style.width=`${width}px`;handles.style.height=`${height}px`;for(const button of handles.querySelectorAll('button'))button.hidden=false;}if(root.hidden)return;const gap=14,endpointLaneHeight=LINK_ENDPOINT_OFFSET+LINK_ENDPOINT_CONTROL_SIZE/2,inset=canvasStageInset(),stageWidth=Math.max(stage.clientWidth,canvas.offsetLeft+canvas.width+inset),stageHeight=Math.max(stage.clientHeight,canvas.offsetTop+canvas.height+inset),rootWidth=root.offsetWidth||46,rootHeight=root.offsetHeight||46,centerX=left+width/2,centerY=top+height/2;let side='above',rootLeft=clamp(centerX-rootWidth/2,4,Math.max(4,stageWidth-rootWidth-4)),rootTop=top-gap-rootHeight;if(rootTop<4){side='below';rootTop=top+height+gap+endpointLaneHeight;}if(rootTop+rootHeight>stageHeight-4){const canRight=left+width+gap+rootWidth<=stageWidth-4;side=canRight?'right':'left';rootLeft=canRight?left+width+gap:Math.max(4,left-gap-rootWidth);rootTop=clamp(centerY-rootHeight/2,4,Math.max(4,stageHeight-rootHeight-4));}if(selectedBossControl(object)){
      const stageRect=stage.getBoundingClientRect(),obstacles=[...$('panelTopologyControls').querySelectorAll('.panel-control')].map(button=>{const rect=button.getBoundingClientRect();return{x:rect.left-stageRect.left,y:rect.top-stageRect.top,w:rect.width,h:rect.height};});
      const xs=[rootLeft,...obstacles.flatMap(rect=>[rect.x-rootWidth-8,rect.x+rect.w+8])],ys=[rootTop,...obstacles.flatMap(rect=>[rect.y-rootHeight-8,rect.y+rect.h+8])];
      const candidates=xs.flatMap(x=>ys.map(y=>({x,y,w:rootWidth,h:rootHeight,distance:Math.hypot(x-rootLeft,y-rootTop)}))).filter(rect=>rect.x>=4&&rect.y>=4&&rect.x+rect.w<=stageWidth-4&&rect.y+rect.h<=stageHeight-4&&!obstacles.some(obstacle=>rectsOverlap(rect,obstacle))).sort((a,b)=>a.distance-b.distance);
      if(candidates[0]){rootLeft=candidates[0].x;rootTop=candidates[0].y;}
  ...
  function beginDomResize(event){
    const object=selectedObject();
    if(!object||TYPE_DEFS[object.type].fixedSize||state.domResize||state.drag||state.pan||state.pinch||state.mobilePaletteDrag||state.wireDrag||state.touchObjectIntent)return;
  ...
  function validateLevel(selectChecks=false){const issues=[];const add=(severity,message,objectId=null)=>issues.push({severity,message,objectId});const level=state.level;const title=level.title.trim();if(!title)add('error','Введите название уровня.');const titleLimit=state.slot?.kind==='campaign'?96:48;if(title.length>titleLimit)add('error',`Название длиннее ${titleLimit} символов.`);if(/(?:https?:\/\/|www\.|@\w+\.)/iu.test(title))add('error','В названии нельзя размещать ссылки или адреса.');if(/(?:хуй|пизд|еба|бля|fuck|shit)/iu.test(title))add('error','Название не прошло локальный фильтр публикации.');
    ...
    const gravityCount=level.objects.filter(o=>o.type==='pickup'&&o.props?.abilityGroup==='gravity').length,gravityLimit=state.slotKey==='lobby-04'?3:1;if(gravityCount>gravityLimit)add('error',`Допустимо предметов гравитации: ${gravityLimit}.`);
    const spawns=level.objects.filter(object=>object.type==='spawn'),exits=level.objects.filter(object=>object.type==='exit');if(spawns.length!==1)add('error',`Нужен ровно один вход; найдено ${spawns.length}.`,spawns[0]?.id);if(exits.length!==1)add('error',`Нужен ровно один выход; найдено ${exits.length}.`,exits[0]?.id);const secretExits=level.objects.filter(object=>object.type==='secretExit');if(secretExits.length>1)add('error','На карте может быть только один секретный выход.',secretExits[1].id);
    const ids=new Set(),objectsById=new Map();for(const object of level.objects){if(ids.has(object.id))add('error','Повторяется внутренний идентификатор предмета.',object.id);ids.add(object.id);objectsById.set(object.id,object);if(!TYPE_DEFS[object.type])add('error',`Игра не знает предмет ${object.type}.`,object.id);if(!rectInsideLevelShape(level,object))add('error',isPanelLevel(level)?'Предмет выходит за доступную форму уровня.':'Предмет выходит за границы.',object.id);else if(object.type==='portal'&&portalShellFootprints(object).some(footprint=>!rectInsideLevelShape(level,footprint)))add('error','Корпус портала выходит за доступную область уровня.',object.id);if([object.x,object.y,object.w,object.h].some(value=>Math.abs(value/GRID_STEP-Math.round(value/GRID_STEP))>1e-8))add('error','Предмет не привязан к сетке.',object.id);if(object.type==='solid'&&[object.x,object.y,object.w,object.h].some(value=>!Number.isInteger(value)))add('error','Монолит должен занимать целые клетки — игра не округляет его скрыто.',object.id);}
    for(const boss of level.objects.filter(object=>object.type==='boss')){const controls=bossControls(boss);if(controls.some(control=>!rectInsideLevelShape(level,control)))add('error','Педаль босса выходит за доступную форму уровня.',boss.id);if(rectsOverlap(...controls))add('error','Две педали босса не должны перекрываться.',boss.id);if(!bossKindDefinition(boss.props?.bossKind)||boss.w!==bossKindDefinition(boss.props.bossKind).size[0]||boss.h!==bossKindDefinition(boss.props.bossKind).size[1])add('error','Размер босса определяется его видом: паук занимает 5×5 клеток.',boss.id);if((boss.props?.targets||[]).some(target=>targetDescriptorId(target)===boss.id))add('error','Босс не может управлять самим собой.',boss.id);}
    for(const portal of level.objects.filter(object=>object.type==='portal')){const validSide=DIRECTION_CYCLE.includes(portal.props?.side),horizontal=['up','down'].includes(portal.props?.side),orientation=horizontal?'horizontal':'vertical',canonical=validSide&&portal.props?.orientation===orientation&&Number(portal.props?.length)===6&&portal.w===(horizontal?6:2)&&portal.h===(horizontal?2:6);if(!canonical)add('error','Габариты портала не соответствуют направлению стрелки. Смените направление ещё раз или переустановите портал.',portal.id);}
    ...
    const budget=calculateBudget(level);if((budget.counts.coin||0)>coinLimit(level))add('error',`Монет больше допустимых ${coinLimit(level)} для карты ${level.size.width}×${level.size.height}.`);
  ...
  function shiftPath(object,dx,dy){if(object.type==='boss'&&Array.isArray(object.props?.controls))object.props.controls=object.props.controls.map(control=>({...control,x:control.x+dx,y:control.y+dy}));if(!Array.isArray(object.props?.path))return;object.props.path=object.props.path.map(point=>({x:point.x+dx,y:point.y+dy}));}
  function objectOutsideLevelBounds(object,width,height){return placementFootprints(object).some(footprint=>footprint.x<0||footprint.y<0||footprint.x+footprint.w>width||footprint.y+footprint.h>height);}
  function objectAffectedBySideCut(object,side,amount,nextSize){const footprints=placementFootprints(object),horizontal=side==='left'||side==='right';return footprints.some(footprint=>{const start=horizontal?footprint.x:footprint.y,end=start+(horizontal?footprint.w:footprint.h);return side==='left'||side==='top'?start<amount:end>nextSize;});}
  ...
  function updateInteractionHintHeight(mode=((window.visualViewport?.width||innerWidth)>(window.visualViewport?.height||innerHeight)?'landscape':'portrait')){
    const shell=$('appShell'),root=$('interactionHint');if(!shell||!root)return;
    if(mode!=='portrait'){shell.style.removeProperty('--editor-portrait-hint-height');return;}
    if(state.hintsHidden)return;
    const strong=root.querySelector('strong'),copy=root.querySelector('span'),style=getComputedStyle(root),numeric=value=>Number.parseFloat(value)||0,contentHeight=Math.ceil((strong?.scrollHeight||0)+(copy?.scrollHeight||0)+numeric(style.paddingTop)+numeric(style.paddingBottom)+numeric(style.borderTopWidth)+numeric(style.borderBottomWidth));
    shell.style.setProperty('--editor-portrait-hint-height',`${clamp(contentHeight,88,164)}px`);
  }
  function setHintsHidden(hidden,{persist=true}={}){state.hintsHidden=!!hidden;document.documentElement.classList.toggle('hints-hidden',state.hintsHidden);const button=$('toggleHintsButton');if(button){button.textContent='Подсказки';button.setAttribute('aria-pressed',String(!state.hintsHidden));}if(persist)try{localStorage.setItem(HINT_VISIBILITY_KEY,state.hintsHidden?'1':'0');}catch(error){}requestAnimationFrame(syncEditorViewportLayout);}
  function showHintText(label,help,attention=false){const root=$('interactionHint');if(!root)return;root.querySelector('strong').textContent=label;root.querySelector('span').textContent=help;updateInteractionHintHeight();requestAnimationFrame(syncEditorViewportLayout);if(attention){root.classList.remove('attention');void root.offsetWidth;root.classList.add('attention');}}
  function showInteractionHint(objectOrItem){const type=objectOrItem?.type,def=TYPE_DEFS[type],label=objectOrItem?.label||(def?movingPlatformLabel(objectOrItem):'Поле уровня');let help=objectOrItem?.help||(type==='movingPlatform'&&(objectOrItem.props||objectOrItem.preset)?.loop!==false?PALETTE_ITEMS.find(item=>item.id==='hoist').help:TYPE_HELP[type])||(state.selectionMode?'Протяните прямоугольник вокруг предметов, затем тяните любой выделенный предмет. Повторное нажатие «Выделить» возвращает движение карты.':'Тяните пустое место, чтобы двигать карту.');if(type==='label'&&String(objectOrItem?.props?.text||'').trim())help=`${help} Текст: «${String(objectOrItem.props.text).trim()}»`;showHintText(label,help);}
  ...
  function libraryMetaText(slot,{campaign=false,dateStyle='short'}={}){
    ...
    return `${campaign?`Слот ${slot.sequence} · ${librarySectionForKey(slot.key)==='lobby'?'':difficultyTitle(state.libraryDifficulty)+' · '}`:''}${levelSizeLabel(level)} · ${verb} ${date}`;
  }
  function makeLibraryCard(slot,{campaign=false}={}){
    ...
    select.addEventListener('click',()=>{state.librarySelectedKey=state.librarySelectedKey===slot.key?null:slot.key;state.librarySelectedSlot=state.librarySelectedKey?slot:null;renderLibrary({anchorKey:slot.key});});
  ...
  async function renderLibrary({anchorKey=null,resetScroll=false}={}){
    ...
    if(source==='lobby')state.libraryDifficulty='easy';$('libraryDifficultySelect').hidden=source==='lobby';$('libraryDifficultySelect').closest('label').hidden=source==='lobby';
    ...
    const scrollAnchor=oldCard?{scrollTop:scroller.scrollTop,top:oldCard.getBoundingClientRect().top-scroller.getBoundingClientRect().top}:null;
    ...
    $('libraryHeading').textContent=showUsers?'Мои уровни':section.title;$('libraryDescription').textContent=showUsers?'Черновики и опубликованные уровни на этом устройстве.':`${section.title}: 24 карты${source==='lobby'?'':' и три сложности'}. Выберите карту для редактирования.`;
    ...
    if(state.libraryNeedsScroll){state.libraryNeedsScroll=false;requestAnimationFrame(()=>{container.querySelector('.current,.selected')?.scrollIntoView({block:'center',behavior:'auto'});updateLibraryScrollCue();});}
    else {
      if(scrollAnchor){const card=[...container.children].find(candidate=>candidate.dataset.key===anchorKey);scroller.scrollTop=scrollAnchor.scrollTop;if(card)scroller.scrollTop+=card.getBoundingClientRect().top-scroller.getBoundingClientRect().top-scrollAnchor.top;}
      else if(resetScroll)scroller.scrollTop=0;
      requestAnimationFrame(updateLibraryScrollCue);
    }
  }
  ...
  function bindObjectNumber(id,property){$(id).addEventListener('change',()=>{
    const object=selectedObject();if(!object)return;if(TYPE_DEFS[object.type].fixedSize&&['w','h'].includes(property)){refreshInspector();return;}
  ...
  function nearestPortalPasteCandidate(source){
    const maxX=Math.max(0,state.level.size.width-source.w),maxY=Math.max(0,state.level.size.height-source.h),origin={x:clamp(source.x,0,maxX),y:clamp(source.y,0,maxY)},positions=[];
    for(let y=0;y<=maxY;y++)for(let x=0;x<=maxX;x++)positions.push({x,y,distance:Math.abs(x-origin.x)+Math.abs(y-origin.y)});
    positions.sort((first,second)=>first.distance-second.distance||Math.abs(first.y-origin.y)-Math.abs(second.y-origin.y)||Math.abs(first.x-origin.x)-Math.abs(second.x-origin.x)||first.y-second.y||first.x-second.x);
    for(const position of positions){const candidate={...deepClone(source),x:position.x,y:position.y};if(portalPairPlacement(candidate).ok)return candidate;}
    return null;
  }
  ...
  function handleKeyboard(event){
    ...
    if(command&&event.key.toLowerCase()==='v'&&!editing&&state.objectClipboard){event.preventDefault();let source=deepClone(state.objectClipboard);const oldX=source.x,oldY=source.y;source.id=nextObjectId(source.type);source.x=clamp(source.x+1,0,state.level.size.width-source.w);source.y=clamp(source.y+1,0,state.level.size.height-source.h);if(PATH_ENDPOINT_TYPES.has(source.type)||source.type==='boss')shiftPath(source,source.x-oldX,source.y-oldY);if(source.type==='portal'){source.props.pairId=nextPortalPairId();source=nearestPortalPasteCandidate(source);}const placement=source&&canPlace(source,[],source.type==='smartPlatform'?{checkOwnPair:false}:{});if(placement?.ok)addPlacedObject(source);else toast('Для вставки нет свободного места.','error');return;}
    if(editing)return;if(event.code==='Space'){state.spaceHeld=true;event.preventDefault();}if(event.key.toLowerCase()==='v')setTool('select');if(event.key.toLowerCase()==='e')setTool('erase');if(event.key.toLowerCase()==='r')rotateSelected();if(event.key.toLowerCase()==='p')playLevel();if(event.key==='Delete'||event.key==='Backspace'){event.preventDefault();if(selectedBossControl())toast('Педаль принадлежит боссу и не удаляется отдельно.');else if(state.selectedId)removeObject(state.selectedId);}if(event.key==='Escape'){resetCanvasGestureState({restoreSelection:true});state.selectedId=null;setTool('select');refreshAll();}if(event.key==='+'||event.key==='=')setZoom(state.zoom+.1);if(event.key==='-')setZoom(state.zoom-.1);
  }
  ...
  function bindUi(){renderPalette();renderMobilePalette();updateGridMarkingButton();
    ...
    $('paletteSearch').addEventListener('input',event=>renderPalette(event.target.value));$('gridMarkingButton').addEventListener('click',cycleGridMarking);
    ...
    window.addEventListener('message',async event=>{
      ...
      state.pageSuspended=false;
      resetCanvasGestureState({restoreSelection:true});
      setPlayInFlight(false);
      await consumePlaytestResult();
      updatePlayAvailability(validateLevel(false));
    ...
    window.addEventListener('blur',()=>{state.spaceHeld=false;resetCanvasGestureState({restoreSelection:true});persistEditorView();saveNow().catch(()=>{});});
    document.addEventListener('visibilitychange',()=>{if(document.hidden){resetCanvasGestureState({restoreSelection:true});persistEditorView();saveNow().catch(()=>{});}});
  ...
  async function initialize() {
    try{state.hintsHidden=localStorage.getItem(HINT_VISIBILITY_KEY)==='1';const storedGrid=localStorage.getItem(GRID_MARKING_KEY);if(['4','5','none'].includes(storedGrid))state.gridMarking=storedGrid;}catch(error){}
    bindUi();
    setHintsHidden(state.hintsHidden,{persist:false});
    setPlayInFlight(false);
    ...
    try {
      ...
      const campaignArchiveCount=(await migrateCampaignLayout())+(state.mirrorLegacyArchivedCount||0);
      ...
      if(campaignKeysToEnsure.size){
        ...
        await Promise.all([...campaignKeysToEnsure].map(candidate=>ensureCampaignSlot(candidate,{preferredDifficulty:candidate===emergencyTarget?.slotKey?emergencyTarget.difficulty:difficulty,fast:candidate!==emergencyTarget?.slotKey})));
      }
      ...
      await loadSlot(key, difficulty, { skipCampaignEnsure: campaignKeysToEnsure.size > 0 });
      ...
      if(campaignArchiveCount)toast(`Прежние карты эпизода сохранены в архиве (${campaignArchiveCount}). Архив можно скачать в библиотеке.`,'ok');
      else if(recovery)toast('Аварийная копия черновика восстановлена.','ok');

(() => {
  ...
  function tramPathIssue(object,path) {
    if(!Array.isArray(path)||path.length<2)return'У трамвая должно быть хотя бы два узла.';
    for(const point of path)if(!Number.isFinite(point?.x)||!Number.isFinite(point?.y)||point.x<0||point.y<0||point.x+object.w>state.level.size.width||point.y+object.h>state.level.size.height)return'Узел трамвая выходит за границы уровня.';
    for(let first=0;first<path.length;first++)for(let second=first+1;second<path.length;second++)if(Math.hypot(path[first].x-path[second].x,path[first].y-path[second].y)<2-.001)return'Между любыми узлами трамвая должно оставаться минимум 2 клетки.';
    return'';
  }

  const tramInsertionCache=new WeakMap();
  function tramInsertionPoints(object,path=object?.props?.path) {
    if(object?.type!=='smartPlatform'||!Array.isArray(path)||path.length<2)return[];
    const occupied=state.level.objects.filter(candidate=>candidate.id!==object.id).map(candidate=>`${candidate.id}:${candidate.x},${candidate.y},${candidate.w},${candidate.h}:${Array.isArray(candidate.props?.path)?candidate.props.path.map(point=>`${point.x},${point.y}`).join(';'):''}`).join('|');
    const cacheable=path===object.props?.path,cacheKey=cacheable?`${state.level.size.width}x${state.level.size.height}:${object.w}x${object.h}:${path.map(point=>`${point.x},${point.y}`).join(';')}:${occupied}`:'',cached=cacheable?tramInsertionCache.get(object):null;
    if(cached?.key===cacheKey)return cached.handles;
    if(tramPathIssue(object,path))return[];
    ...
    for(let index=0;index<path.length;index++){
      ...
      if(length<4-.001)continue;
      const distances=[];for(let distance=3;distance<=length-2+.001;distance+=3)distances.push(distance);if(!distances.length)distances.push(length/2);
      for(const distance of distances){
        const point={x:clamp(snap(a.x+dx*distance/length,1),0,state.level.size.width-object.w),y:clamp(snap(a.y+dy*distance/length,1),0,state.level.size.height-object.h)},key=`${point.x}:${point.y}`;
        const candidate={...object,x:point.x,y:point.y},placement=canPlace(candidate,[object.id],{checkOwnPair:false});
        if(seen.has(key)||!placement.ok||path.some(node=>Math.hypot(node.x-point.x,node.y-point.y)<2-.001)||path.some(node=>rectsOverlap({...object,x:node.x,y:node.y},candidate)))continue;
  ...
  function pathNodeFromDrag(drag) {
    ...
    return{x:clamp(snap(point.rawX-drag.offsetX,1),0,state.level.size.width-object.w),y:clamp(snap(point.rawY-drag.offsetY,1),0,state.level.size.height-object.h)};
  }
  ...
  function drawConnections(cell) {
    ...
    for(const object of state.level.objects){
      ...
      if(object.type==='smartPlatform'&&state.drag?.kind==='pathNode'&&state.drag.object.id===object.id){const next=pathNodeFromDrag(state.drag);points=points.map((node,index)=>index===state.drag.nodeIndex?next:node);}
      ...
      const color=object.type==='smartPlatform'?(tramPathIssue(object,points)?'#ff6974':TYPE_DEFS[object.type].color):(pathEndpointPlacement(object,end).ok?TYPE_DEFS[object.type].color:'#ff6974');
      ...
      if(object.type==='smartPlatform'){
        ctx.setLineDash([]);
        for(let index=0;index<points.length;index++){const point=points[index];ctx.beginPath();ctx.arc((point.x+object.w/2)*cell,(point.y+object.h/2)*cell,6,0,Math.PI*2);ctx.fillStyle=index===0?color:'#08100d';ctx.fill();ctx.lineWidth=2;ctx.strokeStyle=color;ctx.stroke();}
  ...
  function drawTramInsertionHandles(cell) {
    ctx.save();
    for(const object of state.level.objects.filter(candidate=>candidate.type==='smartPlatform')){let path=object.props.path;if(state.drag?.kind==='pathNode'&&state.drag.object.id===object.id){const next=pathNodeFromDrag(state.drag);path=path.map((point,index)=>index===state.drag.nodeIndex?next:point);}else if(state.drag?.kind==='move'&&state.drag.object.id===object.id&&!state.drag.deleteCandidate){const target=movePreviewRectFromDrag(state.drag),dx=target.x-state.drag.object.x,dy=target.y-state.drag.object.y;path=state.drag.object.props.path.map(point=>({x:point.x+dx,y:point.y+dy}));}for(const handle of tramInsertionPoints(object,path)){ctx.beginPath();ctx.arc((handle.point.x+object.w/2)*cell,(handle.point.y+object.h/2)*cell,3.5,0,Math.PI*2);ctx.fillStyle=TYPE_DEFS.smartPlatform.color;ctx.fill();ctx.lineWidth=1.5;ctx.strokeStyle='#08100d';ctx.stroke();}}
    ctx.restore();
  }

  function drawPairedPlacementPreview(object,cell,color) {
    ...
    for(let index=1;index<points.length;index++){const point=points[index];ctx.globalAlpha=.3;drawObjectShape(ctx,{...object,x:point.x,y:point.y},point.x*cell,point.y*cell,object.w*cell,object.h*cell,{preview:true,color});ctx.globalAlpha=1;ctx.beginPath();ctx.arc((point.x+object.w/2)*cell,(point.y+object.h/2)*cell,4,0,Math.PI*2);ctx.fillStyle=color;ctx.fill();}
    ctx.restore();
  }
  ...
  function renderCanvas() {
    ...
    if (preview) {const previewColor=state.drag?.kind==='erase'||preview.authoringPairError?'#ff6974':undefined;drawObjectShape(ctx,preview,preview.x*cell,preview.y*cell,preview.w*cell,preview.h*cell,{preview:true,color:previewColor});if(PATH_ENDPOINT_TYPES.has(preview.type))drawPairedPlacementPreview(preview,cell,previewColor||TYPE_DEFS[preview.type].color);}
  ...
  function pairedPathPlacement(object,path) {
    ...
    const mainPlacement=canPlace(object,[object.id],{checkOwnPair:false});if(!mainPlacement.ok)return mainPlacement;
    if(object.type==='smartPlatform'){
      const issue=tramPathIssue(object,path);if(issue)return{ok:false,message:issue};
    }
    ...
    for(let index=1;index<path.length;index++){
      ...
      const placement=canPlace(candidate,[object.id],{checkOwnPair:false});if(!placement.ok)return placement;
  ...
  function handlePointerDown(event){if(!state.ready||state.domResize||state.mobilePaletteDrag||state.wireDrag||event.target?.closest?.('button,input,select,textarea,label,.context-toolbar,.resize-handles'))return;if(event.pointerType==='touch')event.preventDefault();if(state.mobilePaletteExpanded)closeMobileCategorySheet();state.pointers.set(event.pointerId,{x:event.clientX,y:event.clientY});viewport.setPointerCapture?.(event.pointerId);if(state.pointers.size>=2){if(!state.pinch)beginPinch();return;}viewport.focus({preventScroll:true});if(!pointInsideCanvas(event.clientX,event.clientY)){if(event.button===0||event.button===undefined){state.pan={pointerId:event.pointerId,x:event.clientX,y:event.clientY,scrollLeft:viewport.scrollLeft,scrollTop:viewport.scrollTop};viewport.classList.add('dragging');}return;}const point=pointerGridPoint(event),wireHit=state.tool==='select'?wireAtPoint(point,event.pointerType):null;if(wireHit?.edge==='target'||wireHit?.edge==='source'){state.selectedWire={sourceId:wireHit.source.id,targetId:wireHit.target.id,descriptor:wireHit.descriptor};state.selectedId=wireHit.source.id;beginWireDrag(event,wireHit.source.id,wireHit.descriptor,wireHit.edge);showHintText('Провод выбран',`Потяните край к другой розетке или отпустите в стороне, чтобы отключить: ${TYPE_DEFS[wireHit.source.type].label} → ${TYPE_DEFS[wireHit.target.type].label}.`);return;}if(wireHit?.edge==='line'){state.selectedWire={sourceId:wireHit.source.id,targetId:wireHit.target.id,descriptor:wireHit.descriptor};state.selectedId=wireHit.source.id;showHintText('Провод выбран',`${TYPE_DEFS[wireHit.source.type].label} → ${TYPE_DEFS[wireHit.target.type].label}. Потяните за кружок на конце, чтобы переподключить.`);refreshAll();return;}
    const tramHandle=tramRouteHandleAt(point,event.pointerType);if((event.button===2||state.tool==='erase')&&tramHandle?.nodeIndex>0){toast('Основные узлы трамвая нельзя удалить — их можно только перетянуть.');return;}
  ...
  function handlePointerUp(event){state.pointers.delete(event.pointerId);if(state.pinch){if(state.pointers.size<2)state.pinch=null;return;}if(state.pan){if(state.pan.pointerId!==event.pointerId)return;state.pan=null;viewport.classList.remove('dragging');return;}const drag=state.drag;if(drag&&drag.pointerId!==event.pointerId)return;state.drag=null;if(!drag)return;
    ...
    else if(drag.kind==='move'){if(!pointInsideCanvas(event.clientX,event.clientY)){if(removeObject(drag.object.id))toast('Предмет удалён: он вынесен за границу поля.','ok');}else{const target=movePreviewRectFromDrag(drag),current=state.level.objects.find(object=>object.id===drag.object.id);if(current&&target&&(target.x!==current.x||target.y!==current.y)){const fixedPath=Array.isArray(drag.object.props?.path)?drag.object.props.path.map(point=>({...point})):null,dx=target.x-drag.object.x,dy=target.y-drag.object.y,movedPath=current.type==='smartPlatform'&&fixedPath?.length?fixedPath.map(point=>({x:point.x+dx,y:point.y+dy})):fixedPath?.length>1?[{x:target.x,y:target.y},fixedPath[fixedPath.length-1]]:fixedPath,candidate={...current,...target,props:{...current.props,...(movedPath?{path:movedPath}:{})}},placement=canPlace(candidate,[current.id],{checkOwnPair:false}),routePlacement=PATH_ENDPOINT_TYPES.has(candidate.type)?pairedPathPlacement(candidate,movedPath):{ok:true};if(placement.ok&&routePlacement.ok)mutate('Предмет перемещён',()=>{current.x=target.x;current.y=target.y;if(movedPath)current.props.path=movedPath;});else toast(placement.ok?routePlacement.message:placement.message,'error');}}}
    ...
    else if(drag.kind==='pathNode'){const current=state.level.objects.find(object=>object.id===drag.object.id),next=pathNodeFromDrag(drag),previous=drag.object.props.path[drag.nodeIndex];if(current&&!pointInsideCanvas(event.clientX,event.clientY)){toast('Узел остался на месте: маршрут нельзя вытянуть за поле.');}else if(current&&previous&&(next.x!==previous.x||next.y!==previous.y)){const candidatePath=current.props.path.map((point,index)=>index===drag.nodeIndex?next:point),placement=pairedPathPlacement(current,candidatePath);if(placement.ok)mutate('Узел маршрута перемещён',()=>{current.props.path=candidatePath;});else toast(`Узел маршрута: ${placement.message}`,'error');}}
  ...
  function renderContextToolbar(){const root=$('contextToolbar'),handles=$('resizeHandles'),object=selectedObject();if(!root||!handles||!object||state.tool==='link'){if(root)root.hidden=true;if(handles)handles.hidden=true;return;}const def=TYPE_DEFS[object.type],add=(icon,label,action,className='',active=false,id='')=>{const button=contextIconButton(icon,label,action,className,active,id);root.append(button);return button;};root.innerHTML='';root.hidden=false;
    ...
    if(object.type==='playerCannon'){const manual=!!object.props?.manual,manualControl=add(manual?'↻':'✋',manual?'Включить автоматическое вращение':'Включить ручное управление',()=>contextMutation('Режим пушки изменён',()=>{object.props.manual=!manual;},manual?'Включено автовращение':'Включено ручное управление'),'context-state context-parent',manual,'manual');manualControl.style.order='-12';if(!manual){const interval=Number(object.props?.rotateInterval)||.5,clockwise=object.props?.clockwise!==false,speedControl=add(interval<=.25?'>>>':interval<=.5?'>>':'>',`Скорость вращения: ${formatNumber(interval)} сек`,()=>cycleContextNumber(object,'rotateInterval',[1,.5,.25],'Шаг вращения',' сек'),'context-variant context-nested',false,'cannon-speed'),clockwiseControl=add(clockwise?'↻':'↺',clockwise?'Вращение по часовой стрелке':'Вращение против часовой стрелки',()=>contextMutation('Направление вращения изменено',()=>{object.props.clockwise=!clockwise;},clockwise?'Вращение против часовой':'Вращение по часовой'),'context-variant context-nested',clockwise,'cannon-clockwise');speedControl.style.order='-11';clockwiseControl.style.order='-10';}}
  ...
  function beginDomResize(event){const object=selectedObject();if(!object||state.domResize||state.drag||state.pan||state.pinch||state.mobilePaletteDrag||state.wireDrag)return;event.preventDefault();event.stopPropagation();state.domResize={pointerId:event.pointerId,handle:event.currentTarget.dataset.resizeHandle,object:deepClone(object),preview:deepClone(object)};event.currentTarget.setPointerCapture?.(event.pointerId);}
  ...
  function renderLibrarySelection(slot,showUsers,card){
    ...
    $('librarySelectedKind').textContent=showUsers?'Уровень игрока':`Эпизод ${slot.episode||1}`;
    ...
    $('libraryCopyMapButton').hidden=false;$('libraryPasteMapButton').hidden=false;$('libraryPasteMapButton').disabled=showUsers&&published;$('libraryPasteMapButton').title=showUsers&&published?'Сначала отзовите опубликованный уровень.':'Заменить выбранную карту копией';for(const id of ['libraryExportLevelButton','libraryImportLevelLabel'])$(id).hidden=showUsers;
  ...
  async function copySelectedLibraryMap(){const selected=state.librarySelectedSlot;if(!selected)return;const key=selected.key,difficulty=selected.kind==='campaign'?state.libraryDifficulty:'easy';return queueLibraryWrite(async()=>{if(state.slotKey===key)await saveNow();const slot=await dbGet(key),level=slot?.difficulties?.[difficulty]||slot?.difficulties?.easy;if(!level)return;const payload={kind:'nubu.map-clipboard',version:1,copiedAt:Date.now(),level:deepClone(level)};state.mapClipboard=payload;try{localStorage.setItem(MAP_CLIPBOARD_KEY,JSON.stringify(payload));}catch(error){}toast(`Карта ${level.size.width}×${level.size.height} скопирована.`,'ok');});}
  async function replaceSelectedMapFromClipboard(){const selected=state.librarySelectedSlot;if(!selected)return;if(selected.kind==='user'&&playerSlotStatus(selected).key==='submitted'){toast('Сначала отзовите опубликованный уровень.','error');return;}const key=selected.key,kind=selected.kind,difficulty=kind==='campaign'?state.libraryDifficulty:'easy';await state.libraryWriteQueue;let payload=state.mapClipboard;try{payload=payload||JSON.parse(localStorage.getItem(MAP_CLIPBOARD_KEY)||'null');}catch(error){}if(payload?.kind!=='nubu.map-clipboard'||payload.level?.kind!=='nubu.level'){toast('Сначала скопируйте карту в библиотеке.','error');return;}const sourceTitle=payload.level.title||'без названия',targetTitle=selected.difficulties?.[difficulty]?.title||selected.title,ok=await confirmAction('Заменить выбранную карту?',`${kind==='campaign'?difficultyTitle(difficulty)+' карта':'Карта'} «${targetTitle}» будет заменена копией «${sourceTitle}» ${payload.level.size.width}×${payload.level.size.height}.`);if(!ok)return;return queueLibraryWrite(async()=>{const slot=await dbGet(key);if(!slot)throw new Error('Выбранный уровень больше не существует.');if(slot.kind==='user'&&playerSlotStatus(slot).key==='submitted'){toast('Сначала отзовите опубликованный уровень.','error');return;}const current=slot.difficulties[difficulty],next=normalizeLevel({...deepClone(payload.level),id:current.id,title:current.title,episode:slot.episode,sequence:slot.sequence,metadata:{...(payload.level.metadata||{}),difficulty}},{difficulty,episode:slot.episode,sequence:slot.sequence});slot.difficulties[difficulty]=next;slot.updatedAt=Date.now();if(slot.kind==='user')invalidateSlotVerification(slot,difficulty);await dbPut(slot);scheduleLibraryMirror();syncActiveSlotAfterLibraryWrite(slot,difficulty,'Карта заменена из библиотеки');await renderLibrary();toast('Выбранная карта заменена копией.','ok');});}

(() => {
  ...
  function linkSocketGeometry(object,x,y,w,h) {
    ...
    return{radius,cx:x+w/2,cy:y+h/2,anchorX:x+w/2,anchorY:y+h/2,external:false};
  }
  ...
  function drawObjectShape(context, object, x, y, w, h, options = {}) {
    ...
    if (platformTypes.has(object.type)) {
      ...
      context.fillRect(x,y,w,h);context.strokeRect(x+.5,y+.5,w-1,h-1);context.fillStyle='#0b1411';context.font=`900 ${Math.max(8,Math.min(w,h)*.48)}px sans-serif`;context.textAlign='center';context.textBaseline='middle';context.fillText(object.type==='playerCannon'&&object.props?.manual?'M':directionArrow(object.props?.direction||'right'),x+w/2,y+h/2);
  ...
  function constrainPathEndpoint(object, end) {
    ...
    if (object.type === 'movingPlatform') {
      const candidates=[];
      for(let sx=-4;sx<=4;sx++)for(let sy=-4;sy<=4;sy++){
        if(!sx&&!sy)continue;
        const ax=Math.abs(sx),ay=Math.abs(sy),small=Math.min(ax||Infinity,ay||Infinity),large=Math.max(ax,ay);
        if(ax&&ay&&large/small>4)continue;
        const x=object.x+sx*5,y=object.y+sy*5;
        if(x<0||y<0||x+object.w>state.level.size.width||y+object.h>state.level.size.height)continue;
        candidates.push({x,y,distance:Math.hypot(x-constrained.x,y-constrained.y)});
      }
  ...
  function tramRouteSegmentHit(object, point, pointerType = 'mouse') {
    ...
    for (let index = 0; index < path.length - 1; index++) {
      ...
      const length=Math.sqrt(length2),steps=Math.floor(length/3);
      for(let step=1;step<=steps;step++){const t=Math.min(.92,(step*3)/length);if(t<=.08)continue;const x=a.x+dx*t,y=a.y+dy*t,distance=Math.hypot(px-x,py-y);if(distance<=threshold&&(!best||distance<best.distance))best={index:index+1,point:{x:clamp(snap(x,GRID_STEP),0,state.level.size.width-object.w),y:clamp(snap(y,GRID_STEP),0,state.level.size.height-object.h)},distance};}
    }
    return best;
  }
  ...
  function tramCornerPathFromDrag(drag){const path=drag.object.props?.path||[];if(path.length!==4||drag.nodeIndex<1||drag.nodeIndex>3)return null;const base=path[0],point=drag.current||drag.start,maxX=state.level.size.width-drag.object.w,maxY=state.level.size.height-drag.object.h;let width=path[1].x-base.x,height=path[3].y-base.y;if(drag.nodeIndex===1||drag.nodeIndex===2){const raw=point.rawX-drag.offsetX-base.x;width=Math.sign(raw||width||1)*Math.max(3,Math.round(Math.abs(raw)/3)*3);}if(drag.nodeIndex===2||drag.nodeIndex===3){const raw=point.rawY-drag.offsetY-base.y;height=Math.sign(raw||height||1)*Math.max(3,Math.round(Math.abs(raw)/3)*3);}const fit=(origin,delta,max)=>clamp(origin+delta,0,max)-origin;width=fit(base.x,width,maxX);height=fit(base.y,height,maxY);if(Math.abs(width)<3||Math.abs(height)<3)return path.map(node=>({...node}));return[{...base},{x:base.x+width,y:base.y},{x:base.x+width,y:base.y+height},{x:base.x,y:base.y+height}];}
  ...
  function pointSegmentDistance(px,py,a,b){const dx=b.cx-a.cx,dy=b.cy-a.cy,length2=dx*dx+dy*dy;if(length2<.001)return Math.hypot(px-a.cx,py-a.cy);const t=clamp(((px-a.cx)*dx+(py-a.cy)*dy)/length2,0,1);return Math.hypot(px-(a.cx+dx*t),py-(a.cy+dy*t));}
  function wireAtPoint(point,pointerType='mouse'){const px=point.rawX*cellPixels(),py=point.rawY*cellPixels(),endpointRadius=pointerType==='touch'?25:15,lineRadius=pointerType==='touch'?16:8;let line=null;for(const entry of connectionEntries().reverse()){if(Math.hypot(px-entry.targetPoint.cx,py-entry.targetPoint.cy)<=endpointRadius)return{...entry,edge:'target'};if(Math.hypot(px-entry.sourcePoint.cx,py-entry.sourcePoint.cy)<=endpointRadius)return{...entry,edge:'source'};const distance=pointSegmentDistance(px,py,entry.sourcePoint,entry.targetPoint);if(distance<=lineRadius&&(!line||distance<line.distance))line={...entry,edge:'line',distance};}return line;}
  function drawWireArrowlets(a,b,color){const dx=b.cx-a.cx,dy=b.cy-a.cy,length=Math.hypot(dx,dy);if(length<28)return;const ux=dx/length,uy=dy/length,px=-uy,py=ux,size=4,count=Math.max(1,Math.min(4,Math.floor(length/70)));for(let index=1;index<=count;index++){const t=index/(count+1),cx=a.cx+dx*t,cy=a.cy+dy*t;ctx.beginPath();ctx.moveTo(cx+ux*size,cy+uy*size);ctx.lineTo(cx-ux*size+px*size*.65,cy-uy*size+py*size*.65);ctx.lineTo(cx-ux*size-px*size*.65,cy-uy*size-py*size*.65);ctx.closePath();ctx.fillStyle=color;ctx.fill();}}

  function drawConnections(cell) {
    ctx.save();ctx.lineWidth=1.5;
    for(const object of state.level.objects){
      if(!PATH_ENDPOINT_TYPES.has(object.type))continue;
      const end=pathEndForRender(object),placement=pathEndpointPlacement(object,end),color=placement.ok?TYPE_DEFS[object.type].color:'#ff6974';
      let points=object.type==='smartPlatform'?(object.props?.path||routeForObject(object,end)):[{x:object.x,y:object.y},end];
      if(object.type==='smartPlatform'&&state.drag?.kind==='pathNode'&&state.drag.object.id===object.id){const rectangle=tramCornerPathFromDrag(state.drag);points=rectangle||points.map((node,index)=>index===state.drag.nodeIndex?pathNodeFromDrag(state.drag):node);}
      ...
      if(object.type==='smartPlatform'){
        ctx.setLineDash([]);
        for(let index=0;index<points.length;index++){const point=points[index],radius=index<4?6:3.5;ctx.beginPath();ctx.arc((point.x+object.w/2)*cell,(point.y+object.h/2)*cell,radius,0,Math.PI*2);ctx.fillStyle=index===0?color:'#08100d';ctx.fill();ctx.lineWidth=2;ctx.strokeStyle=color;ctx.stroke();}
        ...
        for(let index=0;index<ordered.length;index++)drawWireArrowlets(ordered[index],ordered[(index+1)%ordered.length],color);
        const spacing=Math.max(3,Number(object.props?.spacingCells)||10);for(let index=0;index<points.length;index++){const a=points[index],b=points[(index+1)%points.length],dx=b.x-a.x,dy=b.y-a.y,length=Math.hypot(dx,dy),count=Math.floor(length/spacing);for(let step=1;step<=count;step++){const t=step/(count+1),gx=a.x+dx*t,gy=a.y+dy*t;ctx.globalAlpha=.18;drawObjectShape(ctx,{...object,x:gx,y:gy},gx*cell,gy*cell,object.w*cell,object.h*cell,{preview:true,color});ctx.globalAlpha=1;}}
    ...
    for(const entry of connectionEntries(cell)){const selected=state.selectedWire?.sourceId===entry.source.id&&state.selectedWire?.descriptor===entry.descriptor;ctx.strokeStyle=entry.color;ctx.lineWidth=selected?5:3;ctx.setLineDash([7,5]);ctx.beginPath();ctx.moveTo(entry.sourcePoint.cx,entry.sourcePoint.cy);ctx.lineTo(entry.targetPoint.cx,entry.targetPoint.cy);ctx.stroke();ctx.setLineDash([]);drawWireArrowlets(entry.sourcePoint,entry.targetPoint,entry.color);for(const [point,kind] of [[entry.sourcePoint,'plug'],[entry.targetPoint,'socket']]){ctx.beginPath();ctx.arc(point.cx,point.cy,selected?8:6,0,Math.PI*2);ctx.fillStyle='#07100e';ctx.fill();ctx.lineWidth=selected?3:2;ctx.strokeStyle=entry.color;ctx.stroke();if(kind==='plug'){ctx.beginPath();ctx.moveTo(point.cx-2,point.cy-4);ctx.lineTo(point.cx-2,point.cy-8);ctx.moveTo(point.cx+2,point.cy-4);ctx.lineTo(point.cx+2,point.cy-8);ctx.stroke();}else{ctx.beginPath();ctx.arc(point.cx,point.cy,2,0,Math.PI*2);ctx.fillStyle=entry.color;ctx.fill();}}}
  ...
  function renderMobilePalette() {
    ...
    for (const id of category.items) {
      ...
      variant.addEventListener('dblclick',event=>{if(event.sourceCapabilities?.firesTouchEvents===true)return;setPaletteTool(id);closeMobileCategorySheet();});
  ...
  function linkSelectedTo(target,replaceDescriptor=null){const source=state.level.objects.find(object=>object.id===state.linkSourceId);if(!source){setTool('select');return;}if(!target||!LINKABLE_TYPES.has(target.type)||target.id===source.id){toast('Выберите мигающий сокет двери, платформы, шипа, пресса или генератора.','error');return;}const descriptor=target.linkDescriptor||target.id,targets=Array.isArray(source.props?.targets)?source.props.targets:[];if(targets.some(value=>String(value)===String(descriptor)&&String(value)!==String(replaceDescriptor))){toast('Эта пара уже соединена с выбранной розеткой — второй провод наложился бы сверху.');setTool('select');return;}mutate(replaceDescriptor?'Провод переподключён':'Создана связь',()=>{source.props.targets=targets.filter(value=>String(value)!==String(replaceDescriptor));source.props.targets.push(descriptor);});state.selectedId=source.id;state.selectedWire={sourceId:source.id,targetId:target.id,descriptor:String(descriptor)};setTool('select');toast(`${replaceDescriptor?'Провод переподключён':'Связь создана'}: ${target.linkSocketLabel||'управление'}.`,'ok');}

  function beginWireDrag(event,sourceId,existingDescriptor=null,existingEdge=null){if(event.button!==undefined&&event.button!==0)return;event.preventDefault();event.stopPropagation();const source=state.level.objects.find(object=>object.id===sourceId&&object.type==='button');if(!source)return;state.wireDrag={pointerId:event.pointerId,sourceId,existingDescriptor,existingEdge,startX:event.clientX,startY:event.clientY,clientX:event.clientX,clientY:event.clientY};event.currentTarget?.setPointerCapture?.(event.pointerId);renderCanvas();}
  ...
  function handlePointerDown(event){if(!state.ready)return;if(event.pointerType==='touch')event.preventDefault();if(state.mobilePaletteExpanded)closeMobileCategorySheet();state.pointers.set(event.pointerId,{x:event.clientX,y:event.clientY});canvas.setPointerCapture?.(event.pointerId);if(state.pointers.size===2){beginPinch();return;}viewport.focus({preventScroll:true});const point=pointerGridPoint(event),wireHit=state.tool==='select'?wireAtPoint(point,event.pointerType):null;if(wireHit?.edge==='target'||wireHit?.edge==='source'){state.selectedWire={sourceId:wireHit.source.id,targetId:wireHit.target.id,descriptor:wireHit.descriptor};state.selectedId=wireHit.source.id;beginWireDrag(event,wireHit.source.id,wireHit.descriptor,wireHit.edge);showHintText('Провод выбран',`Потяните край к другой розетке или отпустите в стороне, чтобы отключить: ${TYPE_DEFS[wireHit.source.type].label} → ${TYPE_DEFS[wireHit.target.type].label}.`);return;}if(wireHit?.edge==='line'){state.selectedWire={sourceId:wireHit.source.id,targetId:wireHit.target.id,descriptor:wireHit.descriptor};state.selectedId=wireHit.source.id;showHintText('Провод выбран',`${TYPE_DEFS[wireHit.source.type].label} → ${TYPE_DEFS[wireHit.target.type].label}. Потяните за кружок на конце, чтобы переподключить.`);refreshAll();return;}
    const selectedTram=selectedObject()?.type==='smartPlatform'?selectedObject():null,routeNodeIndex=selectedTram?tramRouteNodeHit(selectedTram,point,event.pointerType):-1;if((event.button===2||state.tool==='erase')&&routeNodeIndex>0){toast('Основные узлы трамвая нельзя удалить — их можно только перетянуть.');return;}
  ...
  function handlePointerMove(event){if(state.pointers.has(event.pointerId))state.pointers.set(event.pointerId,{x:event.clientX,y:event.clientY});if(state.pinch){updatePinch();return;}const point=pointerGridPoint(event);state.hoverPoint=point;$('cursorReadout').style.display='none';$('cursorStatus').textContent='';
    ...
    else {const hovered=objectAt(point),wire=state.tool==='select'?wireAtPoint(point,event.pointerType):null,tram=selectedObject()?.type==='smartPlatform'&&(tramRouteNodeHit(selectedObject(),point,event.pointerType)>=0||tramRouteSegmentHit(selectedObject(),point,event.pointerType));if(state.tool==='select'){canvas.style.cursor=wire||tram||pathGhostObjectAt(point)||pathEndpointHit(selectedObject(),point,event.pointerType)?'grab':hovered?'pointer':'grab';showInteractionHint(hovered);}if(state.tool==='place')renderCanvas();}
  }
  ...
  function handlePointerUp(event){state.pointers.delete(event.pointerId);if(state.pinch){if(state.pointers.size<2)state.pinch=null;return;}if(state.pan){state.pan=null;viewport.classList.remove('dragging');return;}const drag=state.drag;state.drag=null;if(!drag)return;
    ...
    else if(drag.kind==='move'){if(!pointInsideCanvas(event.clientX,event.clientY)){if(removeObject(drag.object.id))toast('Предмет удалён: он вынесен за границу поля.','ok');}else{const target=movePreviewRectFromDrag(drag);const current=state.level.objects.find(object=>object.id===drag.object.id);if(current&&target&&(target.x!==current.x||target.y!==current.y)){const placement=canPlace({...current,...target},[current.id]);if(placement.ok)mutate('Предмет перемещён',()=>{const fixedPath=Array.isArray(drag.object.props?.path)?drag.object.props.path.map(point=>({...point})):null;current.x=target.x;current.y=target.y;if(PATH_ENDPOINT_TYPES.has(current.type)&&fixedPath?.length>1){fixedPath[0]={x:current.x,y:current.y};current.props.path=current.type==='smartPlatform'?fixedPath:[fixedPath[0],fixedPath[fixedPath.length-1]];}});else toast(placement.message,'error');}}}
    ...
    else if(drag.kind==='pathNode'){const current=state.level.objects.find(object=>object.id===drag.object.id),cornerPath=tramCornerPathFromDrag(drag),next=pathNodeFromDrag(drag),previous=drag.object.props.path[drag.nodeIndex];if(current&&!pointInsideCanvas(event.clientX,event.clientY)){toast('Узел остался на месте: маршрут нельзя вытянуть за поле.');}else if(current&&cornerPath){mutate('Прямоугольник трамвая изменён',()=>{current.props.path=cornerPath;});}else if(current&&previous&&(next.x!==previous.x||next.y!==previous.y)){const placement=pathEndpointPlacement(current,next);if(placement.ok)mutate('Узел маршрута перемещён',()=>{current.props.path[drag.nodeIndex]=next;});else toast(`Узел маршрута: ${placement.message}`,'error');}}
  ...
  function renderContextToolbar(){const root=$('contextToolbar'),handles=$('resizeHandles'),object=selectedObject();if(!root||!handles||!object||state.tool==='link'){if(root)root.hidden=true;if(handles)handles.hidden=true;return;}const def=TYPE_DEFS[object.type],add=(icon,label,action,className='',active=false,id='')=>{const button=contextIconButton(icon,label,action,className,active,id);root.append(button);return button;};root.innerHTML='';root.hidden=false;
    ...
    if(object.type==='enemyFlyer'){const value=Number(object.props?.distance)||5,label=value<=2?'Близко':value>=10?'Далеко':'Средне',arrow=directionArrow(object.props?.direction||'right'),icon=value<=2?arrow:value>=10?`${arrow}${arrow}${arrow}`:`${arrow}${arrow}`;add(icon,`Направление и расстояние: ${label}`,()=>cycleContextNumber(object,'distance',[2,5,10],'Расстояние',''),'context-variant context-distance',false,'distance');}
  ...
  function openLibrary(){if(state.slot){$('librarySourceSelect').value=state.slot.kind==='campaign'?`campaign-${state.slot.episode||1}`:'user';state.librarySelectedKey=state.slotKey;state.libraryDifficulty=state.difficulty;state.libraryUserFilter=state.slot.kind==='user'?playerSlotStatus(state.slot).key:'draft';}state.libraryNeedsScroll=true;$('libraryDifficultySelect').value=state.libraryDifficulty;showHintText('Библиотека','Уровни идут одной колонкой. Нажмите карточку — действия раскроются прямо под ней.');renderLibrary();$('libraryModal').classList.add('open');$('libraryModal').setAttribute('aria-hidden','false');}
  ...
  function makeLibraryCard(slot,{campaign=false}={}){
    ...
    title.textContent=campaign?`${slot.episode}-${slot.sequence} · ${displayTitle}`:`№${slot.sequence} · ${displayTitle}`;
    ...
    statusChip.className='level-status';statusChip.dataset.status=status.key;statusChip.textContent=slot.key===state.slotKey?'Текущий уровень':status.label;
  ...
  function renderLibrarySelection(slot,showUsers,card){
    ...
    state.librarySelectedSlot=slot;panel.hidden=false;card.append(panel);
    ...
    $('librarySelectedKind').textContent=slot.key===state.slotKey?'Текущий уровень':status.label;
    $('librarySelectedTitle').textContent=showUsers?`№${slot.sequence}`:`${slot.episode}-${slot.sequence} · ${displayTitle}`;
    $('librarySelectedMeta').textContent=`${showUsers?'Одна карта':difficultyTitle(state.libraryDifficulty)} · ${level.size.width}×${level.size.height} · изменён ${new Date(slot.updatedAt||slot.createdAt||Date.now()).toLocaleString('ru-RU',{dateStyle:'medium',timeStyle:'short'})}`;
    $('libraryLevelTitleInput').value=displayTitle||'';$('librarySelectedTitleField').hidden=!showUsers;
    $('examLevelButton').hidden=!showUsers||published;$('withdrawLevelButton').hidden=!showUsers||!published;$('libraryDeleteButton').hidden=!showUsers;
    for(const id of ['libraryCopyMapButton','libraryPasteMapButton','libraryExportLevelButton','libraryImportLevelLabel'])$(id).hidden=showUsers;
  ...
  async function withdrawSelectedLevel(){const slot=state.librarySelectedSlot;if(slot?.kind!=='user'||playerSlotStatus(slot).key!=='submitted')return;slot.publicationStatus='draft';slot.updatedAt=Date.now();await dbPut(slot);if(state.slotKey===slot.key)state.slot=slot;state.libraryUserFilter='draft';state.librarySelectedKey=slot.key;await renderLibrary();refreshPlayerHeader();toast('Уровень отозван и снова находится в работе.','ok');}

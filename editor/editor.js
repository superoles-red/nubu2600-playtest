(() => {
  ...
  const PALETTE_ITEMS = [
    ...
    { id:'movingPlatform', type:'movingPlatform', label:'Лифт', preset:{loop:false} },
    { id:'hoist', type:'movingPlatform', label:'Подъёмник', preset:{loop:true}, help:'Одна платформа ездит туда и обратно. Максимальная длина маршрута — 10 клеток.' },
  ...
  function movingPlatformLabel(object){return object.type==='movingPlatform'&&(object.props||object.preset)?.loop!==false?'Подъёмник':TYPE_DEFS[object.type]?.label||object.type;}
  ...
  function pathEndpointPlacement(object, end) {
    if(!routeLengthAllowed(object,[object,end]))return{ok:false,message:'Максимальная длина маршрута подъёмника — 10 клеток.'};
  ...
  function pairedPathPlacement(object,path,level=state.level,{checkStyle=true}={}) {
    if(!routeLengthAllowed(object,path,level))return{ok:false,message:'Максимальная длина маршрута подъёмника — 10 клеток.'};
  ...
  function refreshInspector(){
    ...
    renderFriendlyProperties(object);requestAnimationFrame(()=>drawToolIcon($('selectedObjectIcon'),{type:object.type,preset:object.props,color:def.color}));
  }

(() => {
  ...
  function drawObjectShape(context, object, x, y, w, h, options = {}) {
    ...
    if(!mini&&LINKABLE_TYPES.has(object.type)&&(selected||state.wireDrag||(selectedObject()?.type==='button'&&buttonHasTarget(selectedObject(),object.id)))){const linked=incomingLinks(object.id).length>0,socket=linkSocketGeometry(object,x,y,w,h),{radius,cx,cy}=socket;context.setLineDash([]);if(socket.external){context.beginPath();context.moveTo(socket.anchorX,socket.anchorY);context.lineTo(cx-(cx-socket.anchorX)*.32,cy-(cy-socket.anchorY)*.32);context.strokeStyle='#65ff9a';context.lineWidth=3;context.stroke();}context.beginPath();context.arc(cx,cy,radius,0,Math.PI*2);context.fillStyle=linked?'#65ff9a':'#07140d';context.strokeStyle='#65ff9a';context.lineWidth=3;context.fill();context.stroke();context.fillStyle=linked?'#07140d':'#65ff9a';context.beginPath();context.arc(cx-radius*.34,cy,radius*.2,0,Math.PI*2);context.arc(cx+radius*.34,cy,radius*.2,0,Math.PI*2);context.fill();}

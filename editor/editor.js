(() => {
  ...
  const MOBILE_PALETTE_CATEGORIES = [
    { id:'base', label:'Основа', icon:'▦', items:['button-toggle','solid','coin','label'] },
    { id:'platforms', label:'Платформы', icon:'═', items:['oneWay','fragilePlatform','blinkPlatform','movingPlatform','smartPlatform','fallingPlatform','conveyor','bouncePad'] },
    { id:'hazards', label:'Опасности', icon:'▲', items:['spike','pushBlock','enemyGoomba','enemyFlyer','enemyLeech'] },
    { id:'mechanisms', label:'Механизмы', icon:'⚙', items:['button-toggle','driftField','crusherWall','door','portal','playerCannon','generator'] },
    { id:'other', label:'Особые', icon:'⋯', items:['collectible','pickup-gravity','unlockSwitch','pickup-abilities','heartVendor','developerNote'] },
  ];
  ...
  function defaultProps(type) {
    switch (type) {
      ...
      case 'smartPlatform': return { path: [], speedCellsPerSecond: 2.4, loop: true, persistent: true, spacingCells: 10 };
  ...
  function transactionDone(transaction) { return withTimeout(new Promise((resolve, reject) => { transaction.oncomplete = resolve; transaction.onerror = () => reject(transaction.error); transaction.onabort = () => reject(transaction.error || new Error('IndexedDB transaction aborted')); })); }
  ...
  async function writeLibraryMirrorNow(){if(!state.db||!navigator.storage?.getDirectory)return false;try{return await withTimeout((async()=>{const slots=await dbGetAll();const payload={kind:'nubu.library-backup',version:1,exportedAt:Date.now(),slots};const root=await navigator.storage.getDirectory();const handle=await root.getFileHandle(LIBRARY_MIRROR_FILE,{create:true});const writer=await handle.createWritable();await writer.write(`${JSON.stringify(payload)}\n`);await writer.close();return true;})(),2500,'Резервное зеркало библиотеки не ответило');}catch(error){return false;}}
  ...
  function linkSocketGeometry(object,x,y,w,h) {
    const radius=clamp(Math.min(w,h)*.13,5,7);
    return{radius,cx:x+w/2,cy:y+h+radius+4,anchorX:x+w/2,anchorY:y+h,external:true};
  }

  function linkSocketEntries(object,x,y,w,h){const base=linkSocketGeometry(object,x,y,w,h),definitions=object.type==='movingPlatform'?[['','Движение'],['@visibility','Прозрачность']]:object.type==='crusherWall'?[['','Движение'],['@spikes','Шипы']]:object.type==='conveyor'?[['','Движение'],['@reverse','Реверс']]:[['','Управление']],gap=base.radius*2+5,total=(definitions.length-1)*gap;return definitions.map(([suffix,label],index)=>({suffix,label,...base,cx:base.cx-total/2+index*gap}));}
  ...
  function tramRouteSegmentHit(object, point, pointerType = 'mouse') {
    ...
    for (let index = 0; index < path.length - 1; index++) {
      const a=path[index],b=path[index+1],dx=b.x-a.x,dy=b.y-a.y,length2=dx*dx+dy*dy;
      if (length2 < .001) continue;
      const t=clamp(((px-a.x)*dx+(py-a.y)*dy)/length2,.08,.92),x=a.x+dx*t,y=a.y+dy*t,distance=Math.hypot(px-x,py-y);
      if (distance<=threshold&&(!best||distance<best.distance)) best={index:index+1,point:{x:clamp(snap(x,GRID_STEP),0,state.level.size.width-object.w),y:clamp(snap(y,GRID_STEP),0,state.level.size.height-object.h)},distance};
    }
    return best;
  }
  ...
  function pathNodeFromDrag(drag) {
    ...
    const desired={x:clamp(snap(point.rawX-drag.offsetX,GRID_STEP),0,state.level.size.width-object.w),y:clamp(snap(point.rawY-drag.offsetY,GRID_STEP),0,state.level.size.height-object.h)},path=object.props?.path||[],previous=path[Math.max(0,drag.nodeIndex-1)]||{x:object.x,y:object.y},next=path[(drag.nodeIndex+1)%path.length];return snapTramNodeBetween(previous,next,desired,object);
  }

  function validTramSegment(a,b){const dx=Math.abs(b.x-a.x),dy=Math.abs(b.y-a.y);return dx<.001||dy<.001||Math.abs(dx-dy)<.001;}
  function snap45From(anchor,desired){const dx=desired.x-anchor.x,dy=desired.y-anchor.y;if(Math.abs(dx)<.5)return{x:anchor.x,y:desired.y};if(Math.abs(dy)<.5)return{x:desired.x,y:anchor.y};const distance=Math.max(1,Math.round((Math.abs(dx)+Math.abs(dy))/2));return{x:anchor.x+Math.sign(dx)*distance,y:anchor.y+Math.sign(dy)*distance};}
  function snapTramNodeBetween(previous,next,desired,object){const candidates=[snap45From(previous,desired),{x:previous.x,y:desired.y},{x:desired.x,y:previous.y}];if(next)candidates.push(snap45From(next,desired),{x:next.x,y:desired.y},{x:desired.x,y:next.y});const bounded=candidates.map(point=>({x:clamp(snap(point.x),0,state.level.size.width-object.w),y:clamp(snap(point.y),0,state.level.size.height-object.h)})).filter(point=>validTramSegment(previous,point)&&(!next||validTramSegment(point,next)));return(bounded.sort((a,b)=>Math.hypot(a.x-desired.x,a.y-desired.y)-Math.hypot(b.x-desired.x,b.y-desired.y))[0]||snap45From(previous,desired));}
  ...
  function drawConnections(cell) {
    ...
    for (const object of state.level.objects) {
      if (object.type === 'button') {
        ...
        for (const descriptor of targets) { const target = state.level.objects.find(candidate => candidate.id === targetDescriptorId(descriptor)); if (!target) continue; const sourceSocket=linkSocketGeometry(object,object.x*cell,object.y*cell,object.w*cell,object.h*cell),targetSocket=linkSocketEntries(target,target.x*cell,target.y*cell,target.w*cell,target.h*cell).find(entry=>String(descriptor).endsWith(entry.suffix)&&entry.suffix)||linkSocketEntries(target,target.x*cell,target.y*cell,target.w*cell,target.h*cell)[0];ctx.strokeStyle = String(descriptor).endsWith('@reverse')?'#67d7ff':String(descriptor).endsWith('@spikes')?'#ff8d99':String(descriptor).endsWith('@visibility')?'#d5a8ff':'#65ff9a'; ctx.lineWidth=3; ctx.setLineDash([7,5]); ctx.beginPath(); ctx.moveTo(sourceSocket.cx,sourceSocket.cy); ctx.lineTo(targetSocket.cx,targetSocket.cy); ctx.stroke(); }
      }
      if (PATH_ENDPOINT_TYPES.has(object.type)) {
        const end = pathEndForRender(object); const placement = pathEndpointPlacement(object,end); const color = placement.ok ? TYPE_DEFS[object.type].color : '#ff6974';let points=object.type==='smartPlatform'?(state.drag?.kind==='pathEndpoint'&&state.drag.object.id===object.id?routeForObject(object,end):(object.props?.path||routeForObject(object,end))):[{x:object.x,y:object.y},end];if(object.type==='smartPlatform'&&state.drag?.kind==='pathNode'&&state.drag.object.id===object.id){points=points.map((node,index)=>index===state.drag.nodeIndex?pathNodeFromDrag(state.drag):node);}ctx.strokeStyle = color; ctx.setLineDash([5,4]); ctx.beginPath();points.forEach((point,index)=>{const x=(point.x+object.w/2)*cell,y=(point.y+object.h/2)*cell;if(index)ctx.lineTo(x,y);else ctx.moveTo(x,y);});if(object.type==='smartPlatform'&&points.length>2)ctx.closePath();ctx.stroke();if(object.type==='smartPlatform'){ctx.setLineDash([]);for(const point of points.slice(1)){ctx.beginPath();ctx.arc((point.x+object.w/2)*cell,(point.y+object.h/2)*cell,5,0,Math.PI*2);ctx.fillStyle='#08100d';ctx.fill();ctx.lineWidth=2;ctx.strokeStyle=color;ctx.stroke();}const centers=points.map(point=>({x:point.x+object.w/2,y:point.y+object.h/2}));for(let index=0;index<centers.length;index++){const next=index===centers.length-1?centers[0]:centers[index+1];if(index<centers.length-1||centers.length>2)drawRouteArrow(centers[index],next,cell,color);}}ctx.globalAlpha=.34; drawObjectShape(ctx,{...object,x:end.x,y:end.y},end.x*cell,end.y*cell,object.w*cell,object.h*cell,{preview:true,color});ctx.globalAlpha=1;
      }
    }
  ...
  function drawWireDrag(){const drag=state.wireDrag;if(!drag)return;const rect=canvas.getBoundingClientRect(),source=state.level.objects.find(object=>object.id===drag.sourceId);if(!source)return;const cell=cellPixels(),plug=linkSocketGeometry(source,source.x*cell,source.y*cell,source.w*cell,source.h*cell);ctx.save();ctx.strokeStyle='#65ff9a';ctx.lineWidth=4;ctx.setLineDash([7,5]);ctx.beginPath();ctx.moveTo(plug.cx,plug.cy);ctx.lineTo(drag.clientX-rect.left,drag.clientY-rect.top);ctx.stroke();ctx.restore();}

  function drawAvailableLinkSockets(cell){if(!state.wireDrag&&!state.linkSourceId)return;const time=performance.now()/260;ctx.save();ctx.setLineDash([]);for(const object of state.level.objects.filter(candidate=>LINKABLE_TYPES.has(candidate.type))){for(const socket of linkSocketEntries(object,object.x*cell,object.y*cell,object.w*cell,object.h*cell)){ctx.beginPath();ctx.arc(socket.cx,socket.cy,socket.radius,0,Math.PI*2);ctx.fillStyle='#08110e';ctx.fill();ctx.lineWidth=2.5;ctx.strokeStyle=socket.suffix==='@spikes'?'#ff8d99':socket.suffix==='@visibility'?'#d5a8ff':socket.suffix==='@reverse'?'#67d7ff':'#65ff9a';ctx.globalAlpha=.55+.45*Math.abs(Math.sin(time+object.x+object.y));ctx.stroke();ctx.globalAlpha=1;ctx.beginPath();ctx.arc(socket.cx,socket.cy,Math.max(1.5,socket.radius*.32),0,Math.PI*2);ctx.fillStyle=ctx.strokeStyle;ctx.fill();}}ctx.restore();}
  ...
  function renderCanvas() {
    ...
    if (state.drag?.kind === 'move') { const object=state.drag.object;const target=movePreviewRect();if(state.drag.deleteCandidate){ctx.save();ctx.globalAlpha=.78;ctx.fillStyle='rgba(255,45,67,.48)';ctx.fillRect(object.x*cell,object.y*cell,object.w*cell,object.h*cell);ctx.strokeStyle='#ff3348';ctx.lineWidth=4;ctx.beginPath();ctx.moveTo(object.x*cell,object.y*cell);ctx.lineTo((object.x+object.w)*cell,(object.y+object.h)*cell);ctx.moveTo((object.x+object.w)*cell,object.y*cell);ctx.lineTo(object.x*cell,(object.y+object.h)*cell);ctx.stroke();ctx.restore();}else if(target) drawObjectShape(ctx,{...object,...target},target.x*cell,target.y*cell,target.w*cell,target.h*cell,{preview:true,selected:true}); }
  ...
  function renderMobilePalette() {
    ...
    for (const id of category.items) {
      ...
      variant.addEventListener('click', () => {
        ...
        if(!isMobilePlayerMode()){setPaletteTool(id);closeMobileCategorySheet();return;}
  ...
  function routeForObject(object,end){
    ...
    const horizontalHalf=Math.min(3,Math.floor(Math.max(start.x,maxX-start.x)/2));
    const verticalRoom=Math.min(start.y,maxY-start.y);
    if(horizontalHalf>=1&&verticalRoom>=1){
      const sign=start.x+horizontalHalf*2<=maxX?1:-1,offset=Math.min(horizontalHalf,verticalRoom);
      return[start,{x:start.x+sign*offset,y:start.y+offset},{x:start.x+sign*offset*2,y:start.y},{x:start.x+sign*offset,y:start.y-offset}];
    }
    const verticalHalf=Math.min(3,Math.floor(Math.max(start.y,maxY-start.y)/2));
    const horizontalRoom=Math.min(start.x,maxX-start.x);
    if(verticalHalf>=1&&horizontalRoom>=1){
      const sign=start.y+verticalHalf*2<=maxY?1:-1,offset=Math.min(verticalHalf,horizontalRoom);
      return[start,{x:start.x+offset,y:start.y+sign*offset},{x:start.x,y:start.y+sign*offset*2},{x:start.x-offset,y:start.y+sign*offset}];
    }
    const roomRight=maxX-start.x,roomLeft=start.x,roomDown=maxY-start.y,roomUp=start.y;
    const xSign=roomRight>=roomLeft?1:-1,ySign=roomDown>=roomUp?1:-1;
    const width=Math.min(3,xSign>0?roomRight:roomLeft),height=Math.min(3,ySign>0?roomDown:roomUp);
    if(width>=1&&height>=1)return[start,{x:start.x+xSign*width,y:start.y},{x:start.x+xSign*width,y:start.y+ySign*height},{x:start.x,y:start.y+ySign*height}];
    return[start,end];
  }
  function appendTramNode(object){const path=Array.isArray(object.props?.path)?object.props.path.map(point=>({...point})):[{x:object.x,y:object.y}];const last=path[path.length-1]||{x:object.x,y:object.y},previous=path[path.length-2]||{x:last.x-5,y:last.y};let dx=Math.sign(last.x-previous.x),dy=Math.sign(last.y-previous.y);if(!dx&&!dy)dx=1;const desired={x:clamp(last.x+dx*5,0,state.level.size.width-object.w),y:clamp(last.y+dy*5,0,state.level.size.height-object.h)},next=snapTramNodeBetween(last,path[0],desired,object);path.push(next);object.props.path=path;}
  function removeTramNode(object){const path=Array.isArray(object.props?.path)?object.props.path:[];if(path.length>2)path.pop();}
  ...
  function linkTargetAt(point,pointerType='mouse') {
    ...
    for(const object of targets.reverse())for(const socket of linkSocketEntries(object,object.x*cell,object.y*cell,object.w*cell,object.h*cell))if(Math.hypot(x-socket.cx,y-socket.cy)<=Math.max(socket.radius,hitRadius))return{...object,linkDescriptor:`${object.id}${socket.suffix}`,linkSocketLabel:socket.label};
    return null;
  }

  function linkedSocketAt(point,pointerType='mouse'){const source=selectedObject();if(source?.type!=='button')return null;const target=linkTargetAt(point,pointerType);return target&&buttonHasTarget(source,target.id)?target:null;}
  ...
  function linkSelectedTo(target){const source=state.level.objects.find(object=>object.id===state.linkSourceId);if(!source){setTool('select');return;}if(!target||!LINKABLE_TYPES.has(target.type)||target.id===source.id){toast('Выберите мигающий сокет двери, платформы, шипа, пресса или генератора.','error');return;}if(buttonHasTarget(source,target.id)){toast('Эти предметы уже соединены — второй провод не нужен.');setTool('select');return;}const descriptor=target.linkDescriptor||target.id;mutate('Создана связь',()=>{source.props.targets=Array.isArray(source.props.targets)?source.props.targets:[];source.props.targets.push(descriptor);});state.selectedId=source.id;setTool('select');toast(`Связь создана: ${target.linkSocketLabel||'управление'}. К розетке можно подключить несколько разных кнопок.`,'ok');}
  ...
  function handlePointerDown(event){if(!state.ready)return;if(event.pointerType==='touch')event.preventDefault();if(state.mobilePaletteExpanded)closeMobileCategorySheet();state.pointers.set(event.pointerId,{x:event.clientX,y:event.clientY});canvas.setPointerCapture?.(event.pointerId);if(state.pointers.size===2){beginPinch();return;}viewport.focus({preventScroll:true});const point=pointerGridPoint(event);
    const selectedTram=selectedObject()?.type==='smartPlatform'?selectedObject():null,routeNodeIndex=selectedTram?tramRouteNodeHit(selectedTram,point,event.pointerType):-1;if((event.button===2||state.tool==='erase')&&routeNodeIndex>0&&selectedTram.props.path.length>2){mutate('Узел маршрута удалён',()=>selectedTram.props.path.splice(routeNodeIndex,1));toast('Узел трамвая удалён.','ok');return;}
    ...
    if(state.tool==='select'||state.tool==='place'){let endpointObject=selectedObject(),linkedTarget=state.tool==='select'?linkedSocketAt(point,event.pointerType):null;if(linkedTarget){state.drag={kind:'wireDetach',sourceId:endpointObject.id,targetId:linkedTarget.id,start:point,current:point};toast('Вилка вынута. Отпустите в стороне, чтобы удалить провод.');renderCanvas();return;}if(state.tool==='select'&&endpointObject?.type==='smartPlatform'){let nodeIndex=tramRouteNodeHit(endpointObject,point,event.pointerType);if(nodeIndex<0){const segment=tramRouteSegmentHit(endpointObject,point,event.pointerType);if(segment){mutate('Узел маршрута добавлен',()=>{endpointObject.props.path.splice(segment.index,0,segment.point);});nodeIndex=segment.index;}}if(nodeIndex>=1){const previousTap=state.lastTramNodeTap,now=Date.now();if(previousTap?.id===endpointObject.id&&previousTap.index===nodeIndex&&now-previousTap.time<340&&endpointObject.props.path.length>2){state.lastTramNodeTap=null;mutate('Узел маршрута удалён',()=>endpointObject.props.path.splice(nodeIndex,1));toast('Узел трамвая удалён двойным нажатием.','ok');return;}state.lastTramNodeTap={id:endpointObject.id,index:nodeIndex,time:now};const node=endpointObject.props.path[nodeIndex];state.drag={kind:'pathNode',nodeIndex,pointerType:event.pointerType,object:deepClone(endpointObject),start:point,current:point,offsetX:point.rawX-node.x,offsetY:point.rawY-node.y};canvas.style.cursor='grabbing';renderCanvas();return;}}if(state.tool==='select'){endpointObject=pathEndpointObjectAt(point,event.pointerType)||endpointObject;if(endpointObject&&pathEndpointHit(endpointObject,point,event.pointerType)){state.selectedId=endpointObject.id;const end=pathEnd(endpointObject);state.drag={kind:'pathEndpoint',pointerType:event.pointerType,object:deepClone(endpointObject),start:point,current:point,offsetX:point.rawX-end.x,offsetY:point.rawY-end.y};refreshAll();canvas.style.cursor='grabbing';renderCanvas();return;}}const object=objectAt(point);if(object&&!(state.tool==='place'&&state.activePaletteId==='solid')){state.selectedId=object.id;showInteractionHint(object);state.drag={kind:'move',pointerType:event.pointerType,object:deepClone(object),start:point,current:point,offsetX:point.rawX-object.x,offsetY:point.rawY-object.y};refreshAll();return;}if(state.tool==='select'){state.selectedId=null;showInteractionHint(null);state.pan={x:event.clientX,y:event.clientY,scrollLeft:viewport.scrollLeft,scrollTop:viewport.scrollTop};viewport.classList.add('dragging');refreshAll();return;}}
  ...
  function handlePointerMove(event){if(state.pointers.has(event.pointerId))state.pointers.set(event.pointerId,{x:event.clientX,y:event.clientY});if(state.pinch){updatePinch();return;}const point=pointerGridPoint(event);state.hoverPoint=point;$('cursorReadout').style.display='none';$('cursorStatus').textContent='';
    ...
    else {const hovered=objectAt(point);if(state.tool==='select'){canvas.style.cursor=pathEndpointHit(selectedObject(),point,event.pointerType)?'grab':hovered?'pointer':'grab';showInteractionHint(hovered);}if(state.tool==='place')renderCanvas();}
  }
  ...
  function handlePointerUp(event){state.pointers.delete(event.pointerId);if(state.pinch){if(state.pointers.size<2)state.pinch=null;return;}if(state.pan){state.pan=null;viewport.classList.remove('dragging');return;}const drag=state.drag;state.drag=null;if(!drag)return;
    ...
    else if(drag.kind==='move'){if(!pointInsideCanvas(event.clientX,event.clientY)){if(removeObject(drag.object.id))toast('Предмет удалён: он вынесен за границу поля.','ok');}else{const target=movePreviewRectFromDrag(drag);const current=state.level.objects.find(object=>object.id===drag.object.id);if(current&&target&&(target.x!==current.x||target.y!==current.y)){const placement=canPlace({...current,...target},[current.id]);if(placement.ok)mutate('Предмет перемещён',()=>{const moveDx=target.x-drag.object.x,moveDy=target.y-drag.object.y;current.x=target.x;current.y=target.y;if(PATH_ENDPOINT_TYPES.has(current.type)){if(current.type==='smartPlatform'&&Array.isArray(drag.object.props?.path))current.props.path=drag.object.props.path.map(point=>({x:clamp(point.x+moveDx,0,state.level.size.width-current.w),y:clamp(point.y+moveDy,0,state.level.size.height-current.h)}));else{const end=pathEnd(drag.object);const shifted={x:clamp(end.x+moveDx,0,state.level.size.width-current.w),y:clamp(end.y+moveDy,0,state.level.size.height-current.h)};current.props.path=routeForObject(current,shifted);}}});else toast(placement.message,'error');}}}
    else if(drag.kind==='pathEndpoint'){const current=state.level.objects.find(object=>object.id===drag.object.id);const end=pathEndpointFromDrag(drag);const previous=pathEnd(drag.object);if(current&&(end.x!==previous.x||end.y!==previous.y)){const placement=pathEndpointPlacement(current,end);if(placement.ok)mutate('Конечная точка маршрута перемещена',()=>{if(current.type==='smartPlatform'&&Array.isArray(current.props?.path)&&current.props.path.length>1){const index=smartEndpointIndex(current);current.props.path[index]=snapTramNodeBetween(current.props.path[(index-1+current.props.path.length)%current.props.path.length],current.props.path[(index+1)%current.props.path.length],end,current);}else current.props.path=routeForObject(current,end);});else toast(`Конечная точка маршрута: ${placement.message}`,'error');}}
    else if(drag.kind==='pathNode'){const current=state.level.objects.find(object=>object.id===drag.object.id),next=pathNodeFromDrag(drag),previous=drag.object.props.path[drag.nodeIndex];if(current&&!pointInsideCanvas(event.clientX,event.clientY)&&current.props.path.length>2){mutate('Узел маршрута удалён',()=>current.props.path.splice(drag.nodeIndex,1));toast('Узел трамвая удалён за границей поля.','ok');}else if(current&&previous&&(next.x!==previous.x||next.y!==previous.y)){const placement=pathEndpointPlacement(current,next);if(placement.ok)mutate('Узел маршрута перемещён',()=>{current.props.path[drag.nodeIndex]=next;});else toast(`Узел маршрута: ${placement.message}`,'error');}}
    else if(drag.kind==='wireDetach'){const source=state.level.objects.find(object=>object.id===drag.sourceId),sameSocket=pointInsideCanvas(event.clientX,event.clientY)&&linkTargetAt(pointerGridPoint(event),event.pointerType)?.id===drag.targetId;if(source&&!sameSocket){mutate('Связь удалена',()=>{source.props.targets=(source.props?.targets||[]).filter(value=>targetDescriptorId(value)!==drag.targetId);});toast('Провод отключён.','ok');}}
  ...
  function renderFriendlyProperties(object){const root=$('friendlyProperties');root.innerHTML='';const defs=PROPERTY_DEFS[object.type]||[];for(const definition of defs){const row=document.createElement('label');row.className='property-row';const label=document.createElement('span');label.textContent=definition.label;let input;if(definition.type==='select'){input=document.createElement('select');for(const [value,text] of definition.options){const option=document.createElement('option');option.value=value;option.textContent=text;input.append(option);}input.value=String(getPropertyValue(object,definition.key)??definition.options[0][0]);}else{input=document.createElement('input');input.type=definition.type==='checkbox'?'checkbox':definition.type==='text'?'text':'number';if(definition.type==='checkbox')input.checked=!!getPropertyValue(object,definition.key);else{input.value=String(getPropertyValue(object,definition.key)??'');if(definition.type==='number'){input.min=definition.min;input.max=definition.max;input.step=definition.step;}if(definition.maxLength)input.maxLength=definition.maxLength;}}
    ...
    if(object.type==='button'){const links=document.createElement('div');links.className='link-list';for(const descriptor of object.props?.targets||[]){const target=state.level.objects.find(candidate=>candidate.id===targetDescriptorId(descriptor));const chip=document.createElement('button');chip.type='button';chip.className='link-chip';chip.textContent=target?`${TYPE_DEFS[target.type]?.label||target.type}${String(descriptor).endsWith('@reverse')?' · реверс':''}`:'Потерянная связь';chip.title='Нажмите, чтобы удалить связь';chip.addEventListener('click',()=>mutate('Связь удалена',()=>{object.props.targets=object.props.targets.filter(value=>value!==descriptor);}));links.append(chip);}root.append(links);}
  ...
  function renderContextToolbar(){const root=$('contextToolbar'),handles=$('resizeHandles'),object=selectedObject();if(!root||!handles||!object||state.tool==='link'){if(root)root.hidden=true;if(handles)handles.hidden=true;return;}const def=TYPE_DEFS[object.type],add=(icon,label,action,className='',active=false,id='')=>{const button=contextIconButton(icon,label,action,className,active,id);root.append(button);return button;};root.innerHTML='';root.hidden=false;
    ...
    if(object.type==='spike'){const linked=incomingLinks(object.id).length>0,mode=linked?'toggle':object.props?.mode||'always';if(!linked){add(mode==='always'?'∞':'◷',`Режим: ${mode==='always'?'всегда':'по циклу'}`,()=>cycleContextValue(object,'mode',['always','cycle'],'Режим',['всегда','по циклу']),'context-variant context-parent',mode==='cycle','mode');if(mode==='cycle')add((object.props?.cycle||2)>=4?'>>>':(object.props?.cycle||2)>=2?'>>':'>',`Цикл: ${formatNumber(object.props?.cycle||2)} сек`,()=>cycleContextNumber(object,'cycle',[1,2,4],'Цикл',' сек'),'context-variant context-nested',false,'cycle');}}
    ...
    if(object.type==='playerCannon'){const manual=!!object.props?.manual;add(manual?'↻':'✋',manual?'Включить автоматическое вращение':'Включить ручное управление',()=>contextMutation('Режим пушки изменён',()=>{object.props.manual=!manual;},manual?'Включено автовращение':'Включено ручное управление'),'context-state context-parent',manual,'manual');if(!manual){const interval=Number(object.props?.rotateInterval)||.5,clockwise=object.props?.clockwise!==false;add(interval<=.25?'>>>':interval<=.5?'>>':'>',`Скорость вращения: ${formatNumber(interval)} сек`,()=>cycleContextNumber(object,'rotateInterval',[1,.5,.25],'Шаг вращения',' сек'),'context-variant context-nested',false,'cannon-speed');add(clockwise?'↻':'↺',clockwise?'Вращение по часовой стрелке':'Вращение против часовой стрелки',()=>contextMutation('Направление вращения изменено',()=>{object.props.clockwise=!clockwise;},clockwise?'Вращение против часовой':'Вращение по часовой'),'context-variant context-nested',clockwise,'cannon-clockwise');}}
    if(object.type==='enemyFlyer'){const value=Number(object.props?.distance)||5,label=value<=2?'Близко':value>=10?'Далеко':'Средне',arrow=directionArrow(object.props?.direction||'right'),icon=value<=2?arrow:value>=10?`${arrow}${arrow}${arrow}`:`${arrow}${arrow}`;add(icon,`Направление и расстояние: ${label}`,()=>cycleContextNumber(object,'distance',[2,5,10],'Расстояние',''),'context-variant',false,'distance');}
  ...
  function gameLobbyUrl(){const decoded=decodeURIComponent(window.location.pathname);return decoded.includes('/tools/level-editor/')?new URL('../../02 Разработка/game/index.html',window.location.href):new URL('../index.html',window.location.href);}
  ...
  async function receiveSharedLevel(event){const file=event.target.files?.[0];event.target.value='';if(!file)return;try{const payload=JSON.parse(await file.text());if(payload?.kind!=='nubu.level-transfer'||payload?.version!==1||!payload.slot?.difficulties)throw new Error('Это не файл переноса NuBu2600.');await refreshUserSlots();if(state.userSlots.length>=MAX_USER_LEVELS)throw new Error('Достигнут лимит 100 уровней.');const key=`user-${Date.now()}-${Math.random().toString(36).slice(2,7)}`,slot=deepClone(payload.slot);slot.key=key;slot.kind='user';slot.sequence=state.userSlots.length+1;slot.title=String(slot.title||'Полученный уровень').slice(0,48);slot.publicationStatus='draft';slot.clearProofs={};slot.revisions=[];slot.createdAt=Date.now();slot.updatedAt=Date.now();for(const difficulty of DIFFICULTIES)slot.difficulties[difficulty]=normalizeLevel(slot.difficulties[difficulty]||slot.difficulties.easy,{difficulty});await dbPut(slot);closeLibrary();await loadSlot(key,'easy');toast('Уровень получен и сохранён как новый черновик.','ok');}catch(error){toast(`Не удалось получить уровень: ${error.message}`,'error');}}

  function openLibrary(){if(state.slot){$('librarySourceSelect').value=state.slot.kind==='campaign'?`campaign-${state.slot.episode||1}`:'user';state.librarySelectedKey=state.slotKey;state.libraryDifficulty=state.difficulty;}$('libraryDifficultySelect').value=state.libraryDifficulty;renderLibrary();$('libraryModal').classList.add('open');$('libraryModal').setAttribute('aria-hidden','false');}
  ...
  function makeLibraryCard(slot,{campaign=false}={}){const status=campaign?{key:'ready',label:'Кампания'}:playerSlotStatus(slot),level=libraryLevelFor(slot),displayTitle=campaign?(level?.title||slot.title):slot.title,card=document.createElement('article');card.className='level-card-wrap';card.dataset.status=status.key;card.classList.toggle('selected',slot.key===state.librarySelectedKey);card.classList.toggle('current',slot.key===state.slotKey);const select=document.createElement('button');select.type='button';select.className='level-card';select.setAttribute('aria-pressed',String(slot.key===state.librarySelectedKey));const thumb=document.createElement('canvas');thumb.width=64;thumb.height=64;const info=document.createElement('div'),title=document.createElement('strong'),meta=document.createElement('small'),statusChip=document.createElement('small');title.textContent=campaign?`${slot.episode}-${slot.sequence} · ${displayTitle}`:`№${slot.sequence} · ${displayTitle}`;meta.textContent=`${campaign?`${difficultyTitle(state.libraryDifficulty)} · `:''}${level.size.width}×${level.size.height} · ${new Date(slot.updatedAt||slot.createdAt||Date.now()).toLocaleString('ru-RU',{dateStyle:'short',timeStyle:'short'})}`;statusChip.className='level-status';statusChip.dataset.status=status.key;statusChip.textContent=slot.key===state.slotKey?'Сейчас открыт':status.label;info.append(title,meta,statusChip);select.append(thumb,info);select.addEventListener('click',()=>{state.librarySelectedKey=slot.key;state.librarySelectedSlot=slot;renderLibrary();showHintText('Уровень выбран','Теперь можно открыть его или выбрать действие в крупной карточке.');});card.append(select);drawLevelThumbnail(thumb,level);return card;}
  function renderLibrarySelection(slot,showUsers){const panel=$('librarySelected');if(!slot){panel.hidden=true;state.librarySelectedSlot=null;return;}state.librarySelectedSlot=slot;panel.hidden=false;const level=libraryLevelFor(slot),status=showUsers?playerSlotStatus(slot):{key:'ready',label:'Кампания'},displayTitle=showUsers?slot.title:(level?.title||slot.title);$('librarySelectedKind').textContent=slot.key===state.slotKey?'Сейчас открыт':status.label;$('librarySelectedTitle').textContent=showUsers?`№${slot.sequence} · ${displayTitle}`:`${slot.episode}-${slot.sequence} · ${displayTitle}`;$('librarySelectedMeta').textContent=`${difficultyTitle(showUsers?'easy':state.libraryDifficulty)} · ${level.size.width}×${level.size.height} · изменён ${new Date(slot.updatedAt||slot.createdAt||Date.now()).toLocaleString('ru-RU',{dateStyle:'medium',timeStyle:'short'})}`;$('libraryLevelTitleInput').value=displayTitle||'';$('librarySelectedTitleField').hidden=!showUsers;$('examLevelButton').hidden=!showUsers;$('libraryDeleteButton').hidden=!showUsers;for(const id of ['libraryCopyMapButton','libraryPasteMapButton','libraryExportLevelButton','libraryImportLevelLabel'])$(id).hidden=showUsers;drawLevelThumbnail($('librarySelectedPreview'),level);}
  async function renderLibrary(){await refreshUserSlots();const root=$('libraryList'),campaignRoot=$('campaignLibraryList'),source=$('librarySourceSelect').value||'user',showUsers=source==='user',all=await dbGetAll();root.innerHTML='';campaignRoot.innerHTML='';const episode=Number(source.split('-')[1])||1;$('libraryHeading').textContent=showUsers?'Мои уровни':`Эпизод ${episode}`;$('libraryDescription').textContent=showUsers?'Черновики и опубликованные уровни на этом устройстве.':`Уровни эпизода ${episode}; выберите сложность и карту.`;$('userSlotCount').textContent=`${state.userSlots.length} / ${MAX_USER_LEVELS}`;$('libraryDifficultySelect').value=state.libraryDifficulty;root.hidden=!showUsers;$('campaignLibrarySection').hidden=showUsers;$('newUserLevelButton').hidden=!showUsers;$('randomStarterCheckbox').closest('label').hidden=!showUsers;$('userSlotCount').hidden=!showUsers;document.querySelector('.player-status-legend').hidden=!showUsers;$('exportLibraryButton').hidden=showUsers;if(showUsers){if(!state.userSlots.some(slot=>slot.key===state.librarySelectedKey))state.librarySelectedKey=state.userSlots.find(slot=>slot.key===state.slotKey)?.key||state.userSlots[0]?.key||null;if(!state.userSlots.length){const empty=document.createElement('div');empty.className='empty-state';empty.textContent='Пока нет пользовательских уровней. Создайте первый уровень.';root.append(empty);}for(const slot of state.userSlots)root.append(makeLibraryCard(slot));renderLibrarySelection(state.userSlots.find(slot=>slot.key===state.librarySelectedKey)||null,true);return;}const campaignSlots=all.filter(slot=>slot.kind==='campaign'&&Number(slot.episode||1)===episode).sort((a,b)=>a.sequence-b.sequence);$('libraryEpisodeSelect').value=String(episode);if(!campaignSlots.some(slot=>slot.key===state.librarySelectedKey))state.librarySelectedKey=campaignSlots.find(slot=>slot.key===state.slotKey)?.key||campaignSlots[0]?.key||null;for(const slot of campaignSlots)campaignRoot.append(makeLibraryCard(slot,{campaign:true}));renderLibrarySelection(campaignSlots.find(slot=>slot.key===state.librarySelectedKey)||null,false);}

  async function openSelectedLibraryLevel(){const slot=state.librarySelectedSlot;if(!slot)return;closeLibrary();await loadSlot(slot.key,slot.kind==='campaign'?state.libraryDifficulty:'easy');}
  function copySelectedLibraryMap(){const level=libraryLevelFor(state.librarySelectedSlot);if(!level)return;const payload={kind:'nubu.map-clipboard',version:1,copiedAt:Date.now(),level:deepClone(level)};state.mapClipboard=payload;try{localStorage.setItem(MAP_CLIPBOARD_KEY,JSON.stringify(payload));}catch(error){}toast(`Карта ${level.size.width}×${level.size.height} скопирована.`,'ok');}
  async function replaceSelectedCampaignFromClipboard(){const slot=state.librarySelectedSlot;if(slot?.kind!=='campaign')return;let payload=state.mapClipboard;try{payload=payload||JSON.parse(localStorage.getItem(MAP_CLIPBOARD_KEY)||'null');}catch(error){}if(payload?.kind!=='nubu.map-clipboard'||payload.level?.kind!=='nubu.level'){toast('Сначала скопируйте карту кампании.','error');return;}const ok=await confirmAction('Заменить выбранную карту?',`${difficultyTitle(state.libraryDifficulty)} карта «${slot.title}» будет заменена копией.`);if(!ok)return;const current=slot.difficulties[state.libraryDifficulty],next=normalizeLevel({...deepClone(payload.level),id:current.id,title:current.title,episode:slot.episode,sequence:slot.sequence,metadata:{...(payload.level.metadata||{}),difficulty:state.libraryDifficulty}},{difficulty:state.libraryDifficulty,episode:slot.episode,sequence:slot.sequence});slot.difficulties[state.libraryDifficulty]=next;slot.updatedAt=Date.now();await dbPut(slot);if(state.slotKey===slot.key&&state.difficulty===state.libraryDifficulty)await loadSlot(slot.key,state.libraryDifficulty);await renderLibrary();toast('Выбранная карта заменена копией.','ok');}
  ...
  async function importSelectedCampaignLevel(event){const file=event.target.files?.[0];event.target.value='';const slot=state.librarySelectedSlot;if(!file||slot?.kind!=='campaign')return;try{if(file.size>512*1024)throw new Error('файл больше 512 КБ');const raw=JSON.parse(await file.text());if(raw?.kind!=='nubu.level'||raw?.schemaVersion!==1)throw new Error('это не карта NuBu2600');const unknown=raw.objects?.find(object=>!TYPE_DEFS[object?.type]);if(unknown)throw new Error(`неизвестный предмет ${unknown.type}`);const ok=await confirmAction('Заменить карту из файла?',`${difficultyTitle(state.libraryDifficulty)} карта «${slot.title}» будет заменена файлом ${file.name}.`);if(!ok)return;const current=slot.difficulties[state.libraryDifficulty],next=normalizeLevel({...raw,id:current.id,title:current.title,episode:slot.episode,sequence:slot.sequence,metadata:{...(raw.metadata||{}),difficulty:state.libraryDifficulty}},{difficulty:state.libraryDifficulty,episode:slot.episode,sequence:slot.sequence});slot.difficulties[state.libraryDifficulty]=next;slot.updatedAt=Date.now();await dbPut(slot);if(state.slotKey===slot.key&&state.difficulty===state.libraryDifficulty)await loadSlot(slot.key,state.libraryDifficulty);await renderLibrary();toast(`Карта заменена файлом ${file.name}.`,'ok');}catch(error){toast(`Не удалось заменить карту: ${error.message}`,'error');}}
  ...
  function bindUi(){renderPalette();renderFavorites();renderMobilePalette();
    ...
    $('libraryButton').addEventListener('click',openLibrary);$('mobileLevelButton').addEventListener('click',openLibrary);$('closeLibraryButton').addEventListener('click',closeLibrary);$('newUserLevelButton').addEventListener('click',()=>createUserSlot($('randomStarterCheckbox').checked));$('randomStarterButton').addEventListener('click',()=>createUserSlot(true));$('manualSaveButton').addEventListener('click',()=>shareCurrentLevel().catch(error=>toast(error.message,'error')));$('libraryOpenButton').addEventListener('click',()=>openSelectedLibraryLevel().catch(error=>toast(error.message,'error')));$('libraryLevelTitleInput').addEventListener('change',async event=>{const slot=state.librarySelectedSlot;if(slot?.kind!=='user')return;const title=String(event.target.value).trim().slice(0,48)||'Без названия';slot.title=title;for(const difficulty of DIFFICULTIES)slot.difficulties[difficulty].title=title;slot.updatedAt=Date.now();await dbPut(slot);if(state.slotKey===slot.key){state.slot=slot;state.level=slot.difficulties[state.difficulty];refreshAll();}await refreshUserSlots();renderLibrary();});$('libraryCopyMapButton').addEventListener('click',copySelectedLibraryMap);$('libraryPasteMapButton').addEventListener('click',()=>replaceSelectedCampaignFromClipboard().catch(error=>toast(error.message,'error')));$('libraryExportLevelButton').addEventListener('click',exportSelectedCampaignLevel);$('libraryImportLevelInput').addEventListener('change',importSelectedCampaignLevel);$('libraryDeleteButton').addEventListener('click',()=>state.librarySelectedSlot?.kind==='user'&&deleteUserSlot(state.librarySelectedSlot.key));$('librarySourceSelect').addEventListener('change',()=>{state.librarySelectedKey=null;renderLibrary();});$('libraryLobbyButton').addEventListener('click',async()=>{await saveNow({revision:true});window.location.assign(gameLobbyUrl().href);});$('libraryEpisodeSelect').addEventListener('change',()=>{$('librarySourceSelect').value=`campaign-${$('libraryEpisodeSelect').value}`;state.librarySelectedKey=null;renderLibrary();});$('libraryDifficultySelect').addEventListener('change',event=>{state.libraryDifficulty=event.target.value;renderLibrary();});$('examLevelButton').addEventListener('click',async()=>{const slot=state.librarySelectedSlot;if(slot?.kind!=='user'){toast('Экзамен доступен только для уровней игрока.','error');return;}closeLibrary();await loadSlot(slot.key,'easy');const ok=await confirmAction('Экзамен уровня','Уровень запустится с обычного входа. Чтобы получить статус «Опубликован», нужно дойти до выхода без смерти.');if(!ok)return;try{localStorage.setItem(EXAM_PENDING_KEY,JSON.stringify({slotKey:slot.key,difficulty:'easy'}));}catch(error){}playLevel();});document.querySelectorAll('[data-size]').forEach(button=>button.addEventListener('click',()=>{document.querySelectorAll('[data-size]').forEach(item=>item.classList.toggle('active',item===button));state.chosenSize=button.dataset.size.split('x').map(Number);}));const updateChosenSize=()=>{state.chosenSize=[Number($('newLevelWidth').value),Number($('newLevelHeight').value)];};$('newLevelWidth').addEventListener('change',updateChosenSize);$('newLevelHeight').addEventListener('change',updateChosenSize);

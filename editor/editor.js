(() => {
  ...
  const MOBILE_PALETTE_CATEGORIES = [
    { id:'base', label:'Основа', icon:'▦', items:['solid','coin','pickup-gravity','label'] },
    ...
    { id:'mechanisms', label:'Механизмы', icon:'⚙', items:['driftField','crusherWall','door','button-toggle','portal','playerCannon','generator'] },
    { id:'other', label:'Остальное', icon:'⋯', items:['collectible','pickup-grab','pickup-wall','pickup-double','pickup-jetpack','pickup-float','unlockSwitch','heartVendor','developerNote'] },
  ];
  ...
  function linkSocketGeometry(object,x,y,w,h) {
    ...
    return{radius,cx:x+w/2,cy:y+h+radius+9,anchorX:x+w/2,anchorY:y+h,external:true};
  }
  ...
  function pathNodeFromDrag(drag) {
    ...
    return{x:clamp(snap(point.rawX-drag.offsetX,GRID_STEP),0,state.level.size.width-object.w),y:clamp(snap(point.rawY-drag.offsetY,GRID_STEP),0,state.level.size.height-object.h)};
  }
  ...
  function drawConnections(cell) {
    ...
    for (const object of state.level.objects) {
      if (object.type === 'button') {
        ...
        for (const descriptor of targets) { const target = state.level.objects.find(candidate => candidate.id === targetDescriptorId(descriptor)); if (!target) continue; ctx.strokeStyle = String(descriptor).endsWith('@reverse')?'#67d7ff':'#65ff9a'; ctx.lineWidth=3; ctx.setLineDash([7,5]); ctx.beginPath(); ctx.moveTo((object.x+object.w/2)*cell,(object.y+object.h/2)*cell); ctx.lineTo((target.x+target.w/2)*cell,(target.y+target.h/2)*cell); ctx.stroke(); }
      }
      if (PATH_ENDPOINT_TYPES.has(object.type)) {
        const end = pathEndForRender(object); const placement = pathEndpointPlacement(object,end); const color = placement.ok ? TYPE_DEFS[object.type].color : '#ff6974';let points=object.type==='smartPlatform'?(state.drag?.kind==='pathEndpoint'&&state.drag.object.id===object.id?routeForObject(object,end):(object.props?.path||routeForObject(object,end))):[{x:object.x,y:object.y},end];if(object.type==='smartPlatform'&&state.drag?.kind==='pathNode'&&state.drag.object.id===object.id){points=points.map((node,index)=>index===state.drag.nodeIndex?pathNodeFromDrag(state.drag):node);}ctx.strokeStyle = color; ctx.setLineDash([5,4]); ctx.beginPath();points.forEach((point,index)=>{const x=(point.x+object.w/2)*cell,y=(point.y+object.h/2)*cell;if(index)ctx.lineTo(x,y);else ctx.moveTo(x,y);});ctx.stroke();if(object.type==='smartPlatform'){ctx.setLineDash([]);for(const point of points.slice(1)){ctx.beginPath();ctx.arc((point.x+object.w/2)*cell,(point.y+object.h/2)*cell,5,0,Math.PI*2);ctx.fillStyle='#08100d';ctx.fill();ctx.lineWidth=2;ctx.strokeStyle=color;ctx.stroke();}}ctx.globalAlpha=.34; drawObjectShape(ctx,{...object,x:end.x,y:end.y},end.x*cell,end.y*cell,object.w*cell,object.h*cell,{preview:true,color});ctx.globalAlpha=1;
      }
    }
  ...
  function routeForObject(object,end){return [{x:object.x,y:object.y},end];}
  function appendTramNode(object){const path=Array.isArray(object.props?.path)?object.props.path.map(point=>({...point})):[{x:object.x,y:object.y}];const last=path[path.length-1]||{x:object.x,y:object.y},previous=path[path.length-2]||{x:last.x-5,y:last.y};let dx=Math.sign(last.x-previous.x),dy=Math.sign(last.y-previous.y);if(!dx&&!dy)dx=1;const next={x:clamp(last.x+dx*5,0,state.level.size.width-object.w),y:clamp(last.y+dy*5,0,state.level.size.height-object.h)};if(next.x===last.x&&next.y===last.y){next.x=clamp(last.x-dx*5,0,state.level.size.width-object.w);next.y=clamp(last.y-dy*5,0,state.level.size.height-object.h);}path.push(next);object.props.path=path;}
  ...
  function linkSelectedTo(target){const source=state.level.objects.find(object=>object.id===state.linkSourceId);if(!source){setTool('select');return;}if(!target||!LINKABLE_TYPES.has(target.type)||target.id===source.id){toast('Выберите мигающий сокет двери, платформы, шипа, пресса или генератора.','error');return;}if(buttonHasTarget(source,target.id)){toast('Эти предметы уже соединены — второй провод не нужен.');setTool('select');return;}const reverse=target.type==='conveyor'&&incomingLinks(target.id).length>0,descriptor=reverse?`${target.id}@reverse`:target.id;mutate('Создана связь',()=>{source.props.targets=Array.isArray(source.props.targets)?source.props.targets:[];source.props.targets.push(descriptor);});state.selectedId=source.id;setTool('select');toast(reverse?'Связь подключена к голубой розетке реверса конвейера.':'Связь создана. К одной розетке можно подключать разные кнопки.','ok');}
  ...
  function handlePointerUp(event){state.pointers.delete(event.pointerId);if(state.pinch){if(state.pointers.size<2)state.pinch=null;return;}if(state.pan){state.pan=null;viewport.classList.remove('dragging');return;}const drag=state.drag;state.drag=null;if(!drag)return;
    ...
    else if(drag.kind==='pathEndpoint'){const current=state.level.objects.find(object=>object.id===drag.object.id);const end=pathEndpointFromDrag(drag);const previous=pathEnd(drag.object);if(current&&(end.x!==previous.x||end.y!==previous.y)){const placement=pathEndpointPlacement(current,end);if(placement.ok)mutate('Конечная точка маршрута перемещена',()=>{if(current.type==='smartPlatform'&&Array.isArray(current.props?.path)&&current.props.path.length>1)current.props.path[current.props.path.length-1]=end;else current.props.path=routeForObject(current,end);});else toast(`Конечная точка маршрута: ${placement.message}`,'error');}}
    else if(drag.kind==='pathNode'){const current=state.level.objects.find(object=>object.id===drag.object.id),next=pathNodeFromDrag(drag),previous=drag.object.props.path[drag.nodeIndex];if(current&&previous&&(next.x!==previous.x||next.y!==previous.y)){const placement=pathEndpointPlacement(current,next);if(placement.ok)mutate('Узел маршрута перемещён',()=>{current.props.path[drag.nodeIndex]=next;});else toast(`Узел маршрута: ${placement.message}`,'error');}}
  ...
  function renderContextToolbar(){const root=$('contextToolbar'),handles=$('resizeHandles'),object=selectedObject();if(!root||!handles||!object||state.tool==='link'){if(root)root.hidden=true;if(handles)handles.hidden=true;return;}const def=TYPE_DEFS[object.type],add=(icon,label,action,className='',active=false,id='')=>{const button=contextIconButton(icon,label,action,className,active,id);root.append(button);return button;};root.innerHTML='';root.hidden=false;
    ...
    if(object.type==='blinkPlatform'){const value=Number(object.props?.cycle)||2,label=value>=4?'Медленно':value>=2?'Средне':'Быстро';add(value>=4?'>':value>=2?'>>':'>>>',`Скорость цикла: ${label}`,()=>cycleContextNumber(object,'cycle',[4,2,1],'Скорость','','',['медленно','средне','быстро']),'context-variant',false,'cycle');}
  ...
  function isAllowedLevelSize(width,height){return width>=20&&height>=20&&width<=100&&height<=100&&width%20===0&&height%20===0&&(width===20||height===20);}
  function canResizeLevelAxis(horizontal,delta){const width=state.level.size.width,height=state.level.size.height,nextWidth=horizontal?width+delta:width,nextHeight=horizontal?height:height+delta;return isAllowedLevelSize(nextWidth,nextHeight);}
  ...
  function validateLevel(selectChecks=false){const issues=[];const add=(severity,message,objectId=null)=>issues.push({severity,message,objectId});const level=state.level;const title=level.title.trim();if(!title)add('error','Введите название уровня.');if(title.length>48)add('error','Название длиннее 48 символов.');if(/(?:https?:\/\/|www\.|@\w+\.)/iu.test(title))add('error','В названии нельзя размещать ссылки или адреса.');if(/(?:хуй|пизд|еба|бля|fuck|shit)/iu.test(title))add('error','Название не прошло локальный фильтр публикации.');
    if(!isAllowedLevelSize(level.size.width,level.size.height))add('error','Допустимы только шахты: 20×20…100 или 20…100×20. Карты 40×40, 100×40 и 100×100 запрещены.');
  ...
  function showHintText(label,help){const root=$('interactionHint');if(!root)return;root.querySelector('strong').textContent=label;root.querySelector('span').textContent=help;}
  ...
  function openLibrary(){if(state.slot)$('librarySourceSelect').value=state.slot.kind==='campaign'?`campaign-${state.slot.episode||1}`:'user';renderLibrary();$('libraryModal').classList.add('open');$('libraryModal').setAttribute('aria-hidden','false');}
  function closeLibrary(){$('libraryModal').classList.remove('open');$('libraryModal').setAttribute('aria-hidden','true');}
  function makeLibraryCard(slot,{campaign=false,difficulty='easy'}={}){const status=campaign?{key:'ready',label:'Кампания'}:playerSlotStatus(slot),level=slot.difficulties[difficulty]||slot.difficulties.easy,card=document.createElement('article');card.className='level-card-wrap';card.dataset.status=status.key;const open=document.createElement('button');open.type='button';open.className='level-card';const thumb=document.createElement('canvas');thumb.width=64;thumb.height=64;const info=document.createElement('div'),title=document.createElement('strong'),meta=document.createElement('small'),statusChip=document.createElement('small');title.textContent=campaign?`${slot.episode}-${slot.sequence} · ${slot.title}`:`№${slot.sequence} · ${slot.title}`;meta.textContent=campaign?`${difficultyTitle(difficulty)} · ${level.size.width}×${level.size.height}`:`${level.size.width}×${level.size.height} · ${new Date(slot.updatedAt).toLocaleDateString('ru-RU')}`;statusChip.className='level-status';statusChip.dataset.status=status.key;statusChip.textContent=status.label;info.append(title,meta,statusChip);open.append(thumb,info);open.addEventListener('click',async()=>{closeLibrary();await loadSlot(slot.key,campaign?difficulty:'easy');});card.append(open);if(!campaign){const remove=document.createElement('button');remove.type='button';remove.className='level-card-delete';remove.textContent='Удалить';remove.addEventListener('click',()=>deleteUserSlot(slot.key));card.append(remove);}drawLevelThumbnail(thumb,level);return card;}
  async function renderLibrary(){await refreshUserSlots();const root=$('libraryList'),campaignRoot=$('campaignLibraryList'),source=$('librarySourceSelect').value||'user',showUsers=source==='user';root.innerHTML='';campaignRoot.innerHTML='';$('libraryHeading').textContent=showUsers?'Мои уровни':`Эпизод ${source.split('-')[1]||1}`;$('libraryDescription').textContent=showUsers?'Черновики и проверенные уровни на этом устройстве.':'Уровни кампании с теми же превью и данными.';$('userSlotCount').textContent=`${state.userSlots.length} / ${MAX_USER_LEVELS}`;$('libraryLevelTitleInput').value=state.level?.title||'';$('libraryDifficultySelect').value=state.difficulty;root.hidden=!showUsers;$('campaignLibrarySection').hidden=showUsers;for(const id of ['newUserLevelButton','randomStarterCheckbox','manualSaveButton','libraryLevelTitleInput','examLevelButton','userSlotCount'])$(id).closest('label,button,span')?.classList.toggle('source-hidden',!showUsers);if(showUsers){if(!state.userSlots.length){const empty=document.createElement('div');empty.className='empty-state';empty.textContent='Пока нет пользовательских уровней. Создайте первый уровень.';root.append(empty);}for(const slot of state.userSlots)root.append(makeLibraryCard(slot));return;}const episode=Number(source.split('-')[1])||1;$('libraryEpisodeSelect').value=String(episode);const difficulty=$('libraryDifficultySelect').value;const campaignSlots=(await dbGetAll()).filter(slot=>slot.kind==='campaign'&&Number(slot.episode||1)===episode).sort((a,b)=>a.sequence-b.sequence);for(const slot of campaignSlots)campaignRoot.append(makeLibraryCard(slot,{campaign:true,difficulty}));}
  ...
  function bindUi(){renderPalette();renderFavorites();renderMobilePalette();
    ...
    $('libraryButton').addEventListener('click',openLibrary);$('mobileLevelButton').addEventListener('click',openLibrary);$('closeLibraryButton').addEventListener('click',closeLibrary);$('newUserLevelButton').addEventListener('click',()=>createUserSlot($('randomStarterCheckbox').checked));$('randomStarterButton').addEventListener('click',()=>createUserSlot(true));$('manualSaveButton').addEventListener('click',()=>shareCurrentLevel().catch(error=>toast(error.message,'error')));$('libraryLevelTitleInput').addEventListener('change',event=>{const title=String(event.target.value).trim().slice(0,48)||'Без названия';mutate('Название изменено',()=>{state.level.title=title;state.slot.title=title;});renderLibrary();});$('libraryCopyMapButton').addEventListener('click',copyMap);$('libraryPasteMapButton').addEventListener('click',pasteMap);$('librarySourceSelect').addEventListener('change',renderLibrary);$('libraryLobbyButton').addEventListener('click',async()=>{await saveNow({revision:true});window.location.assign(gameLobbyUrl().href);});$('libraryEpisodeSelect').addEventListener('change',()=>{$('librarySourceSelect').value=`campaign-${$('libraryEpisodeSelect').value}`;renderLibrary();});$('libraryDifficultySelect').addEventListener('change',renderLibrary);$('examLevelButton').addEventListener('click',async()=>{if(state.slot?.kind!=='user'){toast('Экзамен доступен только для уровней игрока.','error');return;}const ok=await confirmAction('Экзамен уровня','Уровень запустится с обычного входа. Чтобы получить статус «Опубликован», нужно дойти до выхода без смерти.');if(!ok)return;try{localStorage.setItem(EXAM_PENDING_KEY,JSON.stringify({slotKey:state.slotKey,difficulty:state.difficulty}));}catch(error){}closeLibrary();playLevel();});document.querySelectorAll('[data-size]').forEach(button=>button.addEventListener('click',()=>{document.querySelectorAll('[data-size]').forEach(item=>item.classList.toggle('active',item===button));state.chosenSize=button.dataset.size.split('x').map(Number);}));const updateChosenSize=()=>{state.chosenSize=[Number($('newLevelWidth').value),Number($('newLevelHeight').value)];};$('newLevelWidth').addEventListener('change',updateChosenSize);$('newLevelHeight').addEventListener('change',updateChosenSize);
